...
from typing import Any, Dict, List, Optional, Set, Type, Union, Callable, AsyncGenerator
...
from src.assistant.component_manager import ComponentManager, ComponentInterface
...
from src.skills.skill_registry import (
    SkillRegistry, SkillInterface, SkillMetadata, SkillType, 
    SkillState, SkillRegistration, SkillCapability
)
...
class SkillFactory(ComponentInterface):
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        health_status = {
            ...
            "circuit_breakers": {
                ...
                for skill_id, cb in self.circuit_breakers.items()
            },
    ...
    @handle_exceptions
    async def create_skill(self, skill_id: str, config: Optional[Dict[str, Any]] = None,
                          context: Optional[SkillExecutionContext] = None) -> Any:
        """
        ...
        try:
            ...
            async with self.initialization_lock:
                ...
                # Fire event
                await self.event_bus.emit(SkillCreated(
                    skill_id=skill_id,
        ...
        try:
            ...
            async with self.execution_semaphore:
                ...
                try:
                    ...
                    raise TimeoutError(f"Skill execution timed out after {context.timeout_seconds}s")
                ...
                self.logger.debug(f"Executed skill {skill_id} successfully in {execution_time:.2f}ms")
    ...
    @handle_exceptions
    async def compose_skills(self, composition: SkillComposition,
                           input_data: Any, context: Optional[SkillExecutionContext] = None) -> Any:
        """
            ...
            for skill_id in composition.execution_order:
                ...
                if not execution_result.success:
                    ...
                    if error_handling.get("continue_on_error", False):
                        self.logger.warning(f"Skill {skill_id} failed but continuing: {execution_result.error}")
    ...
    async def hot_reload_skill(self, skill_id: str) -> bool:
        ...
        try:
            ...
            await self.event_bus.emit(SkillHotReloaded(
                skill_id=skill_id,
    ...
    @handle_exceptions
    async def create_skill_from_template(self, template_id: str, skill_name: str,
    ...
    def _can_execute_skill(self, circuit_breaker: CircuitBreaker) -> bool:
        ...
        if circuit_breaker.state == CircuitBreakerState.OPEN:
            ...
            if (circuit_breaker.last_failure_time and
                datetime.now(timezone.utc) - circuit_breaker.last_failure_time > circuit_breaker.recovery_timeout):
    ...
    async def _record_success(self, skill_id: str):
        ...
        if circuit_breaker.state == CircuitBreakerState.HALF_OPEN:
            ...
            if circuit_breaker.success_count >= circuit_breaker.success_threshold:
                ...
                await self.event_bus.emit(CircuitBreakerTriggered(
                    skill_id=skill_id,
    ...
    def get_execution_history(self, skill_id: Optional[str] = None, 
                            limit: int = 100) -> List[SkillExecutionResult]:
    ...
    def get_factory_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "circuit_breaker_summary": {
                ...
                "closed": sum(1 for cb in self.circuit_breakers.values() 
                            if cb.state == CircuitBreakerState.CLOSED),
                "open": sum(1 for cb in self.circuit_breakers.values() 
                          if cb.state == CircuitBreakerState.OPEN),
