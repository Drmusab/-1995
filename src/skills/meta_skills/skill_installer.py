...
from src.core.events.event_types import (
    SkillInstallationStarted, SkillInstallationCompleted, SkillInstallationFailed,
    SkillUninstallationStarted, SkillUninstallationCompleted, SkillUninstallationFailed,
    SkillUpdateStarted, SkillUpdateCompleted, SkillUpdateFailed,
...
class SkillInstaller:
    ...
    async def install_skill(self, options: SkillInstallationOptions) -> Dict[str, Any]:
        ...
        await self.event_bus.emit(SkillInstallationStarted(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                self.logger.info(f"Installing skill from {options.source_type.value}: {options.source_path}")
                ...
                await self.event_bus.emit(SkillInstallationCompleted(
                    installation_id=installation_id,
                ...
                # Emit failure event
                await self.event_bus.emit(SkillInstallationFailed(
                    installation_id=installation_id,
                    error_message=str(e),
                    execution_time=duration
                ))
    ...
    async def uninstall_skill(self, skill_name: str, preserve_data: bool = False) -> Dict[str, Any]:
        ...
        await self.event_bus.emit(SkillUninstallationStarted(
        ...
        try:
            ...
            # Log success
            self.logger.info(f"Successfully uninstalled skill {skill_name} in {duration:.2f} seconds")
            
            # Emit completion event
            await self.event_bus.emit(SkillUninstallationCompleted(
                uninstallation_id=uninstallation_id,
                skill_name=skill_name,
                execution_time=duration
            ))
            ...
            # Emit failure event
            await self.event_bus.emit(SkillUninstallationFailed(
                uninstallation_id=uninstallation_id,
                skill_name=skill_name,
                error_message=str(e),
                execution_time=duration
    ...
    async def update_skill(self, skill_name: str, version: Optional[str] = None) -> Dict[str, Any]:
        ...
        await self.event_bus.emit(SkillUpdateStarted(
            update_id=update_id,
            skill_name=skill_name,
        ...
        try:
            ...
            self.logger.info(f"Updating skill: {skill_name} from version {result['previous_version']} to {version or 'latest'}")
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                update_options = SkillInstallationOptions(
                    source_type=SkillSource.GIT_REPOSITORY if metadata.source_url.endswith('.git') else SkillSource.URL,
                ...
                if version is None or pkg_resources.parse_version(updated_metadata.version) > pkg_resources.parse_version(metadata.version):
                    ...
                    try:
                        ...
                        # Copy files
                        target_path = await self._copy_skill_files(temp_path, update_options, updated_metadata)
                    ...
                    self.logger.info(f"Skill '{skill_name}' is already at the latest version: {metadata.version}")
                    result["status"] = "completed"
            ...
            # Emit completion event
            await self.event_bus.emit(SkillUpdateCompleted(
                update_id=update_id,
                skill_name=skill_name,
            ...
            # Emit failure event
            await self.event_bus.emit(SkillUpdateFailed(
                update_id=update_id,
                skill_name=skill_name,
                error_message=str(e),
                execution_time=duration
    ...
    async def list_installed_skills(self, category: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if category:
            skills = [skill for skill in skills if skill.get("category", "").lower() == category.lower()]
        ...
        for skill in skills:
            try:
                ...
                self.logger.warning(f"Could not get full metadata for skill {skill['name']}: {str(e)}")
    ...
    async def _download_skill(self, options: SkillInstallationOptions, temp_path: Path) -> None:
        ...
        if options.source_type == SkillSource.LOCAL_FILE:
            ...
            await self._clone_from_git(options.source_path, temp_path, options.branch, options.version_tag)
        ...
        elif options.source_type == SkillSource.MARKETPLACE or options.source_type == SkillSource.REGISTRY:
            await self._download_from_marketplace(options.source_path, temp_path, options.version_tag)
    ...
    async def _clone_from_git(self, repo_url: str, temp_path: Path, branch: Optional[str] = None, 
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as response:
                    if response.status != 200:
                        raise ValueError(f"Failed to download from URL: {url}, status: {response.status}")
    ...
    async def _download_from_marketplace(self, skill_id: str, temp_path: Path, version: Optional[str] = None) -> None:
    ...
    async def _validate_skill(self, skill_path: Path, options: SkillInstallationOptions) -> None:
        ...
        if pkg_resources.parse_version(metadata.min_system_version) > pkg_resources.parse_version(system_version):
            raise ValueError(
        ...
        try:
            ...
            if module_docstring_match:
                ...
                for line in docstring.split("\n"):
                    if not line.strip() or any(marker in line for marker in ["Author:", "Version:", "License:"]):
                        continue
            ...
            for class_match in class_matches:
                ...
                skill_id_match = re.search(r'def\s+get_skill_id.*?return\s+[\'"](.+?)[\'"]', class_content, re.DOTALL)
                if skill_id_match:
                    metadata["name"] = skill_id_match.group(1)
                
                # Look for get_skill_description method
                skill_desc_match = re.search(r'def\s+get_skill_description.*?return\s+[\'"](.+?)[\'"]', class_content, re.DOTALL)
                ...
                skill_cat_match = re.search(r'def\s+get_skill_category.*?return\s+[\'"](.+?)[\'"]', class_content, re.DOTALL)
                if skill_cat_match:
    ...
    async def _copy_skill_files(self, source_path: Path, options: SkillInstallationOptions, 
                               metadata: SkillMetadata) -> Path:
        """
    ...
    async def _rollback_installation(self, options: SkillInstallationOptions, 
