...
from src.core.events.event_types import (
    SkillOptimizationStarted, SkillOptimizationCompleted, SkillOptimizationFailed,
...
class SkillPerformanceMetrics:
    ...
    parameter_distributions: Dict[str, Counter] = field(default_factory=lambda: defaultdict(Counter))
...
class SkillOptimizer:
    ...
    async def optimize_skill(self, skill_id: str, optimization_type: OptimizationType = OptimizationType.COMPREHENSIVE,
                           level: OptimizationLevel = OptimizationLevel.MODERATE, 
                           mode: OptimizationMode = OptimizationMode.SUGGEST) -> OptimizationResult:
        """
        ...
        await self.event_bus.emit(SkillOptimizationStarted(
        ...
        try:
            with self.tracer.trace(f"skill_optimization_{skill_id}") if self.tracer else None:
                ...
                if optimization_type == OptimizationType.PERFORMANCE:
                    ...
                    await self._optimize_for_quality(skill_id, OptimizationMode.SUGGEST, mode, result)
                    await self._optimize_selection(skill_id, OptimizationMode.SUGGEST, mode, result)
                
                # If changes were applied, get updated metrics
                if mode in [OptimizationMode.APPLY, OptimizationMode.ADAPTIVE] and result.applied_changes:
                ...
                await self.event_bus.emit(SkillOptimizationCompleted(
                    optimization_id=optimization_id,
                    skill_id=skill_id,
                ...
                if self.metrics:
                    ...
                    if result.metrics_after:
                        if result.metrics_before.avg_execution_time > 0:
                            improvement = (
                                (result.metrics_before.avg_execution_time - result.metrics_after.avg_execution_time) / 
                                result.metrics_before.avg_execution_time * 100
                            )
                            if improvement > 0:
                                self.metrics.record("skill_performance_improvement_percent", improvement)
            ...
            # Emit failure event
            await self.event_bus.emit(SkillOptimizationFailed(
                optimization_id=optimization_id,
                skill_id=skill_id,
    ...
    async def get_skill_performance_report(self, skill_id: str) -> Dict[str, Any]:
        ...
        optimization_history = [
            result for result in self.optimization_cache.values()
    ...
    async def analyze_system_wide_performance(self) -> Dict[str, Any]:
        ...
        avg_execution_time = np.mean([m.avg_execution_time for m in skill_metrics.values() if m.execution_count > 0]) if skill_metrics else 0
        system_success_rate = np.mean([m.success_rate for m in skill_metrics.values() if m.execution_count > 0]) if skill_metrics else 1.0
        ...
        slowest_skills = sorted(
            [(skill_id, metrics) for skill_id, metrics in skill_metrics.items() if metrics.execution_count > 0],
        ...
        report = {
            ...
            "most_used_skills": [
                {"skill_id": s[0], "execution_count": s[1].execution_count, "avg_execution_time": s[1].avg_execution_time}
                for s in most_used_skills
            ],
            "slowest_skills": [
                {"skill_id": s[0], "avg_execution_time": s[1].avg_execution_time, "execution_count": s[1].execution_count}
                for s in slowest_skills
            ],
            "least_reliable_skills": [
                {"skill_id": s[0], "success_rate": s[1].success_rate, "error_count": s[1].error_count}
    ...
    async def benchmark_skill(self, skill_id: str, test_parameters: List[Dict[str, Any]] = None, 
        ...
        benchmark_report = {
            ...
            "overall_avg_time": np.mean([r["avg_execution_time"] for r in benchmark_results if r["avg_execution_time"] is not None]),
            "overall_success_rate": np.mean([r["success_rate"] for r in benchmark_results])
        }
        
        return benchmark_report
    
    async def optimize_skill_selection(self, similar_skills: List[str], 
    ...
    async def apply_optimization_suggestions(self, optimization_id: str) -> Dict[str, Any]:
        ...
        for suggestion in result.suggestions:
            ...
            try:
                ...
                result.applied_changes.append(f"Applied {suggestion.optimization_type.value} optimization: {suggestion.description}")
            except Exception as e:
    ...
    async def _get_skill_performance_metrics(self, skill_id: str) -> SkillPerformanceMetrics:
        ...
        for execution in execution_history:
            ...
            if timestamp and (metrics.last_executed is None or timestamp > metrics.last_executed):
                try:
                    metrics.last_executed = timestamp if isinstance(timestamp, datetime) else datetime.fromisoformat(timestamp)
        ...
        metrics.success_rate = success_count / metrics.execution_count if metrics.execution_count > 0 else 1.0
        ...
        if cached_times:
            ...
            metrics.cache_hit_rate = cached_count / metrics.execution_count if metrics.execution_count > 0 else 0
        ...
        if self.memory_profiler:
            try:
                ...
                self.logger.warning(f"Error profiling memory usage for skill '{skill_id}': {str(e)}")
        
        return metrics
    
    async def _optimize_for_performance(self, skill_id: str, level: OptimizationLevel,
                                      mode: OptimizationMode, result: OptimizationResult) -> None:
        """
        ...
        if code_analysis.get("async_issues"):
            suggestions.append(OptimizationSuggestion(
                ...
                impact="High",
                suggested_changes={
                    "async_patterns": code_analysis.get("async_issues")
                },
        ...
        if code_analysis.get("io_operations") and level != OptimizationLevel.MINIMAL:
            suggestions.append(OptimizationSuggestion(
                ...
                impact="Medium",
                suggested_changes={
                    "io_operations": code_analysis.get("io_operations")
                },
                ...
                suggestions.append(OptimizationSuggestion(
                    ...
                    suggested_changes={
                        "caching": {
                            "enabled": True,
                            "cache_key_params": [
                                param for param, counter in metrics.parameter_distributions.items()
                                if counter.most_common(1) and counter.most_common(1)[0][1] > metrics.execution_count * 0.3
                            ]
                        }
                    },
        ...
        if mode == OptimizationMode.APPLY:
            ...
            for suggestion in suggestions:
                if suggestion.optimization_type == OptimizationType.CACHING:
                    ...
                    applied_count += 1
                    result.applied_changes.append(f"Applied caching optimization for skill '{skill_id}'")
                
                # Other optimizations that modify code would be applied here
                # This is a simplified implementation
            
            if applied_count > 0:
                self.logger.info(f"Applied {applied_count} performance optimizations to skill '{skill_id}'")
    
    async def _optimize_for_memory(self, skill_id: str, level: OptimizationLevel,
                                 mode: OptimizationMode, result: OptimizationResult) -> None:
        """
        ...
        if code_analysis.get("large_responses") and level == OptimizationLevel.AGGRESSIVE:
            suggestions.append(OptimizationSuggestion(
                ...
                impact="High",
                suggested_changes={
                    "streaming": code_analysis.get("large_responses")
                },
        ...
        if mode == OptimizationMode.APPLY:
            ...
            if applied_count > 0:
                self.logger.info(f"Applied {applied_count} memory optimizations to skill '{skill_id}'")
    
    async def _optimize_for_quality(self, skill_id: str, level: OptimizationLevel,
                                  mode: OptimizationMode, result: OptimizationResult) -> None:
        """
        ...
        if metrics.parameter_distributions:
            ...
            if potential_improvements and level != OptimizationLevel.MINIMAL:
                suggestions.append(OptimizationSuggestion(
                    ...
                    suggested_changes={
                        "parameter_tuning": {
                            "parameters_to_tune": potential_improvements
                        }
                    },
        ...
        if model_calls and level == OptimizationLevel.AGGRESSIVE:
            suggestions.append(OptimizationSuggestion(
                ...
                suggested_changes={
                    "model_optimization": {
                        "current_models": [call.get("model") for call in model_calls if "model" in call],
        ...
        if mode == OptimizationMode.APPLY:
            ...
            if applied_count > 0:
                self.logger.info(f"Applied {applied_count} quality optimizations to skill '{skill_id}'")
    
    async def _optimize_for_caching(self, skill_id: str, level: OptimizationLevel,
                                  mode: OptimizationMode, result: OptimizationResult) -> None:
        """
        ...
        if metrics.execution_count > 10:
            ...
            for param, counter in metrics.parameter_distributions.items():
                ...
                if most_common and most_common[0][1] > 1:  # Param value used more than once
                    ...
                    if most_common[0][1] > metrics.execution_count * 0.2:  # Used in at least 20% of executions
        ...
        if metrics.cache_hit_rate > 0 and metrics.cache_hit_rate < 0.5:
            ...
            if current_cache_keys:
                ...
                improved_keys = []
                for param, counter in metrics.parameter_distributions.items():
                    if param not in current_cache_keys and len(counter) < metrics.execution_count * 0.5:
            ...
            suggestions.append(OptimizationSuggestion(
                ...
                suggested_changes={
                    "semantic_caching": {
                        ...
                        "target_model_calls": [call.get("location", "unknown") for call in model_calls]
                    }
                },
        ...
        if mode == OptimizationMode.APPLY:
            ...
            for suggestion in suggestions:
                if suggestion.optimization_type == OptimizationType.CACHING:
                    ...
                    await self._apply_caching_optimization(skill_id, suggestion)
                    applied_count += 1
                    result.applied_changes.append(f"Applied caching optimization: {suggestion.description}")
            
            if applied_count > 0:
                self.logger.info(f"Applied {applied_count} caching optimizations to skill '{skill_id}'")
    
    async def _optimize_parameters(self, skill_id: str, level: OptimizationLevel,
                                 mode: OptimizationMode, result: OptimizationResult) -> None:
        """
        ...
        if validation_issues:
            suggestions.append(OptimizationSuggestion(
                ...
                impact="Medium",
                suggested_changes={
                    "parameter_validation": validation_issues
                },
        ...
        if mode == OptimizationMode.APPLY:
            ...
            if applied_count > 0:
                self.logger.info(f"Applied {applied_count} parameter optimizations to skill '{skill_id}'")
    
    async def _optimize_code(self, skill_id: str, level: OptimizationLevel,
                           mode: OptimizationMode, result: OptimizationResult) -> None:
        """
            ...
            for issue in code_quality_issues:
                ...
                if suggested_fix:
                    suggestions.append(OptimizationSuggestion(
                        ...
                        impact="Medium",
                        code_changes={
                            issue.get("original_code", ""): suggested_fix
                        },
