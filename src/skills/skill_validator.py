...
class SkillValidator:
    ...
    async def validate_skill(
        self, skill_id: str, skill_class: Type, metadata: SkillMetadata, profile: str = "full"
    ) -> ValidationReport:
        """
        ...
        await self.event_bus.emit(
            SkillValidationStarted(skill_id=skill_id, validation_id=validation_id)
        )

        # Create validation report
        report = ValidationReport(skill_id=skill_id, validation_id=validation_id)
        ...
        for rule_id in rule_ids:
            ...
            try:
                result = await self._run_validation_rule(rule, skill_id, skill_class, metadata)
        ...
        self.logger.info(
            f"Validation completed for skill {skill_id}: "
    ...
    async def _run_validation_rule(
        self, rule: ValidationRule, skill_id: str, skill_class: Type, metadata: SkillMetadata
    ) -> ValidationResult:
    ...
    def _calculate_report_metrics(self, report: ValidationReport):
        ...
        if report.total_checks > 0:
            ...
            report.overall_score = (
                (report.total_checks - failed_count) / report.total_checks
            ) * 100
        else:
    ...
    async def _validate_required_methods(
        self, skill_id: str, skill_class: Type, metadata: SkillMetadata, config: Dict[str, Any]
    ) -> tuple:
    ...
    async def _validate_method_signatures(
        ...
        if issues:
            return False, f"Method signature issues: {'; '.join(issues)}", {"issues": issues}

        return True, "All method signatures valid", {}

    async def _validate_dangerous_imports(
        self, skill_id: str, skill_class: Type, metadata: SkillMetadata, config: Dict[str, Any]
    ) -> tuple:
        ...
        if found_dangerous:
            return (
                False,
                f"Dangerous imports detected: {', '.join(found_dangerous)}",
    ...
    async def _validate_file_access(
        ...
        if found_patterns:
            return (
                False,
                f"File access patterns detected: {len(found_patterns)} patterns",
    ...
    async def _validate_network_access(
        ...
        if found_patterns:
            return (
                False,
                f"Network access patterns detected: {len(found_patterns)} patterns",
                {"patterns_found": len(found_patterns)},
            )
    ...
    async def _validate_memory_usage(
        ...
        try:
            ...
            if estimated_memory_kb > max_memory_kb:
                return (
                    False,
                    f"Estimated memory usage too high: {estimated_memory_kb:.1f}KB",
                    {"estimated_memory_kb": estimated_memory_kb, "max_allowed_kb": max_memory_kb},
                )

            return (
                True,
                f"Memory usage within limits: {estimated_memory_kb:.1f}KB",
    ...
    async def _validate_execution_time(
        ...
        try:
            ...
            if complexity_score > max_complexity:
                return (
                    False,
                    f"Code complexity too high: {complexity_score}",
                    {"complexity_score": complexity_score, "max_allowed": max_complexity},
                )

            return (
                True,
                f"Code complexity within limits: {complexity_score}",
    ...
    async def _validate_python_version(
        self, skill_id: str, skill_class: Type, metadata: SkillMetadata, config: Dict[str, Any]
    ) -> tuple:
        ...
        if hasattr(metadata, "python_version_min"):
            ...
            if current_version < min_version:
                return (
                    False,
                    f"Python version too old: {current_version} < {min_version}",
                    {
        ...
        if hasattr(metadata, "python_version_max"):
            ...
            if current_version > max_version:
                return (
                    False,
                    f"Python version too new: {current_version} > {max_version}",
                    {
                        "current_version": str(current_version),
        ...
        return (
            True,
            f"Python version compatible: {current_version}",
            {"current_version": str(current_version)},
        )
    ...
    async def _validate_dependency_versions(
        ...
        return (
            True,
            f"Dependencies specified: {len(metadata.dependencies)}",
    ...
    async def _validate_resource_limits(
        ...
        if issues:
            return False, f"Resource limits exceeded: {'; '.join(issues)}", {"issues": issues}

        return True, "Resource requirements within limits", {}

    async def _validate_required_config(
        self, skill_id: str, skill_class: Type, metadata: SkillMetadata, config: Dict[str, Any]
    ) -> tuple:
        ...
        return (
            True,
            f"Configuration schema has {len(required_fields)} required fields",
