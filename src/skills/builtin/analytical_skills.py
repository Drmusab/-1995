...
from src.core.events.event_types import (
    ...
    AnalysisStarted, AnalysisCompleted, AnalysisError, LearningEventOccurred
)
...
class TextAnalyzer(AnalyticalSkill):
    ...
    def get_skill_parameters(self) -> Dict[str, Dict[str, Any]]:
        return {
            ...
            "analysis_types": {
                ...
                "items": {
                    "type": "string",
                    "enum": ["keywords", "themes", "sentiment", "entities", "readability", 
                            "structure", "summary", "discourse", "complexity", "all"]
                },
    ...
    async def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        await self.event_bus.emit(AnalysisStarted(
        ...
        try:
            with self.tracer.trace("text_analysis") if self.tracer else None:
                ...
                analysis_types = kwargs.get("analysis_types", ["keywords", "themes", "sentiment", "summary"])
                ...
                if "all" in analysis_types:
                    analysis_types = ["keywords", "themes", "sentiment", "entities", 
                                     "readability", "structure", "summary", "discourse", 
                                     "complexity"]
            ...
            await self.event_bus.emit(AnalysisError(
                analysis_id=execution_id,
                error_message=str(e)
            ))
    ...
    async def _extract_keywords(self, text: str, max_keywords: int, depth: str) -> Dict[str, Any]:
        ...
        for line in extraction_result.split("\n"):
            ...
            if keyword_match:
                ...
                score_match = re.search(r'(?:relevance|score|rating)?\s*[:=]?\s*(\d+\.\d+|\d+\/\d+|\d+\s*%)', rest, re.IGNORECASE)
                if score_match:
                    ...
                    if "/" in score_str:
                        ...
                        current_info["relevance"] = round(float(score_str.replace("%", "")) / 100, 2)
                    else:
                ...
                elif "relevance" in line.lower() or "score" in line.lower() or "importance" in line.lower():
                    if ":" in line:
                        ...
                        if re.match(r'\d+\.\d+|\d+\/\d+|\d+\s*%', value):
                            if "/" in value:
                                ...
                                current_info["relevance"] = round(float(value.replace("%", "")) / 100, 2)
                            else:
    ...
    async def _extract_themes(self, text: str, depth: str) -> Dict[str, Any]:
        ...
        for line in themes_result.split("\n"):
            ...
            if theme_match:
                ...
                elif "evidence" in line.lower() or "example" in line.lower() or "support" in line.lower():
                    value = line.split(":", 1)[1].strip() if ":" in line else line
                ...
                elif not any(key in current_info for key in ["description", "evidence", "significance"]):
    ...
    async def _analyze_sentiment(self, text: str, depth: str) -> Dict[str, Any]:
        ...
        if depth in [AnalysisLevel.INTERMEDIATE.value, AnalysisLevel.ADVANCED.value, AnalysisLevel.EXPERT.value]:
            ...
            for section in sections:
                ...
                if "tone" in section or "emotion" in section:
                    tones = re.findall(r'(joy|sadness|anger|fear|surprise|disgust|trust|anticipation|optimism|pessimism|anxiety|confidence|confusion|nostalgia)', section)
                ...
                if "shift" in section or "change" in section:
                    shifts = [line.strip() for line in section.split("\n") if line.strip() and ":" in line]
                    sentiment_shifts.extend(shifts)
                
                if "paragraph" in section and "sentiment" in section:
                    para_sents = [line.strip() for line in section.split("\n") if line.strip() and ":" in line]
    ...
    async def _extract_entities(self, text: str, depth: str) -> Dict[str, Any]:
        ...
        if depth in [AnalysisLevel.INTERMEDIATE.value, AnalysisLevel.ADVANCED.value, AnalysisLevel.EXPERT.value]:
            ...
            for line in detailed_entities.split("\n"):
                ...
                category_match = re.match(r'^(people|persons?|individuals?|organizations?|companies|locations?|places|dates?|times?|quantities|numbers|events?|concepts?|other)[:\-]', line.lower())
                if category_match:
                ...
                if current_category and line:
                    ...
                    if entity_match:
                        ...
                        entity_details = entity_match.group(2).strip() if entity_match.group(2) else None
    ...
    async def _analyze_readability(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "readability": {
                    "error": "Could not calculate readability scores"
                }
            }
    ...
    async def _analyze_structure(self, text: str, depth: str) -> Dict[str, Any]:
        ...
        for line in structure_analysis.split("\n"):
            ...
            structure_types = ["introduction", "body", "conclusion", "argument", "flow", "coherence", 
                              "narrative", "progression", "sequence", "hierarchy"]
    ...
    async def _analyze_discourse(self, text: str, depth: str) -> Dict[str, Any]:
        ...
        for line in discourse_analysis.split("\n"):
            ...
            section_match = re.match(r'^(rhetorical devices|persuasion techniques|tone|voice|audience|argumentation|appeals|stance|register|fallacies)[:\-]', line.lower())
            if section_match:
    ...
    async def _analyze_complexity(self, text: str) -> Dict[str, Any]:
        ...
        long_sentences_percentage = len(long_sentences) / len(sentence_lengths) if sentence_lengths else 0
    ...
    def _interpret_complexity(self, avg_sentence_length: float, avg_word_length: float, 
                             lexical_diversity: float) -> str:
    ...
    def _format_as_text(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "sentiment" in analysis:
            ...
            formatted_text += f"- Overall sentiment: {sentiment.get('overall_sentiment', 'neutral')}\n"
    ...
    def _format_as_table(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "themes" in analysis:
            ...
            rows = [
                [theme["theme"], theme.get("description", "")]
                for theme in analysis["themes"]
            ]
    ...
    def get_skill_parameters(self) -> Dict[str, Dict[str, Any]]:
        return {
            ...
            "analysis_types": {
                ...
                "items": {
                    ...
                    "enum": ["descriptive", "distribution", "correlation", 
                            "comparison", "trends", "outliers", "clustering", "all"]
                },
            ...
            "visualizations": {
                ...
                "items": {
                    "type": "string",
                    "enum": ["none", "charts", "tables", "text", "all"]
                },
                "default": ["tables", "text"]
            },
            "depth": {
    ...
    async def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            with self.tracer.trace("data_analysis") if self.tracer else None:
                ...
                # Handle "all" analysis type
                if "all" in analysis_types:
                    analysis_types = ["descriptive", "distribution", "correlation", 
                                     "comparison", "trends", "outliers", "clustering"]
                ...
                if target_columns:
                    ...
                    if not available_columns:
                        raise ValueError(f"None of the target columns {target_columns} found in data")
