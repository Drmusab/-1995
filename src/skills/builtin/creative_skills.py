...
class StoryGenerator(CreativeSkill):
    ...
    def get_skill_description(self) -> str:
        return (
            "Generates creative stories in various genres and formats with customizable parameters."
        )
        ...
        return [
            {
                "parameters": {
                    ...
                    "length": "medium",
                    "characters": [
                        "Dr. Eleanor Voss, a quantum physicist",
                        "Marcus, her younger self",
                    ],
                },
    ...
    async def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            with self.tracer.trace("story_generation") if self.tracer else None:
                ...
                word_count = {"short": 500, "medium": 1500, "long": 3000}.get(length, 1500)
    ...
    async def _generate_plot_structure(
        self, theme: str, genre: str, tone: str, characters: List[str], setting: str
    ) -> Dict[str, Any]:
        ...
        if len(sections) >= 5:
            ...
            plot_structure["rising_action"] = [
                event.strip()
                for event in sections[1].replace("Rising Action: ", "").split(". ")
                if event.strip()
            ]
            plot_structure["climax"] = sections[2].replace("Climax: ", "").strip()
            plot_structure["falling_action"] = [
                event.strip()
                for event in sections[3].replace("Falling Action: ", "").split(". ")
                if event.strip()
            ]
...
class PoetryGenerator(CreativeSkill):
    ...
    def _determine_poem_length(self, form: str, length_preference: str) -> str:
        ...
        length_guide = {"short": "4-8 lines", "medium": "12-20 lines", "long": "25-40 lines"}
    ...
    async def _analyze_poetic_elements(self, poem: str, form: str) -> Dict[str, Any]:
        ...
        # Parse the analysis
        elements = {"imagery": [], "devices": [], "themes": [], "tone": "", "structure": {}}
        ...
        for line in analysis_text.split("\n"):
            ...
            if (
                ...
                "tone" in line.lower() and ":" in line or "emotion" in line.lower() and ":" in line
            ):
            ...
            if current_section == "imagery":
                ...
                if "rhyme scheme" in line.lower():
                    elements["structure"]["rhyme_scheme"] = (
                        line.split(":", 1)[1].strip() if ":" in line else line
                    )
                elif "meter" in line.lower():
                    elements["structure"]["meter"] = (
                        line.split(":", 1)[1].strip() if ":" in line else line
                    )
                elif "stanza" in line.lower():
                    elements["structure"]["stanza_pattern"] = (
                        line.split(":", 1)[1].strip() if ":" in line else line
                    )
                else:
                    key = "general"
...
class CharacterCreator(CreativeSkill):
    ...
    def get_skill_parameters(self) -> Dict[str, Dict[str, Any]]:
        return {
            ...
            "age": {"type": "string", "description": "Character age", "required": False},
            "occupation": {
                "type": "string",
    ...
    def _construct_character_prompt(
        ...
        specifics_text = (
            ". ".join(specifics)
            if specifics
            else "Create appropriate details based on the genre and archetype."
        )
    ...
    async def _extract_character_data(self, character_text: str) -> Dict[str, Any]:
        ...
        for line in extraction_result.split("\n"):
            ...
            if "basic information" in line.lower() or "basic info" in line.lower():
                ...
                "visual" in line.lower()
                or "appearance" in line.lower()
