...
class SkillRegistry:
        ...
        try:
            ...
            if skill_id in self.skills:
                ...
                if existing.metadata.version == metadata.version:
                    self.logger.warning(f"Skill {skill_id} version {metadata.version} already registered")
                    return False
                else:
                    self.logger.info(f"Updating skill {skill_id} from {existing.metadata.version} to {metadata.version}")
            ...
            # Fire event
            await self.event_bus.emit(SkillRegistered(
                skill_id=skill_id,
    ...
    async def unregister_skill(self, skill_id: str, reason: str = "manual") -> bool:
        ...
        try:
            ...
            # Fire event
            await self.event_bus.emit(SkillUnregistered(
                skill_id=skill_id,
                skill_name=registration.metadata.name,
                reason=reason
            ))
    ...
    def search_skills(self, query: str, filters: Optional[Dict[str, Any]] = None) -> List[SkillRegistration]:
        ...
        for skill_id, registration in self.skills.items():
            ...
            if query.lower() in metadata.name.lower() or \
               ...
               any(query.lower() in tag.lower() for tag in metadata.tags):
        ...
        for python_file in path.rglob("*.py"):
            ...
            try:
                ...
                for name, obj in inspect.getmembers(module, inspect.isclass):
                    if (issubclass(obj, SkillInterface) and 
                        obj != SkillInterface and 
    ...
    async def update_skill_state(self, skill_id: str, new_state: SkillState, 
        ...
        # Fire event
        await self.event_bus.emit(SkillStateChanged(
            skill_id=skill_id,
            old_state=old_state,
            new_state=new_state,
            reason=reason
