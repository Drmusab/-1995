...
from src.core.events.event_types import (
    GraphRetrievalCompleted,
    GraphRetrievalStarted,
    ...
    TemporalRetrievalCompleted,
    TemporalRetrievalStarted,
)
            ...
            "items": [
                {
                    ...
                    "relevance": score,
                    "created_at": (
                        memory.metadata.created_at.isoformat() if memory.metadata else None
                    ),
...
class MemoryRetriever:
    ...
    def __init__(self, container: Container):
        ...
        try:
            ...
            self.logger.warning(
                "MemoryGraphStore not available, graph-based retrieval will be limited"
            )
        ...
        try:
            ...
            self.logger.warning(
                "MemoryContextManager not available, contextual retrieval will be limited"
            )
    ...
    async def retrieve_recent(
        self, session_id: str, limit: int = 5, memory_types: Optional[List[MemoryType]] = None
    ) -> List[MemoryItem]:
        """
        ...
        results.sort(key=lambda x: x.metadata.last_accessed or x.metadata.created_at, reverse=True)
        ...
        try:
            ...
            if MemoryType.SEMANTIC in request.memory_types:
                ...
                semantic_items = await self.semantic_memory.retrieve_relevant(
                    request.query, context=request.context, limit=request.max_results
                )
        ...
        try:
            ...
            if not time_range and "time_frame" in request.params:
                ...
                if frame == "today":
                    ...
                    start_time_dt = (end_time - timedelta(days=1)).replace(
                        hour=0, minute=0, second=0, microsecond=0
                    )
                    end_time = end_time.replace(hour=0, minute=0, second=0, microsecond=0)
                elif frame == "this_week":
                    start_time_dt = (end_time - timedelta(days=end_time.weekday())).replace(
                        hour=0, minute=0, second=0, microsecond=0
                    )
                elif frame == "last_week":
                    start_of_this_week = (end_time - timedelta(days=end_time.weekday())).replace(
                        hour=0, minute=0, second=0, microsecond=0
                    )
                    start_time_dt = start_of_this_week - timedelta(days=7)
                    end_time = start_of_this_week
                elif frame == "this_month":
                    start_time_dt = end_time.replace(
            ...
            if self.graph_store and MemoryType.EPISODIC in request.memory_types:
                ...
                await self.event_bus.emit(
                    GraphRetrievalCompleted(
                        ...
                        result_count=sum(
                            1 for _, score in all_items if score >= request.min_relevance
                        ),
                    )
                )
            ...
            if self.graph_store and seed_memories:
                ...
                if seed_node_ids:
                    ...
                    nodes, edges = await self.graph_store.find_subgraph(
                        seed_node_ids, include_edges=True, max_distance=2
                    )
                    ...
                    await self.event_bus.emit(
                        GraphRetrievalCompleted(
                            query=request.query, node_count=len(nodes), result_count=len(all_items)
                        )
                    )
    ...
    async def _importance_retrieval(self, request: RetrievalRequest) -> RetrievalResult:
        ...
        try:
            ...
            if MemoryType.WORKING in request.memory_types and request.session_id:
                tasks.append(
                    self.working_memory.get_recent_items(
                        request.session_id, limit=request.max_results * 2
                    )
                )
            ...
            if MemoryType.EPISODIC in request.memory_types:
                if request.session_id:
                    tasks.append(
                        self.episodic_memory.get_session_memories(
                            request.session_id, limit=request.max_results * 2
                        )
                    )
                elif request.user_id:
                    tasks.append(
                        self.episodic_memory.get_user_memories(
                            request.user_id, limit=request.max_results * 2
                        )
                    )
            ...
            if MemoryType.SEMANTIC in request.memory_types:
                tasks.append(
                    self.semantic_memory.retrieve_relevant(
                        request.query, request.context, limit=request.max_results
                    )
                )
