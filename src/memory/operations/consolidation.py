...
from src.core.events.event_types import (
    ...
    MemoryConsolidationFailed, MemoryItemStored, MemoryItemUpdated,
    MemoryGraphUpdated, SemanticMemoryExtracted
)
...
from src.memory.core_memory.base_memory import (
    BaseMemory, MemoryItem, MemoryType, MemoryStorageType,
    MemoryRetentionPolicy, MemoryMetadata, MemoryError
)
...
from src.memory.storage.memory_graph import (
    MemoryGraphStore, GraphNode, GraphEdge, 
    GraphNodeType, RelationshipType
)
...
class MemoryConsolidator:
    ...
    def __init__(self, container: Container):
        ...
        try:
            ...
            self.logger.warning("MemoryGraphStore not available, relationship consolidation will be limited")
        ...
        try:
            ...
            self.logger.warning("ModelRouter not available, advanced consolidation features will be limited")
        ...
        self.auto_semantic_extraction = self.consolidation_config.get("auto_semantic_extraction", True)
        self.auto_relationship_building = self.consolidation_config.get("auto_relationship_building", True)
        self.working_memory_threshold = self.consolidation_config.get("working_memory_threshold", 100)
        self.temporal_schedule_minutes = self.consolidation_config.get("temporal_schedule_minutes", 60)
    ...
    async def consolidate_session(
        ...
        job = ConsolidationJob(
            ...
            target_types=[MemoryType.EPISODIC] + ([MemoryType.SEMANTIC] if include_semantic else []),
    ...
    async def extract_semantic_memory(
        ...
        try:
            ...
            await self.event_bus.emit(SemanticMemoryExtracted(
            ...
            if self.metrics:
                ...
                self.metrics.record("memory_semantic_extraction_time_seconds", time.time() - start_time)
    ...
    async def build_relationships(
        ...
        try:
            ...
            for memory_id in memory_ids:
                # Try to retrieve from each memory system
                memory = None
                for memory_system in [self.working_memory, self.episodic_memory, self.semantic_memory]:
        ...
        async with self._semaphore:
            ...
            try:
                ...
                if self.metrics:
                    ...
                    self.metrics.increment("memory_consolidation_items_processed", 
                                         len(results.get("memory_ids", [])))
                    self.metrics.record("memory_consolidation_processing_time_seconds", 
                ...
                await self.event_bus.emit(MemoryConsolidationFailed(
                    job_id=job.job_id,
                    error=error_message,
        ...
        for context_id, items in context_groups.items():
            ...
            if MemoryType.EPISODIC in job.target_types:
                ...
                if self.graph_store and self.auto_relationship_building:
                    ...
                    relationships = await self._build_memory_relationships(episodic_memories, context)
    ...
    async def _consolidate_to_episodic(
        ...
        # Standard and higher consolidation levels perform summarization
            ...
            source_memories.sort(key=lambda x: x.metadata.created_at if x.metadata else datetime.now(timezone.utc))
            
            # Group by similar time windows (5-minute intervals)
            time_windows = defaultdict(list)
            for memory in source_memories:
                created_time = memory.metadata.created_at if memory.metadata else datetime.now(timezone.utc)
            ...
            for window_key, window_memories in time_windows.items():
                ...
                for memory in window_memories:
                    if memory.metadata:
                        ...
                        if memory.metadata.custom_metadata and 'emotion' in memory.metadata.custom_metadata:
                            emotion = memory.metadata.custom_metadata['emotion']
                
                # Normalize importance
                importance = min(1.0, importance * 1.1)  # Slightly boost importance for consolidated memories
        ...
        try:
            ...
            for memory in memories:
                ...
                if not node:
                    ...
                    properties = {
                        "created_at": memory.metadata.created_at.isoformat() if memory.metadata else datetime.now(timezone.utc).isoformat(),
                    ...
                    # Create node
                    labels = memory.metadata.tags if memory.metadata and memory.metadata.tags else set()
