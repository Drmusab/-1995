...
from typing import Optional, Dict, Any, List, Set, Tuple, Union, Callable, TypeVar, Generic, DefaultDict
import asyncio
import time
...
from src.core.events.event_types import (
    ...
    MemoryItemStored, MemoryItemRetrieved, MemoryItemUpdated
)
        ...
        if len(self.elements) >= self.max_size:
            ...
            if expired:
                ...
                if (element.priority.value < candidates[0][1] or 
                    (element.priority.value == candidates[0][1] and 
                     element.relevance < candidates[0][2])):
                    return False
...
class MemoryContextManager:
    ...
    def __init__(self, container: Container):
        ...
        try:
            ...
            self.logger.warning("VectorMemoryStore not available, vector-based context retrieval will be limited")
            self.vector_store = None
        
        try:
            self.graph_store = container.get(MemoryGraphStore)
        except Exception:
            self.logger.warning("MemoryGraphStore not available, graph-based context retrieval will be limited")
        ...
        try:
            ...
            self.logger.warning("SentimentAnalyzer not available, emotional context will be limited")
        ...
        try:
            ...
            self.logger.warning("ModelRouter not available, semantic context operations will be limited")
        ...
        self.auto_prune_interval = self.context_config.get("auto_prune_interval", 10)  # Number of updates
    ...
    async def initialize_context(self, session_id: str) -> str:
        ...
        window = ContextWindow(
            ...
            max_size=self.default_window_size
        )
        ...
        # Emit event
        await self.event_bus.emit(ContextWindowChanged(
            session_id=session_id,
            window_id=window_id,
    ...
    async def add_context_element(
        ...
        # Emit event
        await self.event_bus.emit(ContextUpdated(
            session_id=session_id,
        ...
        if self.metrics:
            ...
            self.metrics.gauge("context_window_size", len(context.window.elements), {"session_id": session_id})
    ...
    async def remove_context_element(
        self,
        session_id: str,
        element_id: str
    ) -> bool:
    ...
    async def get_context_element(
        ...
        if element_id in self._element_cache:
            ...
            # Verify session
            if session_id == self._contexts.get(session_id, ConversationContext(session_id="", window=None)).session_id:
                return element
    ...
    async def add_entity(
        ...
        if existing:
            ...
            # Emit event
            await self.event_bus.emit(ContextEntityDetected(
                session_id=session_id,
        ...
        if self.metrics:
            ...
            self.metrics.gauge("context_entity_count", len(context.entities), {"session_id": session_id})
    ...
    async def remove_entity(
        ...
        # Emit event
        if entity:
            await self.event_bus.emit(ContextEntityRemoved(
                session_id=session_id,
                entity_id=entity_id,
    ...
    async def clear_context(
        ...
        # Emit event
        await self.event_bus.emit(ContextCleared(
            session_id=session_id,
            window_id=context.window.window_id
        ))
    ...
    async def restore_context(
        ...
        try:
            ...
            if not memory or not isinstance(memory.content, dict):
                self.logger.warning(f"Cannot restore context: memory {memory_id} not found or invalid format")
                return False
            ...
            if "window" in memory.content and "elements" in memory.content["window"]:
                for element_data in memory.content["window"]["elements"]:
                    ...
                    element = ContextElement(
                        ...
                        created_at=datetime.fromisoformat(element_data.get("created_at", datetime.now(timezone.utc).isoformat())),
                        updated_at=datetime.fromisoformat(element_data.get("updated_at", datetime.now(timezone.utc).isoformat())),
                        source=element_data.get("source"),
                        memory_id=element_data.get("memory_id"),
                        relevance=element_data.get("relevance", 1.0),
                        ttl=timedelta(seconds=int(element_data.get("ttl", 0))) if element_data.get("ttl") else None,
            ...
            if "entities" in memory.content:
                for entity_id, entity_data in memory.content["entities"].items():
                    ...
                    entity = ContextEntity(
                        ...
                        first_mentioned_at=datetime.fromisoformat(entity_data.get("first_mentioned_at", datetime.now(timezone.utc).isoformat())),
                        last_mentioned_at=datetime.fromisoformat(entity_data.get("last_mentioned_at", datetime.now(timezone.utc).isoformat())),
            ...
            # Emit event
            await self.event_bus.emit(ContextRestored(
                session_id=session_id,
                window_id=context.window.window_id,
                memory_id=memory_id,
                element_count=len(context.window.elements),
    ...
    async def get_memories_for_context(
        ...
        try:
            ...
            if "entities" in context:
                ...
                ] if isinstance(context["entities"], dict) else context["entities"]
            ...
            if "conversation" in context:
                ...
                elif isinstance(context["conversation"], dict) and "text" in context["conversation"]:
        ...
        try:
            ...
            if isinstance(element.content, str):
                ...
                if "text" in element.content:
                    ...
                    text = element.content["content"] if isinstance(element.content["content"], str) else None
