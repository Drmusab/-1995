...
from typing import Optional, Dict, Any, List, Set, Callable, Type, Union, AsyncGenerator, TypeVar, Generic, Protocol
import asyncio
import json
import time
...
class MemoryError(Exception):
    ...
    def __init__(self, message: str, memory_id: Optional[str] = None, 
                 error_code: Optional[str] = None, owner_id: Optional[str] = None):
...
class MemorySearchResult:
    ...
    def filter_by_type(self, memory_type: MemoryType) -> 'MemorySearchResult':
        ...
        return MemorySearchResult(
            ...
            len(filtered_items),
            self.query_time,
            {k: v for k, v in self.relevance_scores.items() if k in [item.memory_id for item in filtered_items]}
        )
...
class MemoryCache:
    ...
    async def cleanup_expired(self) -> int:
        ...
        async with self._cache_lock:
            current_time = time.time()
            expired_keys = [
                key for key, expiry in self._expiration_times.items()
...
class AbstractMemoryManager(ABC):
    ...
    @abstractmethod
    async def search_memories(self, query: Any, memory_type: Optional[MemoryType] = None) -> MemorySearchResult:
        """Search memories."""
        pass
    
    @abstractmethod
    async def get_recent_memories(self, memory_type: Optional[MemoryType] = None, limit: int = 10) -> List[MemoryItem]:
        """Get recent memories."""
        pass
...
class MemoryAccessController:
    ...
    async def encrypt_memory_content(self, item: MemoryItem) -> MemoryItem:
        ...
        if item.metadata.sensitivity in [MemorySensitivity.HIGH, MemorySensitivity.CRITICAL]:
            try:
                content_str = json.dumps(item.content) if isinstance(item.content, dict) else str(item.content)
...
class MemoryUtils:
    ...
    def deserialize_memory_item(data: Dict[str, Any]) -> MemoryItem:
        ...
        if 'retention_policy' in metadata_dict:
            metadata_dict['retention_policy'] = MemoryRetentionPolicy(metadata_dict['retention_policy'])
        
        # Convert retention period back to timedelta
        if 'retention_period' in metadata_dict and metadata_dict['retention_period']:
            metadata_dict['retention_period'] = timedelta(seconds=float(metadata_dict['retention_period']))
...
@asynccontextmanager
async def memory_operation_span(tracer: Optional[TraceManager], operation: str, memory_id: Optional[str] = None):
