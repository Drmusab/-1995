...
from src.core.events.event_types import (
    ...
    MemoryConsolidationStarted, MemoryConsolidationCompleted, MemoryBackupStarted,
    MemoryBackupCompleted, MemoryRestoreStarted, MemoryRestoreCompleted,
    MemoryCacheHit, MemoryCacheMiss, MemorySearchStarted, MemorySearchCompleted,
    MemoryFullError, MemoryCorruptionDetected, MemoryAccessDenied
)
...
from src.memory.core_memory.base_memory import (
    BaseMemory, BaseMemoryStore, MemoryItem, MemoryMetadata, MemorySearchResult,
    MemoryQuery, SimpleMemoryQuery, VectorMemoryQuery, SemanticMemoryQuery,
    MemoryType, MemoryStorageType, MemoryAccess, MemorySensitivity, 
    MemoryRetentionPolicy, MemoryIndexer, MemoryCache, MemoryAccessController,
    MemoryUtils, AbstractMemoryManager, memory_operation_span, 
    register_memory_metrics, MemoryError, MemoryAccessError, MemoryNotFoundError,
    MemoryCorruptionError, MemoryStorageError, MemoryOperationError
)
...
from src.memory.core_memory.memory_types import (
    ...
    ProceduralMemory, SensoryMemory, ShortTermMemory, LongTermMemory
)
...
class MemoryManagerConfig:
    ...
    def __init__(self, config_loader: ConfigLoader):
        ...
        self.short_term_memory_capacity = config_loader.get("memory.short_term_memory_capacity", 10000)
        self.long_term_memory_capacity = config_loader.get("memory.long_term_memory_capacity", 1000000)
        
        # Memory retention times (in seconds)
        self.working_memory_retention = config_loader.get("memory.working_memory_retention", 3600)  # 1 hour
        self.short_term_retention = config_loader.get("memory.short_term_retention", 86400)  # 1 day
        self.long_term_retention = config_loader.get("memory.long_term_retention", 31536000)  # 1 year
        
        # Consolidation settings
        self.consolidation_interval = config_loader.get("memory.consolidation_interval", 1800)  # 30 minutes
...
class MemoryManager(AbstractMemoryManager):
        ...
        **kwargs
        ...
        try:
            async with memory_operation_span(self.tracer, "store", memory_id):
                ...
                mem_metadata.expiration = MemoryUtils.calculate_expiration(mem_metadata.retention_policy)
    ...
    @handle_exceptions
    async def retrieve_memory(self, memory_id: str, user_id: Optional[str] = None) -> Optional[MemoryItem]:
        """
                ...
                if self.memory_cache:
                    ...
                    if cached_item:
                        ...
                        if not await self.access_controller.check_access(cached_item, user_id):
                            raise MemoryAccessError(f"Access denied to memory {memory_id}", memory_id, owner_id=user_id)
                        
                        # Decrypt if needed
                        if cached_item.metadata.encryption_status and self.encryption_manager:
                            cached_item = await self.access_controller.decrypt_memory_content(cached_item)
    ...
    async def search_memories(
        ...
        **kwargs
        ...
        await self.event_bus.emit(MemorySearchStarted(
        ...
        try:
            async with memory_operation_span(self.tracer, "search"):
                ...
                await self.event_bus.emit(MemorySearchCompleted(
                    query_type=type(query).__name__,
                    memory_type=memory_type.value if memory_type else "all",
    ...
    async def _perform_semantic_search(
        ...
        if query.memory_type and query.memory_type in self.memory_stores:
            ...
            stores = [
                store for store in set(self.memory_stores.values())
            ...
            try:
                if hasattr(store, 'semantic_search'):
                    items, scores = await store.semantic_search(
                        query.query_text,
                        similarity_threshold=query.relevance_threshold
                    )
        ...
        if relevance_scores:
            all_results.sort(
                key=lambda item: relevance_scores.get(item.memory_id, 0.0),
                reverse=True
            )
    ...
    async def _perform_vector_search(
        ...
        if similarity_scores:
            all_results.sort(
                key=lambda item: similarity_scores.get(item.memory_id, 0.0),
                reverse=True
            )
    ...
    async def _perform_structured_search(
        ...
        if hasattr(self.retrieval, 'structured_search'):
            results = await self.retrieval.structured_search(
                query, 
                user_id=user_id
            )
            return results
    ...
    async def consolidate_memories(self) -> None:
        ...
        try:
            ...
            if hasattr(self.consolidation, 'consolidate'):
                ...
                # Emit completion event
                await self.event_bus.emit(MemoryConsolidationCompleted(
    ...
    async def _apply_forgetting(self) -> int:
        ...
        try:
            ...
            for memory in search_result.items:
                ...
                if (memory.metadata.custom_metadata.get('consolidated', False) and
                    memory.metadata.importance < self.config.importance_threshold):
        ...
        for store_type, store in set(self.memory_stores.items()):
            if hasattr(store, 'get_statistics'):
                try:
                    ...
                    self.logger.warning(f"Failed to get stats for store {store_type.value}: {str(e)}")
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                if success and self.backup_manager:
                    await self.backup_manager.register_backup(
                        "memory",
                        str(backup_dir),
                        metadata
                    )
                ...
                await self.event_bus.emit(MemoryBackupCompleted(
                    success=success,
    ...
    async def restore_from_backup(self, backup_path: Union[str, Path]) -> bool:
        ...
        try:
            ...
            await self.event_bus.emit(MemoryRestoreCompleted(
                success=success,
