...
class WorkingMemory(BaseMemory):
    ...
    async def store(self, data: Any, **kwargs) -> str:
        ...
        priority_level = min(self.priority_levels - 1, int(priority * self.priority_levels))
        ...
        if self.metrics:
            ...
            self.metrics.gauge(
                "working_memory_items_per_session", session_items, {"session_id": session_id}
            )
    ...
    async def update(self, session_id: str, data: Dict[str, Any]) -> None:
        ...
        if isinstance(data, dict) and "last_interaction" in data:
            ...
            # Store generic data
            await self.store(data, session_id=session_id, priority=0.5, tags={"update"})
    ...
    async def get_recent_items(self, session_id: str, limit: int = 10) -> List[MemoryItem]:
        ...
        # Sort by recency
        items.sort(key=lambda x: x.metadata.last_accessed or x.metadata.created_at, reverse=True)
    ...
    async def _apply_forgetting(self, session_id: str) -> None:
        ...
        for priority_level in range(self.priority_levels):
            ...
            items_to_forget = self._session_memory[session_id][priority_level][
                : max(1, len(self._session_memory[session_id][priority_level]) // 4)
            ]
            ...
            self.logger.debug(
                f"Applied forgetting to {len(items_to_forget)} items in session {session_id}"
            )
...
class EpisodicMemory(BaseMemory):
    ...
    def __init__(
        ...
        self._time_index: Dict[str, List[Tuple[datetime, str]]] = defaultdict(
            list
        )  # date_key -> [(timestamp, memory_id)]
    ...
    async def store(self, data: Any, **kwargs) -> str:
        ...
        metadata = MemoryMetadata(
            ...
            custom_metadata={"emotion": emotion, "memory_strength": 1.0},  # Initial strength
        )
        ...
        if self.metrics:
            ...
            if user_id:
                ...
                self.metrics.gauge(
                    "episodic_memory_items_per_user", user_items, {"user_id": user_id}
                )
    ...
    async def apply_memory_decay(self, decay_rate: float = 0.05) -> int:
        ...
        for item in items:
            ...
            if abs(new_strength - current_strength) > 0.01:
                ...
                item.metadata.importance = item.metadata.importance * (
        ...
        # Emit event
        await self.event_bus.emit(
            MemoryDecayApplied(memory_type=MemoryType.EPISODIC.value, count=affected_count)
        )
...
class SemanticMemory(BaseMemory):
    ...
    def __init__(
        self, container: Container, vector_store: VectorMemoryStore, model_router: ModelRouter
    ):
        """
        ...
        if isinstance(query, str):
            ...
            try:
                ...
                return await self.vector_store.similarity_search(
                    query_embedding, similarity_threshold=0.7, top_k=10
                )
            except Exception as e:
                self.logger.error(f"Semantic search error: {str(e)}")
            ...
            # Vector query
            return await self.vector_store.similarity_search(
                query, similarity_threshold=0.7, top_k=10
            )
