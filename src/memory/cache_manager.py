...
from src.core.events.event_types import (
    CacheHit, CacheMiss, CacheStored, CacheEvicted, CacheCleared,
    CacheBackupStarted, CacheBackupCompleted, CacheRestoreStarted, 
    CacheRestoreCompleted, ErrorOccurred
)
...
class MemoryCacheManager:
        ...
        try:
            ...
            if key in self._l1_cache:
                ...
                if entry.is_expired():
                    ...
                    # Update metrics
                    if self.metrics:
                        self.metrics.increment("memory_cache_hits_total", 
                                              tags={"level": "l1"})
                        self.metrics.record("memory_cache_get_latency_seconds", 
    ...
    async def set(
        ...
        try:
            ...
            # Emit event
            await self.event_bus.emit(CacheStored(
                key=key,
                level=level.value,
                ttl=ttl,
            ...
            if self.metrics:
                ...
                self.metrics.record("memory_cache_set_latency_seconds", 
                                  time.time() - start_time)
    ...
    async def delete(self, key: str, level: CacheLevel = CacheLevel.L1) -> bool:
        ...
        if success:
            # Emit event
            await self.event_bus.emit(CacheEvicted(
                key=key,
                level=level.value,
    ...
    async def get_memory_item(self, memory_id: str, memory_type: Optional[MemoryType] = None) -> Optional[MemoryItem]:
        """
    ...
    async def invalidate_memory_item(self, memory_id: str, memory_type: Optional[MemoryType] = None) -> bool:
        """
    ...
    async def backup(self, path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            # Emit event
            await self.event_bus.emit(CacheBackupCompleted(
                path=path,
    ...
    async def restore(self, path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            # Emit event
            await self.event_bus.emit(CacheRestoreCompleted(
                path=path,
    ...
    def _set_in_l1(
        ...
        while (len(self._l1_cache) >= self.config.max_size_l1 or 
               self._l1_size_bytes + entry_size > 1024 * 1024 * 100):  # 100MB max
    ...
    async def _health_check(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        if total_requests > 0:
            details["hit_rate"] = round(
                (self._stats["hits_l1"] + self._stats["hits_l2"]) / total_requests, 
                2
            )
