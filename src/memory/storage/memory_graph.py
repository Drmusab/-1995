...
from src.core.events.event_types import (
    ...
    GraphNodeDeleted,
    GraphNodeUpdated,
    GraphStoreBackupCompleted,
    GraphStoreBackupStarted,
    GraphStoreRestoreCompleted,
    GraphStoreRestoreStarted,
...
class GraphPathNode:
    ...
    def __init__(
        self, node: GraphNode, incoming_edge: Optional[GraphEdge] = None, distance: int = 0
    ):
        """
...
class GraphPath:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {"nodes": [node.to_dict() for node in self.nodes], "length": self.length}
...
class MemoryGraphStore(BaseMemoryStore):
    ...
    def __init__(self, container: Container):
        ...
        self._outgoing_edges: Dict[str, List[str]] = defaultdict(
            list
        )  # node_id -> list of outgoing edge_ids
        self._incoming_edges: Dict[str, List[str]] = defaultdict(
            list
        )  # node_id -> list of incoming edge_ids
        self._node_type_index: Dict[GraphNodeType, Set[str]] = defaultdict(
            set
        )  # node_type -> set of node_ids
        self._node_label_index: Dict[str, Set[str]] = defaultdict(set)  # label -> set of node_ids
        self._relationship_index: Dict[RelationshipType, Set[str]] = defaultdict(
            set
        )  # relationship -> set of edge_ids
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"MemoryGraphStore initialized with {len(self._nodes)} nodes and {len(self._edges)} edges"
            )
    ...
    async def store_item(self, item: MemoryItem) -> None:
        ...
        try:
            ...
            if self.metrics:
                ...
                self.metrics.record(
                    "memory_graph_operation_duration_seconds", time.time() - start_time
                )
    ...
    async def add_node(
        ...
        try:
            ...
            await self.event_bus.emit(
                GraphNodeAdded(
                    node_id=node_id, node_type=node_type.value, associated_memory=memory_id
                )
            )
    ...
    async def update_node(
        self, node_id: str, properties: Dict[str, Any], labels: Optional[Set[str]] = None
    ) -> bool:
        """
    ...
    async def update_edge(
        self, edge_id: str, properties: Dict[str, Any], weight: Optional[float] = None
    ) -> bool:
        """
    ...
    async def find_path(
        ...
        try:
            ...
            await self.event_bus.emit(
                GraphTraversalStarted(
                    start_node_id=start_node_id, max_depth=max_depth, traversal_type="path"
                )
            )
            ...
            if NETWORKX_AVAILABLE and self._nx_graph:
                try:
                    ...
                    for u, v, data in self._nx_graph.edges(data=True):
                        ...
                        if (
                            relationships
                            and RelationshipType(data.get("relationship", "related_to"))
                    ...
                    for i, node_id in enumerate(path):
                        ...
                        path_nodes.append(
                            GraphPathNode(node=node, incoming_edge=incoming_edge, distance=i)
                        )
                    ...
                    if self.metrics:
                        ...
                        self.metrics.record(
                            "memory_graph_traversal_time_seconds", time.time() - start_time
                        )
            ...
            while queue:
                ...
                if current_id == end_node_id:
                    ...
                    for i, (node_id, edge) in enumerate([(start_node_id, None)] + path):
                        node = self._nodes[node_id]
                        path_nodes.append(GraphPathNode(node=node, incoming_edge=edge, distance=i))
    ...
    async def find_subgraph(
        self, node_ids: List[str], include_edges: bool = True, max_distance: int = 1
