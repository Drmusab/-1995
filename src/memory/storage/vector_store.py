...
from src.core.events.event_types import (
    ...
    MemoryItemStored,
    MemoryItemUpdated,
    VectorIndexRebuildCompleted,
    VectorIndexRebuildStarted,
    VectorIndexUpdated,
    VectorSearchCompleted,
    VectorSearchStarted,
    VectorStoreBackupCompleted,
    VectorStoreBackupStarted,
    VectorStoreCacheHit,
    VectorStoreCacheMiss,
    VectorStoreRestoreCompleted,
    VectorStoreRestoreStarted,
)
...
class VectorStoreConfig:
    ...
    def __init__(self, config_loader: ConfigLoader):
        ...
        self.rebuild_threshold = vector_config.get(
            "rebuild_threshold", 0.2
        )  # Rebuild when 20% of items changed
    ...
    async def search(self, query_vector: List[float], k: int) -> List[Tuple[str, float]]:
        ...
        for id, vector in self.vectors.items():
            if self.metric == VectorDistanceMetric.COSINE:
                ...
                # Angular similarity
                similarity = 1.0 - (
                    np.arccos(min(1.0, max(-1.0, np.dot(query_vector, vector)))) / np.pi
                )
...
class FAISSVectorIndex(VectorIndex):
        ...
        if FAISS_AVAILABLE:
            ...
            if self.config.default_index_type == VectorIndexType.FLAT:
                ...
                self.index.nprobe = min(
                    10, self.config.ivf_nlist
                )  # Number of clusters to visit during search
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        if isinstance(self.index, faiss.IndexIVFFlat):
            stats.update({"nlist": self.config.ivf_nlist, "nprobe": self.index.nprobe})
...
class VectorMemoryStore(BaseMemoryStore):
        ...
        try:
            ...
            if FAISS_AVAILABLE and self.config.default_index_type in [
                ...
                self._vector_index = FAISSVectorIndex(
                    self.config.dimension, self.config.distance_metric, self.config
                )
                ...
                self._vector_index = FlatVectorIndex(
                    self.config.dimension, self.config.distance_metric
                )
    ...
    async def store_item(self, item: MemoryItem) -> None:
        ...
        try:
            ...
            if self._update_count_since_rebuild >= max(
                100, len(self._memory_items) * self.config.rebuild_threshold
            ):
            ...
            if self.metrics:
                ...
                self.metrics.record(
                    "vector_store_operation_duration_seconds", time.time() - start_time
                )
    ...
    async def update_item(self, memory_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            await self.event_bus.emit(
                MemoryItemUpdated(
                    memory_id=memory_id, memory_type=item.memory_type.value, owner_id=item.owner_id
                )
            )
    ...
    async def query(self, query: SimpleMemoryQuery) -> List[MemoryItem]:
        ...
        try:
            ...
            filtered_items.sort(
                key=lambda x: x.metadata.last_accessed or x.metadata.created_at, reverse=True
            )
            ...
            if self.metrics:
                ...
                self.metrics.record(
                    "vector_store_search_duration_seconds", time.time() - start_time
                )
    ...
    async def similarity_search(
        ...
        try:
            ...
            await self.event_bus.emit(
                VectorSearchStarted(
                    memory_type=memory_type.value if memory_type else "all", top_k=top_k
                )
            )
    ...
    async def backup(self, backup_path: Path) -> bool:
        ...
        try:
            ...
            await self.event_bus.emit(VectorStoreBackupStarted(backup_path=str(backup_path)))
