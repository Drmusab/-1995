...
class ConfigError(Exception):
    ...
    def __init__(
        self, message: str, source_id: Optional[str] = None, error_code: Optional[str] = None
    ):
...
class ConfigurationProvider(ABC):
    ...
    async def watch_for_changes(
        self, source_info: ConfigSourceInfo, callback: Callable[[Dict[str, Any]], None]
    ) -> None:
...
class FileConfigProvider(ConfigurationProvider):
    ...
    async def load_config(self, source_info: ConfigSourceInfo) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ConfigError(
                f"Failed to load config from {file_path}: {str(e)}", source_info.source_id
            )
...
class RemoteConfigProvider(ConfigurationProvider):
    ...
    async def load_config(self, source_info: ConfigSourceInfo) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ConfigError(
                f"Failed to load remote config from {source_info.location}: {str(e)}",
...
class ConfigLoader:
            ...
            try:
                ...
                try:
                    ...
                    self.health_check.register_component(
                        "config_loader", self._health_check_callback
                    )
                except Exception:
                    pass
    ...
    async def add_source(
        ...
        **kwargs,
        ...
        if environment_filter and environment_filter != self.environment:
            self.logger.debug(
                f"Skipping source {source_id} (environment filter: {environment_filter})"
            )
            return
        ...
        if reload_strategy in [ReloadStrategy.FILE_WATCH, ReloadStrategy.POLLING]:
            await provider.watch_for_changes(
                source_info,
    ...
    async def _load_source(self, source_id: str) -> None:
        ...
        try:
            ...
            # Apply interpolation
            config = self._interpolator.interpolate(
                config, {"environment": self.environment, "source_id": source_id}
            )
    ...
    async def _merge_configurations(self) -> None:
        ...
        sorted_sources = sorted(self._sources.items(), key=lambda x: x[1].priority.value)
    ...
    async def remove_source(self, source_id: str) -> None:
        ...
        if self.event_bus:
            await self.event_bus.emit(
                ConfigurationSourceRemoved(source_id=source_id, environment=self.environment)
            )
