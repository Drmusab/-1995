...
class DependencyValidator:
    ...
    def validate_registration(self, metadata: DependencyMetadata) -> List[str]:
        ...
        if (
            metadata.scope == LifecycleScope.SCOPED
            and metadata.strategy == RegistrationStrategy.FACTORY_BASED
        ):
        ...
        if metadata.implementation_type:
            ...
            if hasattr(metadata.registered_type, "__origin__") or not issubclass(
                metadata.implementation_type, metadata.registered_type
            ):
...
class Container:
    ...
    def register_singleton(
        self, registered_type: Type[T], implementation: Optional[Union[Type[T], T]] = None, **kwargs
    ) -> "Container":
    ...
    def register_factory(
        self, registered_type: Type[T], factory: Factory[T], scope: LifecycleScope = None, **kwargs
    ) -> "Container":
        """Register a factory function."""
        return self.register(registered_type, factory, scope or LifecycleScope.TRANSIENT, **kwargs)

    def register_instance(self, registered_type: Type[T], instance: T, **kwargs) -> "Container":
    ...
    def get_all(
        self, interface_type: Type[T], context: Optional[ResolutionContext] = None
    ) -> List[T]:
        """
    ...
    def get_lazy(
        self, dependency_type: Type[T], context: Optional[ResolutionContext] = None
    ) -> LazyProxy[T]:
        """
    ...
    def _resolve_dependency(self, dependency_type: Type[T], context: ResolutionContext) -> T:
        ...
        try:
            ...
            if metadata.condition and not metadata.condition():
                raise MissingDependencyError(
                    f"Conditional dependency {dependency_type} condition not met",
                    dependency_type,
                    context,
                )
    ...
    def _find_registration(
        self, dependency_type: Type, context: ResolutionContext
    ) -> Optional[str]:
        ...
        if candidates:
            ...
            if candidates:
                best_candidate = max(candidates, key=lambda cid: self._registrations[cid].priority)
    ...
    def _get_singleton_instance(
        self, metadata: DependencyMetadata, context: ResolutionContext
    ) -> Any:
    ...
    def _get_lazy_singleton_instance(
        self, metadata: DependencyMetadata, context: ResolutionContext
    ) -> Any:
    ...
    def _get_thread_local_instance(
        self, metadata: DependencyMetadata, context: ResolutionContext
    ) -> Any:
    ...
    def _auto_wire_dependency(self, dependency_type: Type, context: ResolutionContext) -> Any:
        ...
        if not inspect.isclass(dependency_type):
            raise MissingDependencyError(f"Cannot auto-wire non-class type {dependency_type}")
            ...
            for base in inspect.getmro(metadata.implementation_type)[1:]:
                if base in self._interface_mappings:
                    self._interface_mappings[base] = [
                        rid for rid in self._interface_mappings[base] if rid != registration_id
                    ]
    ...
    def get_registration_info(
        self, dependency_type: Type, name: Optional[str] = None
    ...
    def _load_registrations_from_config(self, registrations_config: List[Dict[str, Any]]) -> None:
        ...
        for reg_config in registrations_config:
            try:
                ...
                implementation_type = (
                    self._import_type(implementation_name)
                    if implementation_name != type_name
                    else None
                )
                ...
                self.logger.error(
                    f"Failed to load registration from config: {reg_config}: {str(e)}"
                )
    ...
    def register_from_plugin(
        self, plugin_id: str, registration_func: Callable[["Container"], None]
    ) -> None:
        """
        ...
        try:
            ...
            if new_registrations > 0:
                ...
                self.logger.info(
                    f"Plugin '{plugin_id}' registered {new_registrations} dependencies"
                )

        except Exception as e:
            self.logger.error(
                f"Failed to register dependencies from plugin '{plugin_id}': {str(e)}"
            )
    ...
    def unregister_plugin(self, plugin_id: str) -> int:
        ...
        self.logger.info(
            f"Unregistered {unregistered_count} dependencies from plugin '{plugin_id}'"
        )
    ...
    def hot_swap(
        self, dependency_type: Type[T], new_implementation: Union[Type[T], T, Factory[T]]
    ) -> bool:
        """
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"Container(registrations={len(self._registrations)}, "
...
def create_container(
    config_path: Optional[Union[str, Path]] = None, **config_overrides
) -> Container:
    """
