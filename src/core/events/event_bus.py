...
# Event types
from src.core.events.event_types import BaseEvent, EventCategory, EventMetadata, EventPriority
...
class EventQueue:
    ...
    def __init__(self, max_size: int = 10000):
        ...
        self._queues: Dict[EventPriority, asyncio.Queue] = {
            priority: asyncio.Queue(maxsize=max_size // 5) for priority in EventPriority
        }
        self._total_size = 0
...
class EventBusError(Exception):
    ...
    def __init__(
        self, message: str, event_id: Optional[str] = None, subscription_id: Optional[str] = None
    ):
...
class EnhancedEventBus:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.event_handlers: Dict[str, Set[str]] = defaultdict(
            set
        )  # event_type -> subscription_ids
        self.wildcard_handlers: Dict[str, Set[str]] = defaultdict(
            set
        )  # pattern -> subscription_ids
        ...
        self.event_store = (
            EventStore(Path(self.storage_path), self.max_stored_events)
        ...
        self.thread_pool = ThreadPoolExecutor(
            max_workers=self.max_sync_workers, thread_name_prefix="event_bus"
        )
    ...
    async def emit(
        self, event: BaseEvent, wait_for_delivery: bool = False, timeout: Optional[float] = None
    ) -> Optional[str]:
        """
        ...
        for subscription_id in matching_subscriptions:
            ...
            if subscription.delivery_mode == EventDeliveryMode.BATCH:
                envelope.scheduled_at = datetime.now(timezone.utc) + timedelta(
                    seconds=subscription.batch_timeout
                )
    ...
    def _get_matching_subscriptions(self, event: BaseEvent) -> Set[str]:
        ...
        for subscription_id in matching:
            ...
            # Check tags filter
            if subscription.tags_filter and not subscription.tags_filter.intersection(
                event.metadata.tags
            ):
                continue
    ...
    async def _wait_for_delivery(
        self, envelopes: List[EventEnvelope], timeout: Optional[float]
    ) -> str:
        ...
        **kwargs,
        ...
        if self.enable_circuit_breaker:
            self.circuit_breakers[subscription_id] = CircuitBreaker(
                handler_id=subscription_id, failure_threshold=self.circuit_breaker_threshold
            )
    ...
    async def _process_event_envelope(self, envelope: EventEnvelope) -> None:
        ...
        if not subscription:
            self.logger.warning(
                f"Subscription {subscription_id} not found for envelope {envelope.event_id}"
            )
            return
        ...
        try:
            ...
            if self.metrics:
                ...
                self.metrics.record(
                    "event_bus_processing_duration_seconds", envelope.processing_time
                )
    ...
    async def _execute_handler(
        self, subscription: EventSubscription, envelope: EventEnvelope
    ) -> Any:
        ...
        if subscription.delivery_mode == EventDeliveryMode.SYNC:
            ...
            if asyncio.iscoroutinefunction(handler):
                result = await asyncio.wait_for(
                    handler(event), timeout=subscription.timeout_seconds
                )
            else:
                loop = asyncio.get_event_loop()
            ...
            try:
                ...
                for circuit_breaker in self.circuit_breakers.values():
                    if (
                        ...
                        and circuit_breaker.last_failure_time
                        and (current_time - circuit_breaker.last_failure_time).total_seconds()
                        > circuit_breaker.timeout_duration
                    ):
    ...
    def get_subscription_info(self, subscription_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "last_event_at": (
                subscription.last_event_at.isoformat() if subscription.last_event_at else None
            ),
    ...
    def list_subscriptions(self) -> List[Dict[str, Any]]:
        ...
        return [self.get_subscription_info(sub_id) for sub_id in self.subscriptions.keys()]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        circuit_breaker_stats = {
            ...
            "closed": sum(
                1 for cb in self.circuit_breakers.values() if cb.state == CircuitBreakerState.CLOSED
            ),
            "open": sum(
        ...
        return {
            "subscriptions": {
                ...
                "by_type": {
                    sub_type.value: sum(
                        1 for s in self.subscriptions.values() if s.subscription_type == sub_type
                    )
            ...
            "queue": {
                ...
                "dead_letter_size": (
                    asyncio.create_task(self.dead_letter_queue.size()) if self.is_running else 0
                ),
            },
            "circuit_breakers": circuit_breaker_stats,
            "rate_limiters": len(self.rate_limiters),
            "workers": {"active": len(self.worker_tasks), "running": self.is_running},
            "persistence": {
        ...
        try:
            ...
            health_factors = {
                ...
                "low_error_rate": max(
                    ...
                    1.0
                    - (
                        self.event_stats["total_failed"] / max(1, self.event_stats["total_emitted"])
                    ),
                ),
                "circuit_breakers_healthy": 1.0
                - (
                    sum(
                        1
                        for cb in self.circuit_breakers.values()
                    ...
                    / max(1, len(self.circuit_breakers))
                ),
            }
