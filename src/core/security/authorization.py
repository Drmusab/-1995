...
class PolicyBasedEvaluator(PermissionEvaluator):
    ...
    async def _evaluate_policy(self, policy: Policy, context: AuthorizationContext) -> bool:
        ...
        try:
            ...
            if policy.applies_to_roles and not context.user_roles.intersection(
            ...
            if (
                policy.applies_to_resources
                and context.resource_id not in policy.applies_to_resources
            ):
                return False
    ...
    async def _evaluate_condition(
        self, condition_key: str, condition_value: Any, context: AuthorizationContext
    ) -> bool:
        ...
        try:
            if condition_key == "time_of_day":
                ...
                attr_name, expected_value = condition_value.get("name"), condition_value.get(
                    "value"
                )
...
class AuthorizationManager:
    ...
    def _setup_default_permissions(self) -> None:
        ...
        try:
            ...
            system_permissions = [
                ...
                Permission(
                    "component.read", "Read Components", PermissionType.READ, ResourceType.COMPONENT
                ),
                Permission(
                    "component.manage",
                    "Manage Components",
                ...
                # Session permissions
                Permission(
                    "session.create", "Create Sessions", PermissionType.CREATE, ResourceType.SESSION
                ),
                Permission(
                    "session.manage", "Manage Sessions", PermissionType.MANAGE, ResourceType.SESSION
                ),
                # Plugin permissions
                Permission(
                    "plugin.install", "Install Plugins", PermissionType.CREATE, ResourceType.PLUGIN
                ),
                Permission(
                    "plugin.manage", "Manage Plugins", PermissionType.MANAGE, ResourceType.PLUGIN
                ),
                # API permissions
                Permission("api.read", "Read API", PermissionType.READ, ResourceType.API_ENDPOINT),
                Permission(
                    "api.write", "Write API", PermissionType.WRITE, ResourceType.API_ENDPOINT
                ),
            ]
            ...
            user_role = Role(
                ...
                permissions={"session.create", "workflow.execute", "api.read", "component.read"},
                is_system_role=True,
            )
            ...
            self.logger.info(
                f"Setup {len(system_permissions)} default permissions and {len([admin_role, user_role])} default roles"
            )
                ...
                span.set_attributes(
                    {
                        ...
                        "resource_type": (
                            auth_context.resource_type.value
                            if auth_context.resource_type
                            else "unknown"
                        ),
                    }
                )
                ...
                for evaluator in self.evaluators:
                    if evaluator.can_evaluate(auth_context):
                        try:
                            ...
                            self.logger.error(
                                f"Evaluator {type(evaluator).__name__} failed: {str(e)}"
                            )
    ...
    async def _combine_evaluation_results(
        self, results: List[AuthorizationResult], context: AuthorizationContext
    ) -> AuthorizationResult:
        ...
        try:
            if self.cache:
                await self.cache.set(
                    f"auth:{cache_key}", json.dumps(asdict(result), default=str), ttl=self.cache_ttl
                )
            else:
    ...
    async def revoke_permission(
        self, user_id: str, permission_id: str, revoked_by: Optional[str] = None
    ) -> None:
        """
        ...
        async with self.lock:
            ...
            await self.event_bus.emit(
                PermissionRevoked(
                    user_id=user_id, permission_id=permission_id, revoked_by=revoked_by
                )
            )
    ...
    async def assign_role(
        ...
        async with self.lock:
            ...
            # Emit event
            await self.event_bus.emit(
                RoleAssigned(user_id=user_id, role_id=role_id, assigned_by=assigned_by)
            )
    ...
    async def revoke_role(
        self, user_id: str, role_id: str, revoked_by: Optional[str] = None
    ) -> None:
        """
        ...
        async with self.lock:
            ...
            # Emit event
            await self.event_bus.emit(
                RoleRevoked(user_id=user_id, role_id=role_id, revoked_by=revoked_by)
            )
    ...
    async def get_applicable_policies(self, context: AuthorizationContext) -> List[Policy]:
        ...
        for policy in self.policies.values():
            ...
            if (
                not policy.applies_to_users
                and not policy.applies_to_roles
                and not policy.applies_to_resources
            ):
                applies = True  # Global policy
            else:
                if policy.applies_to_users and context.user_id in policy.applies_to_users:
                    applies = True
                elif policy.applies_to_roles and context.user_roles.intersection(
                    policy.applies_to_roles
                ):
                    applies = True
                elif (
                    policy.applies_to_resources
                    and context.resource_id in policy.applies_to_resources
                ):
                    applies = True
    ...
    async def _clear_user_cache(self, user_id: str) -> None:
        ...
        if self.cache:
            ...
            keys_to_remove = [key for key in self.evaluation_cache.keys() if user_id in key]
            for key in keys_to_remove:
                del self.evaluation_cache[key]
    ...
    async def _store_user_permission(
        self,
        user_id: str,
        permission_id: str,
    ...
    async def _store_user_role(
        self, user_id: str, role_id: str, assigned_by: Optional[str]
    ) -> None:
