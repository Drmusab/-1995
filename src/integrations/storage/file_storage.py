...
from typing import Optional, Dict, Any, List, Set, Callable, Type, Union, AsyncGenerator, BinaryIO, Protocol
import asyncio
import threading
import time
import hashlib
...
from src.core.events.event_types import (
    FileUploaded, FileDownloaded, FileDeleted, FileMoved, FileCopied,
    FileAccessGranted, FileAccessDenied, FileCorrupted, FileExpired,
    StorageQuotaExceeded, StorageHealthChanged, BackupStarted, BackupCompleted,
    CacheHit, CacheMiss, ErrorOccurred, ComponentHealthChanged
)
...
class StorageError(Exception):
    ...
    def __init__(self, message: str, file_id: Optional[str] = None, 
                 operation: Optional[str] = None, backend: Optional[str] = None):
...
class MemoryStorageBackend:
    ...
    async def list_files(self, prefix: str = "") -> List[str]:
        ...
        with self.lock:
            files = [
                path for path in self.storage.keys()
...
class FileStorageManager:
    ...
    def __init__(self, container: Container):
        ...
        self.thread_pool = ThreadPoolExecutor(
            max_workers=self.config_data.concurrent_operations,
            thread_name_prefix="file_storage"
        )
    ...
    def _setup_backends(self) -> None:
        ...
        try:
            ...
            self.backends[StorageBackend.LOCAL] = LocalStorageBackend(
                self.config_data.local_base_path,
                self.logger
            )
            ...
            if self.config_data.aws_bucket and AWS_AVAILABLE:
                try:
                    self.backends[StorageBackend.AWS_S3] = S3StorageBackend(
                        self.config_data.aws_bucket,
                        self.config_data.aws_region,
                        self.logger
                    )
                except Exception as e:
            ...
            if self.config_data.primary_backend in self.backends:
                ...
                self.logger.warning(f"Primary backend {self.config_data.primary_backend} not available, using local")
            ...
            try:
                with self.tracer.trace("file_store") as span:
                    ...
                    if file_size > max_size_bytes:
                        raise StorageError(f"File size {file_size} exceeds maximum {max_size_bytes}")
                    ...
                    file_metadata = FileMetadata(
                        ...
                        encryption_enabled=encryption_enabled if encryption_enabled is not None else self.config_data.encryption_enabled,
                    ...
                    if file_metadata.encryption_enabled and self.encryption_manager:
                        encrypted_content, key_id = await self.encryption_manager.encrypt_data(file_content)
                        ...
                        for fallback_backend_type in self.config_data.fallback_backends:
                            if fallback_backend_type in self.backends:
                                try:
                                    ...
                                    self.logger.warning(f"Fallback backend {fallback_backend_type} failed: {str(fallback_error)}")
                    ...
                    # Emit event
                    await self.event_bus.emit(FileUploaded(
                        file_id=file_id,
                        filename=filename,
                        size_bytes=file_size,
                        session_id=session_id,
        ...
        async with self.semaphore:
            ...
            try:
                with self.tracer.trace("file_retrieve") as span:
                    ...
                    # Check permissions
                    if user_id and not await self._check_file_access(file_metadata, user_id, FileOperation.READ):
                        await self.event_bus.emit(FileAccessDenied(
                            file_id=file_id,
                            user_id=user_id,
                            operation="read"
                        ))
                    ...
                    # Update metrics
                    processing_time = time.time() - start_time
                    transfer_speed = (file_metadata.size_bytes / 1024 / 1024) / max(processing_time, 0.001)
                    ...
                    # Emit event
                    await self.event_bus.emit(FileDownloaded(
                        file_id=file_id,
    ...
    async def delete_file(
        ...
        try:
            with self.tracer.trace("file_delete") as span:
                ...
                # Check permissions
                if user_id and not await self._check_file_access(file_metadata, user_id, FileOperation.DELETE):
                    await self.event_bus.emit(FileAccessDenied(
                        file_id=file_id,
                        user_id=user_id,
                        operation="delete"
                ...
                # Emit event
                await self.event_bus.emit(FileDeleted(
                    file_id=file_id,
                    filename=file_metadata.filename,
                    user_id=user_id,
                    session_id=session_id,
        ...
        if self.cache and self.config_data.cache_enabled:
            ...
            if cached_data:
                try:
                    ...
                    self.logger.warning(f"Failed to deserialize cached metadata for {file_id}: {str(e)}")
        
        # Try database
        if self.database:
            try:
                row = await self.database.fetch_one(
                    "SELECT metadata FROM file_storage WHERE file_id = ?",
                    (file_id,)
                )
                if row:
                ...
                self.logger.warning(f"Failed to load metadata from database for {file_id}: {str(e)}")
    ...
    async def get_storage_stats(self) -> Dict[str, Any]:
        ...
        for metadata in self.file_metadata.values():
            ...
            # By storage class
            storage_class = metadata.storage_class.value
            stats['files_by_class'][storage_class] = stats['files_by_class'].get(storage_class, 0) + 1
            
            # Top owners
            if metadata.owner_id:
                stats['top_owners'][metadata.owner_id] = stats['top_owners'].get(metadata.owner_id, 0) + 1
        
        # Recent activity (last 24 hours)
        recent_cutoff = datetime.now(timezone.utc) - timedelta(hours=24)
        stats['recent_activity'] = {
            'uploads': len([
                m for m in self.file_metadata.values()
                if m.created_at > recent_cutoff
            ]),
            'downloads': len([
                m for m in self.file_metadata.values()
                if m.accessed_at > recent_cutoff
            ])
        }
