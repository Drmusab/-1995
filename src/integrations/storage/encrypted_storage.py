...
from src.core.security.encryption import (
    EncryptionManager, EncryptionAlgorithm, SecurityLevel,
    EncryptionContext, EncryptionResult, KeyType
)
...
from src.core.events.event_types import DataEncrypted, DataDecrypted
...
class EncryptedStorageManager:
    ...
    async def store_user_data(
        ...
        try:
            ...
            await self.event_bus.emit(DataEncrypted(
                user_id=user_id,
                data_type=data_type,
                record_id=record_id,
    ...
    async def retrieve_user_data(
        ...
        try:
            ...
            await self.event_bus.emit(DataDecrypted(
                user_id=user_id,
                data_type=record.data_type,
                record_id=record_id,
                timestamp=datetime.now(timezone.utc)
        ...
        try:
            ...
            if record_id:
                ...
                success = await self._delete_encrypted_record(
                    record_id, user_id, secure_delete
                )
                if success:
                    deleted_count = 1
                ...
                keys_to_delete = [
                    key for key in self._decryption_cache.keys()
        ...
        try:
            ...
            for record in records:
                try:
                    ...
                    self.logger.error(f"Error rotating key for record {record['record_id']}: {str(e)}")
                    stats["errors"] += 1
    ...
    def _update_cache(self, key: str, data: Any) -> None:
        ...
        if len(self._decryption_cache) >= self._cache_max_size:
            oldest_key = min(
                self._decryption_cache.keys(),
                key=lambda k: self._decryption_cache[k][1]
            )
    ...
    async def _get_user_records_by_type(
        self,
        user_id: str,
        data_type: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _get_records_for_rotation(self, force: bool) -> List[Dict[str, Any]]:
        ...
        return await self.database.fetch_all(
            query,
            {"days": self.key_rotation_days} if not force else {}
        )
    
    async def _mark_record_superseded(
        self,
        old_record_id: str,
        new_record_id: str
    ) -> None:
