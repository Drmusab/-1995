...
from src.core.events.event_types import (
    ModelRouteSelected, ModelRequestStarted, ModelRequestCompleted,
    ModelRequestFailed, InferenceStarted, InferenceCompleted, InferenceFailed,
...
class ModelInferenceCoordinator:
    ...
    def __init__(
        ...
        # Configuration
        self.max_profile_age = config.get("model_inference_coordinator.max_profile_age", 3600)  # 1 hour
        self.min_samples_for_profile = config.get("model_inference_coordinator.min_samples", 10)
        self.performance_threshold = config.get("model_inference_coordinator.performance_threshold", 0.8)
    ...
    async def coordinate_inference(self, request: InferenceRequest) -> CoordinatedResponse:
        ...
        try:
            ...
            await self.event_bus.emit(InferenceCompleted(
    ...
    async def _select_optimal_model(
        ...
        self.logger.info(f"Selected model {best_model} for request {request.request_id} "
    ...
    async def _execute_monitored_inference(
        ...
        try:
            ...
            await self.event_bus.emit(InferenceStarted(
                inference_id=request.request_id,
                inference_type=request.inference_type,
                model_id=model_id
            ))
            ...
            result = await self._call_inference_engine(
                request.input_data, 
                inference_context
            )
            ...
            await self.event_bus.emit(InferenceFailed(
                inference_id=request.request_id,
                inference_type=request.inference_type,
                model_id=model_id,
                error_message=str(e)
    ...
    async def _update_model_profile(self, model_id: str, metrics: InferenceMetrics) -> None:
        ...
        with self._lock:
            ...
            if recent_metrics:
                ...
                complexity_metrics = [m for m in recent_metrics if m.complexity == metrics.complexity]
                if complexity_metrics:
                    complexity_successes = [m for m in complexity_metrics if m.success]
                    profile.complexity_scores[metrics.complexity] = (
                        len(complexity_successes) / len(complexity_metrics)
                    )
    ...
    async def _get_available_models(self, inference_type: str) -> List[Dict]:
        ...
        if hasattr(self._model_router, 'get_available_models'):
            return await self._model_router.get_available_models(
    ...
    async def _execute_fallback(self, request: InferenceRequest, error_msg: str) -> CoordinatedResponse:
    ...
    async def get_coordination_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "active_profiles": len([
                    p for p in self.model_profiles.values() 
                    if self._is_profile_valid(p)
                ])
            }
