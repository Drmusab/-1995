...
class WeatherProviderBase(ABC):
    ...
    async def get_current_weather(
        self, location: WeatherLocation, units: WeatherUnits = WeatherUnits.METRIC
    ) -> WeatherConditions:
...
class OpenWeatherMapProvider(WeatherProviderBase):
    ...
    async def get_current_weather(
        ...
        try:
            ...
            raise WeatherError(
                f"OpenWeatherMap API error: {str(e)}", WeatherProvider.OPENWEATHERMAP
            ) from e
    ...
    async def get_forecast(
        ...
        **kwargs,
        ...
        try:
            ...
            self.record_request(False, response_time)
            raise WeatherError(
                f"OpenWeatherMap forecast error: {str(e)}", WeatherProvider.OPENWEATHERMAP
            ) from e
    ...
    async def get_alerts(self, location: WeatherLocation) -> List[WeatherAlert]:
        ...
        try:
            ...
            self.record_request(False, response_time)
            raise WeatherError(
                f"OpenWeatherMap alerts error: {str(e)}", WeatherProvider.OPENWEATHERMAP
            ) from e
    ...
    def _parse_current_weather(
        self, data: Dict[str, Any], units: WeatherUnits
    ) -> WeatherConditions:
    ...
    def _parse_forecast(
        self, data: Dict[str, Any], data_type: WeatherDataType, units: WeatherUnits
    ) -> List[WeatherForecastPeriod]:
        ...
        for alert_data in alerts_data:
            ...
            alert = WeatherAlert(
                ...
                severity=severity_map.get(
                    alert_data.get("severity", "unknown"), AlertSeverity.UNKNOWN
                ),
                start_time=datetime.fromtimestamp(alert_data.get("start", 0), timezone.utc),
                end_time=(
                    datetime.fromtimestamp(alert_data.get("end", 0), timezone.utc)
                    if alert_data.get("end")
                    else None
                ),
...
class WeatherCacheManager:
    ...
    def _deserialize_weather_data(self, cached_data: str) -> WeatherData:
        ...
        if data_dict.get("current_conditions"):
            ...
            if obs_time:
                data_dict["current_conditions"]["observation_time"] = datetime.fromisoformat(
                    obs_time
                )
...
class EnhancedWeatherAPI:
    ...
    async def _register_as_component(self) -> None:
        ...
        try:
            ...
            self.component_manager.register_component(
                "weather_api", type(self), ComponentPriority.NORMAL, []
            )
    ...
    def _is_cache_suitable(self, cached_data: WeatherData, request: WeatherRequest) -> bool:
        ...
        distance = self._calculate_distance(
            ...
            cached_data.location.latitude,
            cached_data.location.longitude,
        )
    ...
    async def _fetch_from_providers(self, request: WeatherRequest) -> WeatherData:
        ...
        providers_to_try = list(request.preferred_providers) + [
            p for p in self.provider_priorities if p not in request.preferred_providers
        ]
        ...
        for provider_type in providers_to_try:
            ...
            try:
                weather_data = await self._fetch_from_single_provider(
                    provider, provider_type, request
                )
                if weather_data:
                    self.logger.info(
                        f"Successfully fetched weather data from {provider_type.value}"
                    )
                    return weather_data
    ...
    async def _fetch_from_single_provider(
        self, provider: WeatherProviderBase, provider_type: WeatherProvider, request: WeatherRequest
    ) -> WeatherData:
                ...
                try:
                    forecast_periods = await provider.get_forecast(
                        ...
                        cnt=(
                            ...
                            if forecast_type == WeatherDataType.FORECAST_HOURLY
    ...
    def _calculate_data_quality(self, weather_data: WeatherData) -> float:
        ...
        if weather_data.current_conditions:
            ...
            present_fields = sum(
                1 for field in required_fields if getattr(conditions, field, None) is not None
            )
    ...
    async def _store_weather_memory(
        self, request: WeatherRequest, weather_data: WeatherData
    ) -> None:
        ...
        try:
            memory_data = {
                ...
                "current_conditions": (
                    asdict(weather_data.current_conditions)
                    if weather_data.current_conditions
                    else None
                ),
            ...
            await self.memory_manager.store_episodic_memory(
                event_type="weather_query", data=memory_data, session_id=request.session_id
            )
    ...
    def _create_forecast_summary(
        self, forecast_periods: List[WeatherForecastPeriod]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "dominant_condition": (
                max(set(conditions), key=conditions.count) if conditions else None
            ),
            "precipitation_expected": any(
                p.conditions.precipitation_probability > 50 for p in forecast_periods
            ),
        }

    @handle_exceptions
    async def get_weather_by_location_name(
        self, location_name: str, session_id: Optional[str] = None, **kwargs
    ) -> WeatherData:
        """
        ...
        request = WeatherRequest(location=location, session_id=session_id, **kwargs)
            ...
            try:
                for provider_type, provider in self.providers.items():
                    try:
                        ...
                        self.logger.warning(
                            f"Provider health check failed for {provider_type.value}: {str(e)}"
                        )
        ...
        while True:
            try:
                ...
                for cache_key, weather_data in self.cache_manager.memory_cache.items():
                    if weather_data.expires_at and current_time > weather_data.expires_at:
