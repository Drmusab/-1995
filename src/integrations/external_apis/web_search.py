...
from src.core.events.event_types import (
    ...
    ComponentRegistered, ComponentInitialized, ComponentHealthChanged,
...
from src.assistant.component_manager import (
    EnhancedComponentManager, ComponentMetadata, ComponentPriority, ComponentInterface
)
from src.assistant.workflow_orchestrator import WorkflowOrchestrator, WorkflowStep, StepExecutor
...
class SearchError(Exception):
    ...
    def __init__(self, message: str, provider: Optional[SearchProvider] = None, 
                 query: Optional[str] = None, error_code: Optional[str] = None):
...
class GoogleSearchProvider(SearchProviderInterface):
        ...
        try:
            ...
            async with self.session.get(self.base_url, params=params) as response:
                if response.status == 200:
                    ...
                    raise SearchError(f"Google Search API error: {response.status}", SearchProvider.GOOGLE)
    ...
    async def _parse_google_results(self, data: Dict[str, Any], query: SearchQuery) -> List[SearchResult]:
        ...
        for item in data.get('items', []):
            try:
                result = SearchResult(
                    ...
                    result_type=ResultType.IMAGE if query.search_type == SearchType.IMAGES else ResultType.WEBPAGE,
...
class BingSearchProvider(SearchProviderInterface):
        ...
        try:
            ...
            async with self.session.get(self.base_url, headers=headers, params=params) as response:
                if response.status == 200:
                    ...
                    raise SearchError(f"Bing Search API error: {response.status}", SearchProvider.BING)
    ...
    async def _parse_bing_results(self, data: Dict[str, Any], query: SearchQuery) -> List[SearchResult]:
...
class DuckDuckGoSearchProvider(SearchProviderInterface):
    ...
    async def search(self, query: SearchQuery) -> List[SearchResult]:
        ...
        try:
            ...
            headers = {
                'User-Agent': 'Mozilla/5.0 (compatible; AI-Assistant-WebSearch/1.0)'
            }
            ...
            async with self.session.get(self.base_url, params=params, headers=headers) as response:
                if response.status == 200:
                    ...
                    raise SearchError(f"DuckDuckGo search failed: {response.status}", SearchProvider.DUCKDUCKGO)
...
class WebSearchStepExecutor(StepExecutor):
    ...
    def can_execute(self, step: WorkflowStep) -> bool:
        ...
        return (step.component_name == "web_search" or 
                step.skill_name == "web_search" or
                "search" in step.step_id.lower())
...
class EnhancedWebSearchManager(ComponentInterface):
    ...
    def __init__(self, container: Container):
        ...
        self.provider_performance: Dict[SearchProvider, deque] = defaultdict(lambda: deque(maxlen=100))
        ...
        try:
            ...
            try:
                provider = DuckDuckGoSearchProvider(
                    self.provider_configs.get("duckduckgo", {}), 
                    self.session
                )
    ...
    async def _process_results(self, results: List[SearchResult], query: SearchQuery) -> List[SearchResult]:
        ...
        try:
            ...
            if (query.extract_content and 
                query.quality in [SearchQuality.BALANCED, SearchQuality.COMPREHENSIVE]):
                await self._extract_content_for_results(results[:5])  # Limit to top 5 for performance
