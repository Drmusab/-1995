...
from src.core.events.event_types import (
    CalendarEventCreated, CalendarEventUpdated, CalendarEventDeleted,
    CalendarSynced, CalendarError, CalendarAuthRequired, CalendarConnected,
    ComponentHealthChanged, ErrorOccurred
)
...
class CalendarError(Exception):
    ...
    def __init__(self, message: str, provider: Optional[CalendarProvider] = None,
                 calendar_id: Optional[str] = None, event_id: Optional[str] = None,
...
class CalendarProviderInterface(ABC):
    ...
    @abstractmethod
    async def authenticate(self, auth_config: CalendarAuthConfig, 
    ...
    @abstractmethod
    async def get_events(self, auth_token: str, calendar_id: str, 
    ...
    @abstractmethod
    async def create_event(self, auth_token: str, calendar_id: str, 
                          event: CalendarEvent) -> CalendarEvent:
        """Create a new calendar event."""
        pass
    
    @abstractmethod
    async def update_event(self, auth_token: str, calendar_id: str, 
                          event: CalendarEvent) -> CalendarEvent:
        """Update an existing calendar event."""
        pass
    
    @abstractmethod
    async def delete_event(self, auth_token: str, calendar_id: str, 
                          event_id: str) -> bool:
        """Delete a calendar event."""
        pass
    
    @abstractmethod
    async def get_free_busy(self, auth_token: str, calendar_ids: List[str],
...
class GoogleCalendarProvider(CalendarProviderInterface):
    ...
    async def authenticate(self, auth_config: CalendarAuthConfig, 
        ...
        try:
            ...
            raise CalendarError(f"Google Calendar authentication failed: {str(e)}", 
                              CalendarProvider.GOOGLE)
    
    async def exchange_code_for_token(self, auth_config: CalendarAuthConfig, 
        ...
        try:
            ...
            raise CalendarError(f"Failed to exchange code for token: {str(e)}", 
                              CalendarProvider.GOOGLE)
    ...
    async def _refresh_token(self, auth_config: CalendarAuthConfig, refresh_token: str) -> Dict[str, Any]:
    ...
    async def get_calendars(self, auth_token: str) -> List[CalendarInfo]:
        ...
        try:
            ...
            raise CalendarError(f"Failed to get Google calendars: {str(e)}", 
                              CalendarProvider.GOOGLE)
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers, params=params) as response:
                    if response.status == 200:
                        data = await response.json()
                        return [self._parse_google_event(event, calendar_id) 
            ...
            raise CalendarError(f"Failed to get Google Calendar events: {str(e)}", 
                              CalendarProvider.GOOGLE, calendar_id)
    ...
    async def update_event(self, auth_token: str, calendar_id: str, 
        ...
        try:
            ...
            raise CalendarError(f"Failed to update Google Calendar event: {str(e)}", 
                              CalendarProvider.GOOGLE, calendar_id, event.id)
    ...
    async def delete_event(self, auth_token: str, calendar_id: str, event_id: str) -> bool:
        ...
        try:
            ...
            raise CalendarError(f"Failed to delete Google Calendar event: {str(e)}", 
                              CalendarProvider.GOOGLE, calendar_id, event_id)
    ...
    async def get_free_busy(self, auth_token: str, calendar_ids: List[str],
        ...
        try:
            ...
            raise CalendarError(f"Failed to get Google Calendar free/busy: {str(e)}", 
                              CalendarProvider.GOOGLE)
...
class CalendarCache:
    ...
    async def get_calendars(self, user_id: str, provider: CalendarProvider) -> Optional[List[CalendarInfo]]:
        """Get cached calendar list."""
        cache_key = f"calendars:{provider.value}:{user_id}"
        return await self.cache.get(cache_key)
    
    async def set_calendars(self, user_id: str, provider: CalendarProvider, 
                           calendars: List[CalendarInfo]) -> None:
        """Cache calendar list."""
        cache_key = f"calendars:{provider.value}:{user_id}"
        await self.cache.set(cache_key, calendars, ttl=self.calendar_list_ttl)
    
    async def get_events(self, user_id: str, calendar_id: str, 
                        start_time: datetime, end_time: datetime) -> Optional[List[CalendarEvent]]:
        """Get cached events."""
        time_key = f"{start_time.isoformat()}:{end_time.isoformat()}"
        cache_key = f"events:{user_id}:{calendar_id}:{time_key}"
        return await self.cache.get(cache_key)
    
    async def set_events(self, user_id: str, calendar_id: str, 
                        start_time: datetime, end_time: datetime, 
...
class EnhancedCalendarAPI:
    ...
    async def authenticate_user(
        ...
        try:
            with self.tracer.trace("calendar_authentication") as span:
                ...
                async with self.credential_locks[user_id]:
                    ...
                    if authorization_code:
                        ...
                        await self.event_bus.emit(CalendarConnected(
                            user_id=user_id,
                            provider=provider.value,
                        ...
                        auth_result = await provider_impl.authenticate(auth_config, existing_credentials)
                        
                        if auth_result.get("requires_user_consent"):
                            # Emit auth required event
                            await self.event_bus.emit(CalendarAuthRequired(
                                user_id=user_id,
                                provider=provider.value,
            ...
            await self.event_bus.emit(CalendarError(
                user_id=user_id,
                provider=provider.value,
                error_message=str(e),
        ...
        try:
            ...
            if hasattr(self.context_manager, 'update_user_context'):
                await self.context_manager.update_user_context(user_id, {
                    f"calendar_{provider.value}_authenticated": True
                })
    ...
    async def get_calendars(
        ...
        try:
            with self.tracer.trace("get_calendars") as span:
                ...
                if not credentials:
                    raise CalendarError(f"No credentials found for user {user_id} and provider {provider.value}")
    ...
    async def get_events(
        ...
        try:
            with self.tracer.trace("get_events") as span:
                ...
                # Limit query size
                max_results = min(max_results or self.max_events_per_query, self.max_events_per_query)
        ...
        try:
            ...
            if session_id:
                await self.memory_manager.store_session_data(
                    session_id=session_id,
                    data_type="recent_calendar_query",
                    data=events_summary
                )
    ...
    async def create_event(
        ...
        try:
            with self.tracer.trace("create_event") as span:
                ...
                # Store in memory
                await self._store_event_operation(
                    user_id, "created", created_event, session_id
                )
                
                # Emit event
                await self.event_bus.emit(CalendarEventCreated(
                    user_id=user_id,
                    calendar_id=calendar_id,
