...
from src.core.events.event_types import (
    ModelRouteSelected, ModelRequestStarted, ModelRequestCompleted, ModelRequestFailed,
    ModelProviderFailover, ModelProviderHealthChanged, ModelCacheHit, ModelCacheMiss,
    ModelRateLimitExceeded, ModelCostThresholdExceeded, ModelPerformanceAlert,
...
class AdaptiveRoutingEngine(RoutingEngine):
    ...
    async def select_model(
        ...
        for model in available_models:
            ...
            if request.required_capabilities and not request.required_capabilities.issubset(model.capabilities):
                continue
    ...
    def _calculate_adaptive_score(self, model: ModelConfig, request: RoutingRequest) -> float:
        ...
        if history:
            recent_success_rate = sum(1 for m in list(history)[-10:] if m.get('success', False)) / min(len(history), 10)
            recent_avg_latency = np.mean([m.get('latency_ms', 1000) for m in list(history)[-10:]])
            recent_avg_quality = np.mean([m.get('quality_score', 0.5) for m in list(history)[-10:] if m.get('quality_score')])
    ...
    def _estimate_latency(self, model: ModelConfig) -> float:
        ...
        history = self.performance_history[model.model_id]
        if history:
            recent_latencies = [m.get('latency_ms', model.avg_latency_ms) for m in list(history)[-10:]]
    ...
    def _estimate_quality(self, model: ModelConfig) -> float:
        ...
        history = self.performance_history[model.model_id]
        if history:
            recent_quality = [m.get('quality_score', model.quality_score) for m in list(history)[-10:] if m.get('quality_score')]
            if recent_quality:
    ...
    def update_performance(self, metrics: RequestMetrics) -> None:
        ...
        self.model_weights[metrics.model_id] = max(0.1, min(2.0, self.model_weights[metrics.model_id]))
...
class RateLimiter:
    ...
    async def check_rate_limit(self, provider_id: str, requests_per_minute: int, tokens_per_minute: int,
                             request_tokens: int = 0) -> bool:
    ...
    async def _check_redis_rate_limit(self, provider_id: str, minute_window: int,
    ...
    def _check_local_rate_limit(self, provider_id: str, minute_window: int,
                               requests_per_minute: int, tokens_per_minute: int,
                               request_tokens: int) -> bool:
...
class ResponseCache:
    ...
    async def cache_response(self, request: RoutingRequest, response: Dict[str, Any], 
...
class ProviderHealthMonitor:
    ...
    async def start_monitoring(self, provider_id: str, provider: BaseLLMProvider,
    ...
    async def _monitor_provider(self, provider_id: str, provider: BaseLLMProvider,
...
class EnhancedModelRouter:
    ...
    async def _load_configurations(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(self.provider_configs)} providers and {len(self.model_configs)} models")
            ...
            if self.database:
                ...
                for row in history_data:
                    metrics = RequestMetrics(
                        ...
                        **{k: v for k, v in row.items() if k not in ['request_id', 'model_id', 'provider_id', 'start_time']}
                    )
            ...
            with self.tracer.trace("model_request_routing") as span:
                ...
                if self.input_sanitizer:
                    ...
                    if request.system_prompt:
                        request.system_prompt = await self.input_sanitizer.sanitize_text(request.system_prompt)
                ...
                if self.enable_caching and self.response_cache and not request.streaming:
                    ...
                    if cached_response:
                        await self.event_bus.emit(ModelCacheHit(
                            request_id=request.request_id,
                ...
                if not cached_response:
                    ...
                    await self.event_bus.emit(ModelCacheMiss(
                        request_id=request.request_id
                    ))
                ...
                if (self.enable_caching and self.response_cache and 
                    response.success and not request.streaming):
                    cache_data = {
    ...
    async def _route_to_model(
        ...
        available_models = [
            model for model in self.model_configs.values()
        ...
        # Emit routing event
        await self.event_bus.emit(ModelRouteSelected(
            request_id=request.request_id,
    ...
    async def _execute_request(
        ...
        if self.enable_rate_limiting:
            ...
            if not rate_limit_ok:
                await self.event_bus.emit(ModelRateLimitExceeded(
                    provider_id=decision.provider_id,
                    request_id=request.request_id
                ))
        ...
        await self.event_bus.emit(ModelRequestStarted(
            request_id=request.request_id,
            model_id=decision.model_id,
            provider_id=decision.provider_id
        ))
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                response = await asyncio.wait_for(
                    provider.generate(request_params),
                ...
                await self.event_bus.emit(ModelRequestCompleted(
                    request_id=request.request_id,
                    model_id=decision.model_id,
                    provider_id=decision.provider_id,
                    tokens_used=response.tokens_used,
                ...
                if attempt < self.max_retries:
                    ...
                    await self.event_bus.emit(ModelRequestFailed(
                        request_id=request.request_id,
                        model_id=decision.model_id,
                        provider_id=decision.provider_id,
                        error_message=str(e),
        ...
        try:
            ...
            if self.database:
                await self.database.execute(
                    ...
                    """,
                    (
                        metrics.request_id, metrics.model_id, metrics.provider_id,
                        metrics.start_time.isoformat(), metrics.total_latency_ms,
                        metrics.total_tokens, metrics.cost, metrics.success, metrics.cached
                    )
                )
    ...
    async def get_available_models(
        ...
        for model_id, model_config in self.model_configs.items():
            ...
            recent_metrics = [
                ...
                m.start_time > datetime.now(timezone.utc) - timedelta(hours=24)
            ]
            
            avg_latency = np.mean([m.total_latency_ms for m in recent_metrics]) if recent_metrics else model_config.avg_latency_ms
