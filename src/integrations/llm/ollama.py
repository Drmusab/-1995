...
class OllamaHealthMonitor:
    ...
    async def start_monitoring(
        self, instance: OllamaInstance, check_interval: float = 30.0
    ) -> None:
                ...
                async with session.get(f"{instance.base_url}/api/tags") as response:
                    if response.status == 200:
                        ...
                        health_factors = {
                            ...
                            "response_time": max(
                                0.0, 1.0 - (instance.avg_response_time / 10.0)
                            ),  # Penalize slow responses
                            "error_rate": max(
                                0.0, 1.0 - (instance.error_count / max(instance.total_requests, 1))
                            ),
...
class OllamaLoadBalancer:
    ...
    def select_instance(
        self, instances: List[OllamaInstance], request: OllamaRequest
    ...
    def _model_affinity_selection(
        self, instances: List[OllamaInstance], model: str
    ) -> OllamaInstance:
...
class OllamaModelManager:
    ...
    async def pull_model(
        self, instance: OllamaInstance, model_name: str
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(f"{instance.base_url}/api/pull", json=payload) as response:
                    if response.status == 200:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    f"{instance.base_url}/api/generate", json=payload
                ) as response:
                    if response.status == 200:
...
class EnhancedOllamaProvider(BaseLLMProvider):
    ...
    def _load_configuration(self) -> None:
        ...
        instances_config = self.config.get(
            "ollama.instances", [{"host": "localhost", "port": 11434}]
        )
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            for instance in self.instances.values():
                ...
                self.health_monitor.health_callbacks[instance.instance_id] = (
                    self._handle_instance_health_change
                )
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=5.0)) as session:
                async with session.get(f"{instance.base_url}/api/tags") as response:
                    if response.status == 200:
                        ...
                        instance.state = OllamaState.ERROR
                        self.logger.error(
                            f"Failed to connect to {instance.instance_id}: HTTP {response.status}"
                        )
        ...
        for instance in self.instances.values():
            if instance.state in [OllamaState.CONNECTED, OllamaState.HEALTHY]:
                try:
                    ...
                    self.logger.warning(
                        f"Model discovery failed for {instance.instance_id}: {str(e)}"
                    )

    async def _start_background_tasks(self) -> None:
        """Start background maintenance tasks."""
        # Model management task
        self.background_tasks.append(asyncio.create_task(self._model_management_loop()))
        ...
        if self.enable_caching:
            self.background_tasks.append(asyncio.create_task(self._cache_management_loop()))

        # Model adaptation task
        if self.model_adapter:
            self.background_tasks.append(asyncio.create_task(self._model_adaptation_loop()))
            ...
            try:
                with self.tracer.trace("ollama_generation") as span:
                    ...
                    instance = self.load_balancer.select_instance(
                        list(self.instances.values()), request
                    )
                    if not instance:
                    ...
                    if request.stream:
                        ...
                        self.performance_stats[f"{instance.instance_id}_{request.model}"].append(
                            response_time
                        )
    ...
    async def _generate_batch(
        self, instance: OllamaInstance, request: OllamaRequest, request_id: str
    ) -> OllamaResponse:
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                timeout=aiohttp.ClientTimeout(total=request.timeout or instance.timeout)
            ) as session:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        ollama_response.quality_score = self._calculate_quality_score(
                            ollama_response
                        )
        ...
        if model not in instance.loaded_models:
            ...
            try:
                ...
                if success:
                    ...
                    await self.event_bus.emit(
                        ModelLoaded(
                            model_name=model, instance_id=instance.instance_id, load_time=load_time
                        )
                    )

                    self.metrics.record("ollama_model_load_time_seconds", load_time)
                    self.logger.info(
                        f"Loaded model {model} on {instance.instance_id} in {load_time:.2f}s"
                    )
                else:
                ...
                await self.event_bus.emit(
                    ModelError(
                        model_name=model, instance_id=instance.instance_id, error_message=str(e)
                    )
                )
                raise
    ...
    async def _cache_response(self, request: OllamaRequest, response: OllamaResponse) -> None:
        ...
        try:
            ...
            await self.cache_strategy.set(
                cache_key, json.dumps(response_data, default=str), ttl=self.cache_ttl
            )
        except Exception as e:
            ...
            for instance in self.instances.values():
                if instance.state in [OllamaState.HEALTHY, OllamaState.CONNECTED]:
                    try:
                        ...
                        self.logger.warning(
                            f"Failed to list models from {instance.instance_id}: {str(e)}"
                        )

            return all_models

    async def switch_model(
        self, old_model: str, new_model: str, instance_id: Optional[str] = None
    ) -> bool:
        """
    ...
    async def pull_model(
        self, model_name: str, instance_id: Optional[str] = None
    ) -> AsyncGenerator[Dict[str, Any], None]:
        """
        ...
        for instance in instances_to_pull:
            try:
                ...
                yield {"instance_id": instance.instance_id, "status": "error", "error": str(e)}
    ...
    # Event handlers
    async def _handle_instance_health_change(
        self, instance: OllamaInstance, old_state: OllamaState, new_state: OllamaState
    ) -> None:
        ...
        self.logger.info(
            f"Instance {instance.instance_id} state changed: {old_state.value} -> {new_state.value}"
        )
    ...
    async def _health_check_callback(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "total_capacity": sum(
                    instance.config.get("max_requests", 10) for instance in self.instances.values()
                ),
            }
