...
from openai.types import ChatCompletion, CreateEmbeddingResponse, Embedding
...
class EnhancedOpenAIProvider(BaseLLMProvider):
    ...
    def _load_config(self) -> OpenAIConfig:
        ...
        return OpenAIConfig(
            ...
            default_embedding_model=config_data.get(
                "default_embedding_model", OpenAIModel.TEXT_EMBEDDING_3_LARGE.value
            ),
        ...
        async with self.request_semaphore:
            ...
            try:
                with self.tracer.trace("openai_chat_completion") as span:
                    ...
                    if chat_response.usage:
                        self.metrics.record(
                            "openai_tokens_per_request", chat_response.usage.get("total_tokens", 0)
                        )
    ...
    async def _stream_chat_completion(
        self, openai_params: Dict[str, Any], request_id: str
    ...
    async def _process_chat_response(
        ...
        if request.stream:
            ...
            cost = self.cost_calculator.calculate_chat_cost(model, prompt_tokens, completion_tokens)
    ...
    async def _calculate_response_quality(
        self, content: str, request: ChatRequest, response: ChatCompletion
    ) -> float:
            ...
            try:
                with self.tracer.trace("openai_embeddings") as span:
                    ...
                    for i in range(0, len(input_texts), request.batch_size):
                        ...
                        # Count tokens for rate limiting
                        batch_tokens = sum(
                            self.token_counter.count_tokens(text, model) for text in batch
                        )
                        ...
                        batch_response = await self.circuit_breaker.call(
                            self.client.embeddings.create, **openai_params
                        )

                        # Extract embeddings
                        batch_embeddings = [
                            embedding.embedding for embedding in batch_response.data
                        ]
        ...
        async with self.request_semaphore:
            ...
            try:
                with self.tracer.trace("openai_image_generation") as span:
                    ...
                    # Make API call
                    response = await self.circuit_breaker.call(
                        self.client.images.generate, **openai_params
                    )
    ...
    async def _check_cache(self, request: Any, request_type: str) -> Optional[Any]:
        ...
        try:
            ...
            cache_key = (
                ...
                else await self._generate_cache_key(request, request_type)
            )
    ...
    async def _generate_cache_key(self, request: Any, request_type: str) -> str:
        ...
        try:
            ...
            cache_dict = {
                k: v for k, v in request_dict.items() if k not in ["cache_key", "metadata", "user"]
            }
    ...
    async def _update_usage_stats(
        self, response: Union[ChatResponse, EmbeddingResponse, ImageResponse]
    ) -> None:
        ...
        try:
            ...
            if self.response_times:
                self.usage_stats.average_response_time = sum(self.response_times) / len(
                    self.response_times
                )
