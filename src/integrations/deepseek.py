...
from src.core.events.event_types import (
    LLMRequestStarted, LLMRequestCompleted, LLMRequestFailed,
    LLMCacheHit, LLMContextWindowExceeded, LLMRateLimitExceeded,
...
from src.integrations.llm.base_provider import (
    BaseLLMProvider, LLMProviderType, ModelCapability, ModelInfo,
    LLMRequest, LLMResponse, LLMError, RequestPriority, ProviderStatus
)
...
class DeepSeekProvider(BaseLLMProvider):
        ...
        try:
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    self.logger.info(f"API connectivity test successful. Available models: {len(data.get('data', []))}")
                elif response.status == 401:
                    raise DeepSeekError("Invalid API key", "INVALID_API_KEY", response.status)
                else:
                    raise DeepSeekError(f"API connectivity test failed: {response.status}", 
                                      "CONNECTIVITY_ERROR", response.status)
                    
        except aiohttp.ClientError as e:
            raise DeepSeekError(f"Network error during connectivity test: {str(e)}", "NETWORK_ERROR")
    ...
    async def _check_rate_limits(self, request: LLMRequest) -> None:
        ...
        while (self.rate_limiter["request_window"] and 
               self.rate_limiter["request_window"][0] < minute_ago):
            self.rate_limiter["request_window"].popleft()
        
        # Check request rate limit
        if len(self.rate_limiter["request_window"]) >= self.config.rate_limit_rpm:
            await self.event_bus.emit(LLMRateLimitExceeded(
    ...
    async def _handle_response(self, response: aiohttp.ClientResponse) -> Dict[str, Any]:
        ...
        if response.status == 200:
            ...
            raise DeepSeekError(f"Rate limit exceeded. Retry after {retry_after} seconds", 
                              "RATE_LIMIT_EXCEEDED", response.status)
        
        elif response.status == 400:
            error_data = await response.json()
            raise DeepSeekError(f"Bad request: {error_data.get('error', {}).get('message', 'Unknown error')}", 
                              "BAD_REQUEST", response.status, error_data)
            ...
            raise DeepSeekError(f"Unexpected status code {response.status}: {error_text}", 
                              "UNEXPECTED_ERROR", response.status)

    async def _stream_api_request(self, url: str, data: Dict[str, Any]) -> AsyncGenerator[Dict[str, Any], None]:
    ...
    async def generate_text(self, 
                          ...
                          stream: bool = False,
                          **kwargs) -> Union[LLMResponse, AsyncGenerator[str, None]]:
        """
        ...
        async with self.request_semaphore:
            ...
            try:
                ...
                if stream:
                    ...
                    return await self._create_response(response_data, request_id, start_time, model_id)
    ...
    async def _handle_streaming_completion(self, api_data: Dict[str, Any], request: LLMRequest) -> LLMResponse:
...
async def create_deepseek_provider(container: Container, config: Optional[Dict[str, Any]] = None) -> DeepSeekProvider:
    """
