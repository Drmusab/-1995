...
class LocalCache(Generic[K, V]):
    ...
    def __init__(
        ...
        self._thread_pool = ThreadPoolExecutor(
            max_workers=self.config.thread_pool_size, thread_name_prefix=f"cache_{name}"
        )
        ...
        self.logger.info(
            f"LocalCache '{name}' initialized with {self.config.eviction_policy.value} eviction policy"
        )
        ...
        with self._lock:
            try:
                ...
                if str_key not in self._entries:
                    ...
                    if self.event_bus:
                        asyncio.create_task(
                            self.event_bus.emit(CacheMiss(cache_name=self.name, key=str_key))
                        )
                    return default
                ...
                if entry.is_expired:
                    ...
                    if self.event_bus:
                        asyncio.create_task(
                            self.event_bus.emit(
                                CacheExpired(
                                    cache_name=self.name, key=str_key, age_seconds=entry.age_seconds
                                )
                            )
                        )
                ...
                if self.event_bus:
                    asyncio.create_task(
                        self.event_bus.emit(
                            CacheHit(cache_name=self.name, key=str_key, access_time_ms=access_time)
                        )
                    )
                ...
                if self.event_bus:
                    asyncio.create_task(
                        self.event_bus.emit(
                            CacheError(cache_name=self.name, key=str_key, error_message=str(e))
                        )
                    )
                return default
        ...
        with self._lock:
            try:
                ...
                entry = CacheEntry(
                    ...
                    compression_type=(
                        self.config.compression_type if is_compressed else CompressionType.NONE
                    ),
                    namespace=namespace,
                    tags=tags or set(),
                ...
                self.logger.debug(
                    f"Cached key '{str_key}' in namespace '{namespace}' "
        ...
        with self._lock:
            if namespace is None:
                ...
                if self.event_bus:
                    asyncio.create_task(
                        self.event_bus.emit(
                            CacheCleared(cache_name=self.name, entries_cleared=count)
                        )
                    )
                ...
                keys_to_remove = [
                    key for key, entry in self._entries.items() if entry.namespace == namespace
                ]
                ...
                self.logger.info(
                    f"Cleared {len(keys_to_remove)} entries from namespace '{namespace}'"
                )
    ...
    def size(self, namespace: Optional[str] = None) -> int:
        ...
        with self._lock:
            if namespace is None:
                ...
                return sum(1 for entry in self._entries.values() if entry.namespace == namespace)
    ...
    async def warm_cache(
        self,
        data_loader: Callable[[List[K]], Dict[K, V]],
        keys: List[K],
        ...
        try:
            ...
            for i in range(0, len(keys), batch_size):
                ...
                try:
                    data = await asyncio.get_event_loop().run_in_executor(
                        self._thread_pool, lambda: data_loader(batch_keys)
                    )
            ...
            if self.event_bus:
                await self.event_bus.emit(
                    CacheWarmed(cache_name=self.name, entries_warmed=warmed_count)
                )
    ...
    def _compress_value(self, value: Any) -> tuple[Any, bool, int]:
        ...
        try:
            ...
            if self.config.compression_enabled and size_bytes > self.config.compression_threshold:
        ...
        with self._stats_lock:
            ...
            if total_requests > 0:
                self.statistics.average_access_time_ms = (
                    current_avg * (total_requests - 1) + access_time_ms
                ) / total_requests
        ...
        while True:
            try:
                ...
                if self.metrics:
                    ...
                    self.metrics.set("cache_entries", len(self._entries), tags={"cache": self.name})
                    self.metrics.set(
                        "cache_memory_usage_bytes",
                        stats.memory_usage_bytes,
                        tags={"cache": self.name},
                    )
                    self.metrics.set("cache_hit_ratio", stats.hit_ratio, tags={"cache": self.name})
                    self.metrics.set(
...
class LocalCacheManager:
    ...
    async def _create_standard_caches(self) -> None:
        ...
        processing_config = CacheConfiguration(
            ...
            default_ttl_seconds=self.config.get(
                "cache.processing.ttl_seconds", 1800.0
            ),  # 30 minutes
            eviction_policy=CachePolicy.LRU,
    ...
    async def create_cache(
        self, name: str, config: Optional[CacheConfiguration] = None
    ) -> LocalCache:
        """
        ...
        with self._manager_lock:
            ...
            cache = LocalCache(
                name=name, config=cache_config, event_bus=self.event_bus, metrics=self.metrics
            )
    ...
    def list_caches(self) -> List[Dict[str, Any]]:
        ...
        for name, cache in self._caches.items():
            try:
                ...
                caches_info.append({"name": name, "error": str(e)})
    ...
    def cache_session_data(
        self, session_id: str, data: Any, ttl_seconds: Optional[float] = None
    ) -> bool:
        """Cache session data."""
        try:
    ...
    @handle_exceptions
    def cache_component_result(
        self, component_id: str, input_hash: str, result: Any, ttl_seconds: Optional[float] = None
    ) -> bool:
    ...
    def cache_workflow_state(
        self, workflow_id: str, execution_id: str, state: Any, ttl_seconds: Optional[float] = None
    ) -> bool:
    ...
    def cache_processing_result(
        self, request_hash: str, result: Any, ttl_seconds: Optional[float] = None
    ) -> bool:
    ...
    @handle_exceptions
    def cache_memory_data(
        self, memory_id: str, data: Any, ttl_seconds: Optional[float] = None
    ) -> bool:
    ...
    @handle_exceptions
    def cache_plugin_data(
        self, plugin_id: str, data_key: str, data: Any, ttl_seconds: Optional[float] = None
    ) -> bool:
        """Cache plugin data."""
        try:
