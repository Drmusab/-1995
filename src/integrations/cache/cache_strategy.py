...
from src.core.events.event_types import (
    CacheHit, CacheMiss, CacheEviction, CacheWarming, CacheInvalidation,
    CacheCleanup, MemoryPressureAlert, CacheCoherenceEvent, CacheHealthCheck,
    SystemPerformanceEvent, ComponentHealthChanged, SessionStarted, SessionEnded,
    WorkflowCompleted, ProcessingCompleted, ErrorOccurred
)
...
class MemoryCacheBackend:
    ...
    async def set(self, key: str, value: Any, ttl: Optional[float] = None, 
                 priority: CachePriority = CachePriority.NORMAL) -> None:
    ...
    async def _evict_lfu(self, count: int) -> None:
        ...
        sorted_keys = sorted(
            self.data.keys(),
            key=lambda k: (self.data[k].priority.value, self.frequency_counter[k])
        )
...
class CacheWarmingService:
    ...
    def register_warming_pattern(self, name: str, patterns: List[str], 
                                warming_function: Callable) -> None:
...
class CacheCoherenceManager:
    ...
    async def invalidate_key(self, key: str, node_id: Optional[str] = None) -> None:
        ...
        await self.event_bus.emit(CacheInvalidation(
            cache_key=key,
...
class EnhancedCacheStrategy:
    ...
    async def _register_event_handlers(self) -> None:
        ...
        self.event_bus.subscribe("cache_invalidation", self.coherence_manager.handle_invalidation_event)
    ...
    async def _start_background_tasks(self) -> None:
        ...
        if self.enable_coherence:
            self.background_tasks.append(
                asyncio.create_task(self._coherence_sync_loop())
            )
    ...
    async def _setup_warming_patterns(self) -> None:
        ...
        self.warming_service.register_warming_pattern(
            ...
            self._warm_session_data
        )
        
        # Component data warming
        self.warming_service.register_warming_pattern(
            "component_data",
            ["component:*:config", "component:*:state"],
            self._warm_component_data
        )
            ...
            with self.tracer.trace("cache_set") as span:
                ...
                for level in levels:
                    if level in self.cache_backends:
                        backend = self.cache_backends[level]
                        task = self._store_in_backend(backend, full_key, value, effective_ttl, priority)
                        tasks.append(task)
                ...
                if self.enable_coherence:
                    await self.event_bus.emit(CacheCoherenceEvent(
                        operation="set",
                        cache_key=full_key,
        ...
        try:
            ...
            for level in self.cache_backends:
                ...
                try:
                    ...
                    self.logger.warning(f"Namespace invalidation failed for level {level}: {str(e)}")
    ...
    async def warm_cache(
        ...
        try:
            ...
            # Emit warming event
            await self.event_bus.emit(CacheWarming(
                namespace=namespace,
    ...
    async def _record_cache_hit(self, namespace: str, level: CacheLevel, key: str) -> None:
        ...
        self.metrics.increment("cache_hits_total", tags={"namespace": namespace, "level": level.value})
    ...
    async def _record_cache_set(self, namespace: str, key: str, value: Any) -> None:
        ...
        self.metrics.increment("cache_operations_total", tags={"namespace": namespace, "operation": "set"})

    def _record_operation_time(self, operation: str, duration: float) -> None:
        """Record operation timing metrics."""
        self.operation_times[operation].append(duration)
        self.metrics.record("cache_operation_duration_seconds", duration, tags={"operation": operation})
    ...
    async def _cleanup_loop(self) -> None:
        ...
        while True:
            try:
                ...
                if cleanup_count > 0:
                    await self.event_bus.emit(CacheCleanup(
                        entries_cleaned=cleanup_count,
        ...
        while True:
            try:
                ...
                for namespace, stats in self.cache_statistics.items():
                    ...
                    # Update metrics
                    self.metrics.set("cache_hit_rate", stats.hit_rate, tags={"namespace": namespace})
                    self.metrics.set("cache_size_bytes", stats.total_size_bytes, tags={"namespace": namespace})
        ...
        while True:
            try:
                ...
                if self.memory_pressure_level > 0.85:  # 85% memory usage
                    await self.event_bus.emit(MemoryPressureAlert(
    ...
    async def _handle_processing_completed(self, event) -> None:
        ...
        try:
            ...
            if hasattr(event, 'result_type') and event.result_type in ['model_output', 'workflow_result']:
