...
from src.core.events.event_types import (
    ...
    GraphIndexUpdated,
    GraphQueryExecuted,
    GraphReasoningCompleted,
    GraphReasoningStarted,
    GraphTraversalCompleted,
    GraphTraversalStarted,
    KnowledgeGraphUpdated,
    KnowledgeInferred,
    RelationshipAdded,
    RelationshipRemoved,
    RelationshipUpdated,
    SimilaritySearchPerformed,
    SystemStateChanged,
)
...
class GraphRelationship:
    ...
    def __str__(self) -> str:
        return (
            f"{self.source_entity_id} --{self.relationship_type.value}--> {self.target_entity_id}"
        )
...
class GraphIndex:
    ...
    def index_relationship(self, relationship: GraphRelationship) -> None:
        ...
        try:
            # Type index
            self.relationships_by_type[relationship.relationship_type].add(
            ...
            if relationship.valid_from or relationship.valid_until:
                timeframe_key = self._get_timeframe_key(
            ...
            self.logger.error(
                f"Failed to index relationship {relationship.relationship_id}: {str(e)}"
            )
    ...
    def _get_timeframe_key(
        self, valid_from: Optional[datetime], valid_until: Optional[datetime]
    ) -> str:
    ...
    def get_neighbors(
        self, entity_id: str, relationship_types: Optional[Set[RelationshipType]] = None
    ) -> Set[str]:
...
class GraphStorage:
    ...
    def __init__(
        self, database: Optional[DatabaseManager] = None, vector_store: Optional[VectorStore] = None
    ):
    ...
    async def store_relationship(
        self, relationship: GraphRelationship, persist: bool = True
    ) -> None:
        ...
        with self.lock:
            ...
            if persist and self.database:
                try:
                    ...
                    self.logger.error(
                        f"Failed to persist relationship {relationship.relationship_id}: {str(e)}"
                    )
    ...
    async def _load_relationship(self, relationship_id: str) -> Optional[GraphRelationship]:
        ...
        result = await self.database.fetch_one(
            "SELECT * FROM graph_relationships WHERE relationship_id = ?", (relationship_id,)
        )
    ...
    async def _delete_entity_from_db(self, entity_id: str) -> None:
        ...
        if self.database:
            await self.database.execute(
                "DELETE FROM graph_entities WHERE entity_id = ?", (entity_id,)
            )
    ...
    async def _delete_relationship_from_db(self, relationship_id: str) -> None:
        ...
        if self.database:
            await self.database.execute(
                "DELETE FROM graph_relationships WHERE relationship_id = ?", (relationship_id,)
            )
...
class GraphQueryEngine:
    ...
    async def _execute_path_finding(self, query: GraphQuery) -> GraphQueryResult:
        ...
        for path_entities, path_relationships in paths:
            path = GraphPath(
                path_id=str(uuid.uuid4()), entities=path_entities, relationships=path_relationships
            )
    ...
    async def _find_paths_bfs(
        self, source_id: str, target_id: str, max_hops: int, query: GraphQuery
        ...
        queue = deque(
            [(source_id, [source_id], [])]
        )  # (current_entity, path_entities, path_relationships)
        visited = set()
        paths = []
        ...
        while queue and len(paths) < query.max_results:
            ...
            for rel_id in outgoing_rel_ids:
                ...
                if (
                    ...
                    and relationship.relationship_type not in query.relationship_types
                ):
                    continue
    ...
    async def _extract_subgraph_recursive(
        self,
        entity_id: str,
        remaining_radius: int,
    ...
    async def _execute_similarity_search(self, query: GraphQuery) -> GraphQueryResult:
        ...
        result.confidence = sum(
            similarities[i][1] for i in range(min(len(similarities), query.max_results))
        ) / max(len(result.entities), 1)
    ...
    def _passes_relationship_filters(
        self, relationship: GraphRelationship, query: GraphQuery
    ) -> bool:
...
class GraphReasoningEngine:
    ...
    async def perform_reasoning(
        ...
        result = ReasoningResult(reasoning_id=str(uuid.uuid4()), success=False, mode=mode)
    ...
    async def _perform_traversal_reasoning(
        self, parameters: Dict[str, Any], result: ReasoningResult, context: Dict[str, Any]
    ) -> None:
    ...
    async def _traverse_for_reasoning(
        self,
        current_entity: str,
        ...
        max_depth: int,
        visited: Set[str],
        reasoning_paths: List[Tuple[List[str], List[str]]],
        ...
        if depth > 1 and self._is_interesting_reasoning_path(
            current_path_entities, path_relationships
        ):
    ...
    async def _apply_transitivity_inference(
        self, result: ReasoningResult, context: Dict[str, Any]
    ) -> None:
    ...
    async def _apply_symmetry_inference(
        self, result: ReasoningResult, context: Dict[str, Any]
    ) -> None:
        ...
        for rel in all_relationships:
            if rel.relationship_type in symmetric_types:
                ...
                reverse_exists = any(
                    r.source_entity_id == rel.target_entity_id
                    and r.target_entity_id == rel.source_entity_id
    ...
    async def _apply_inverse_inference(
        self, result: ReasoningResult, context: Dict[str, Any]
    ) -> None:
