...
from src.core.events.event_types import (
    ...
    LogicEngineHealthChanged,
    LogicReasoningCompleted,
    LogicReasoningFailed,
    LogicReasoningStarted,
    ProofGenerated,
    QueryProcessed,
    RuleAdded,
    RuleLearned,
...
class Proof:
    ...
    def add_step(
        self, step_type: str, description: str, formulas: List[LogicalFormula] = None
    ) -> None:
        """Add a proof step."""
        step = {
...
class LogicError(Exception):
    ...
    def __init__(
        self, message: str, logic_type: Optional[LogicType] = None, error_code: Optional[str] = None
    ):
...
class UnificationEngine:
    ...
    def unify_terms(
        self, term1: Term, term2: Term, substitution: Optional[Substitution] = None
        ...
        if (
            ...
            and term1.term_type == term2.term_type
            and len(term1.arguments) == len(term2.arguments)
        ):
    ...
    def _unify_variable(
        self, var: str, term: Term, substitution: Substitution
    ) -> Optional[Substitution]:
        ...
        if term.is_variable() and term.name in substitution.bindings:
            return self.unify_terms(
                Term(var, "variable"), substitution.bindings[term.name], substitution
            )
...
class ResolutionEngine:
    ...
    def prove_by_resolution(
        self, premises: List[LogicalFormula], goal: LogicalFormula
    ) -> Optional[Proof]:
        """Prove goal using resolution."""
        proof = Proof(proof_id=str(uuid.uuid4()), conclusion=goal, method="resolution")
        ...
        proof.add_step(
            "conversion", "Converted premises and negated goal to clause form", premises + [goal]
        )
                ...
                for j in range(i + 1, len(clauses)):
                    ...
                    for resolvent in resolvents:
                        if not resolvent:  # Empty clause found
                            proof.add_step(
                                "resolution", f"Derived empty clause from clauses {i} and {j}"
                            )
                        ...
                        if resolvent_tuple not in new_clauses:
                            ...
                            proof.add_step(
                                "resolution", f"Resolved clauses {i} and {j} to get {resolvent}"
                            )
...
class ForwardChainEngine:
    ...
    def forward_chain(
        self, facts: List[LogicalFormula], rules: List[LogicalRule], max_iterations: int = 100
    ) -> ReasoningResult:
        ...
        while iteration < max_iterations:
            ...
            for rule in rules:
                ...
                satisfying_substitutions = self._find_satisfying_substitutions(
                    new_facts, rule.premises
                )
    ...
    def _find_satisfying_substitutions(
        self, facts: List[LogicalFormula], premises: List[LogicalFormula]
    ) -> List[Substitution]:
    ...
    def _recursive_satisfaction(
        self,
        facts: List[LogicalFormula],
        premises: List[LogicalFormula],
        premise_index: int,
        current_substitution: Substitution,
    ) -> List[Substitution]:
        ...
        for fact in facts:
            if isinstance(current_premise.content, Predicate) and isinstance(
...
class BackwardChainEngine:
    ...
    def _prove_goal(
        ...
        for rule in rules:
            ...
            if isinstance(rule.conclusion.content, Predicate) and isinstance(
                goal.content, Predicate
            ):
                substitution = self.unification_engine.unify_predicates(
...
class ConstraintSolver:
        ...
        for value in domains[var]:
            if self._is_consistent(var, value, assignment, constraints):
                ...
                if self._backtrack_search(
                    variables, domains, constraints, assignment, start_time, timeout
                ):
                    return True
    ...
    def _is_consistent(
        self, var: str, value: Any, assignment: Dict[str, Any], constraints: List[Callable]
    ) -> bool:
...
class ExplanationGenerator:
    ...
    async def explain_reasoning(
        self, result: ReasoningResult, context: Dict[str, Any] = None
    ) -> str:
        ...
        if len(result.conclusions) == 1:
            explanation_parts.append(
                f"Based on the provided information, I can conclude that: {result.conclusions[0]}"
            )
        else:
            explanation_parts.append(
                "Based on the provided information, I can conclude the following:"
            )
            ...
            if persist and self.memory_manager:
                try:
                    await self.memory_manager.store_semantic_memory(
                        content=str(fact), memory_type="logical_fact", metadata=fact.metadata
                    )
                except Exception as e:
    ...
    async def add_rule(self, rule: LogicalRule, persist: bool = True) -> None:
        ...
        with self.lock:
            ...
            if persist and self.memory_manager:
                try:
                    await self.memory_manager.store_semantic_memory(
                        content=str(rule), memory_type="logical_rule", metadata=rule.metadata
                    )
                except Exception as e:
...
class LogicEngine:
    ...
    def _setup_logical_operators(self) -> None:
        ...
        self.logical_operators = {
            ...
            "¬": LogicalOperator(
                ...
                4,
                "right",
                lambda a: TruthValue.TRUE if a == TruthValue.FALSE else TruthValue.FALSE,
            ),
            "→": LogicalOperator(
                "→",
                "implies",
                ...
                lambda a, b: (
                    TruthValue.FALSE
                    if a == TruthValue.TRUE and b == TruthValue.FALSE
            ...
            "↔": LogicalOperator(
                ...
                1,
                "left",
                lambda a, b: TruthValue.TRUE if a == b else TruthValue.FALSE,
            ),
    ...
    def _load_builtin_rules(self) -> None:
        ...
        modus_ponens = LogicalRule(
            ...
            premises=[LogicalFormula("mp_premise1", "P → Q"), LogicalFormula("mp_premise2", "P")],
        ...
        modus_tollens = LogicalRule(
            ...
            premises=[LogicalFormula("mt_premise1", "P → Q"), LogicalFormula("mt_premise2", "¬Q")],
    ...
    async def _load_persisted_knowledge(self) -> None:
        ...
        try:
            ...
            fact_memories = await self.memory_manager.retrieve_memories(
                memory_type="logical_fact", limit=10000
            )
            ...
            rule_memories = await self.memory_manager.retrieve_memories(
                memory_type="logical_rule", limit=1000
            )
            ...
            self.logger.info(
                f"Loaded knowledge base: {len(self.knowledge_base.facts)} facts, {len(self.knowledge_base.rules)} rules"
            )
        ...
        try:
            ...
            if "(" in fact_str and ")" in fact_str:
                ...
                return LogicalFormula(
                    formula_id=str(uuid.uuid4()), content=predicate, logic_type=LogicType.PREDICATE
                )
            else:
            ...
            try:
                with self.tracer.trace("logic_reasoning") as span:
                    ...
                    if context.get("generate_explanation", False):
                        result.explanation = await self.explanation_generator.explain_reasoning(
                            result, context
                        )
