...
from src.core.events.event_types import (
    GoalDecompositionStarted, GoalDecompositionCompleted, GoalDecompositionFailed,
    GoalDecomposed, SubGoalGenerated, GoalPrioritized, GoalDependencyDetected,
    GoalConflictDetected, GoalConflictResolved, GoalHierarchyUpdated,
    StrategySelected, GoalAdapted, GoalAchieved, GoalAbandoned,
...
from src.reasoning.planning.task_planner import TaskPlanner, Task, TaskPriority, ExecutionPlan
...
from src.memory.core_memory.memory_types import WorkingMemory, SemanticMemory, EpisodicMemory
...
class GoalConstraint:
    ...
    def _check_temporal_constraint(self, goal: 'Goal', context: Dict[str, Any]) -> Tuple[bool, float]:
    ...
    def _check_resource_constraint(self, goal: 'Goal', context: Dict[str, Any]) -> Tuple[bool, float]:
    ...
    def _check_dependency_constraint(self, goal: 'Goal', context: Dict[str, Any]) -> Tuple[bool, float]:
    ...
    def _check_quality_constraint(self, goal: 'Goal', context: Dict[str, Any]) -> Tuple[bool, float]:
...
class GoalHierarchy:
    ...
    def get_ready_goals(self) -> List[Goal]:
        ...
        for goal in self.goals.values():
            if (goal.status == GoalStatus.PLANNED and 
                goal.dependencies.issubset(self.achieved_goals)):
                ready.append(goal)
        return ready
...
class GoalDecompositionError(Exception):
    ...
    def __init__(self, message: str, goal_id: Optional[str] = None, 
                 hierarchy_id: Optional[str] = None, error_code: Optional[str] = None):
...
class DecompositionHeuristic(ABC):
    ...
    @abstractmethod
    def estimate_decomposition_quality(self, original_goal: Goal, 
                                     sub_goals: List[Goal], context: Dict[str, Any]) -> float:
            ...
            if patterns:
                ...
                for i, sub_pattern in enumerate(sub_patterns):
                    sub_goal = Goal(
                        ...
                        estimated_completion_time=sub_pattern.get('estimated_time', 
                                                                goal.estimated_completion_time / len(sub_patterns)),
                        context=goal.context.copy()
                    )
...
class ConflictResolver:
    ...
    async def detect_conflicts(self, hierarchy: GoalHierarchy) -> List[Tuple[str, str, ConflictType]]:
    ...
    async def _check_goal_conflict(self, goal1: Goal, goal2: Goal, 
                                 hierarchy: GoalHierarchy) -> Optional[ConflictType]:
    ...
    def _compete_for_resources(self, goal1: Goal, goal2: Goal) -> bool:
        ...
        common_resources = set(goal1.resource_requirements.keys()) & set(goal2.resource_requirements.keys())
    ...
    async def resolve_conflicts(self, hierarchy: GoalHierarchy, 
                              user_id: Optional[str] = None) -> Dict[str, Dict[str, Any]]:
    ...
    async def _resolve_single_conflict(self, goal1: Goal, goal2: Goal, 
    ...
    async def _resolve_resource_conflict(self, goal1: Goal, goal2: Goal, 
    ...
    async def _resolve_temporal_conflict(self, goal1: Goal, goal2: Goal, 
                                       user_prefs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _resolve_logical_conflict(self, goal1: Goal, goal2: Goal, 
                                      user_prefs: Dict[str, Any]) -> Dict[str, Any]:
...
class GoalDecomposer:
    ...
    def _setup_heuristics(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Initialized {len(self.decomposition_heuristics)} decomposition heuristics")
    ...
    def _load_goal_templates(self) -> None:
        ...
        try:
            ...
            qa_template = GoalTemplate(
                "question_answering",
                "Question Answering",
                "Answer user questions effectively"
            )
            ...
            creative_template.goal_patterns = [
                ...
                {
                    'description': 'Refine and present results',
                    'type': 'creative',
                    'priority': 3
                }
            ]
    ...
    async def create_goal_hierarchy_from_intent(
        ...
        try:
            with self.tracer.trace("goal_hierarchy_creation") as span:
                ...
                await self.event_bus.emit(GoalDecompositionStarted(
                    session_id=session_id,
                    intent=intent,
                    user_id=user_id
                ))
                ...
                if self.conflict_resolution_enabled:
                    ...
                    if conflicts:
                        ...
                        await self.event_bus.emit(GoalConflictDetected(
                ...
                await self.event_bus.emit(GoalDecompositionCompleted(
                    hierarchy_id=hierarchy.hierarchy_id,
                    session_id=session_id,
            ...
            await self.event_bus.emit(GoalDecompositionFailed(
                session_id=session_id,
                intent=intent,
                error_message=str(e),
                decomposition_time=decomposition_time
    ...
    async def _classify_goal_type(self, intent: str, context: Dict[str, Any]) -> GoalType:
        ...
        try:
            ...
            elif 'create' in intent.lower() or 'generate' in intent.lower() or 'design' in intent.lower():
    ...
    async def _estimate_resource_requirements(self, intent: str, context: Dict[str, Any]) -> Dict[str, float]:
    ...
    async def _decompose_goal_hierarchy(
        ...
        for sub_goal in best_sub_goals:
            ...
            await self.event_bus.emit(SubGoalGenerated(
                hierarchy_id=hierarchy.hierarchy_id,
    ...
    async def plan_goal_execution(
        ...
        try:
            ...
            self.logger.error(f"Failed to plan goal execution for hierarchy {hierarchy_id}: {str(e)}")
    ...
    async def update_goal_progress(
        ...
        if goal.progress >= 1.0:
            ...
            await self.event_bus.emit(GoalAchieved(
                hierarchy_id=hierarchy_id,
                goal_id=goal_id,
                achievement_time=(goal.actual_completion_time - goal.created_at).total_seconds(),
