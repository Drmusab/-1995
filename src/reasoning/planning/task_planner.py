...
class TaskDecomposer:
        ...
        if sub_skills:
            ...
            for i, sub_skill in enumerate(sub_skills):
                sub_task = Task(
                    ...
                    estimated_duration=sub_skill.get(
                        "estimated_duration", task.estimated_duration / len(sub_skills)
                    ),
            ...
            if sub_operations:
                ...
                for i, operation in enumerate(sub_operations):
                    sub_task = Task(
                        ...
                        estimated_duration=operation.get(
                            "duration", task.estimated_duration / len(sub_operations)
                        ),
                        priority=task.priority,
    ...
    async def _decompose_generic_task(self, task: Task, context: Dict[str, Any]) -> List[Task]:
        ...
        try:
            ...
            decomposition_patterns = await self.knowledge_graph.query_decomposition_patterns(
                decomposition_query
            )
            ...
            if decomposition_patterns:
                ...
                for i, sub_pattern in enumerate(pattern.get("sub_tasks", [])):
                    sub_task = Task(
                        ...
                        estimated_duration=sub_pattern.get(
                            "duration", task.estimated_duration / len(pattern["sub_tasks"])
                        ),
                        priority=task.priority,
...
class TaskPlanner:
            ...
            try:
                with self.tracer.trace("plan_creation") as span:
                    ...
                    await self.event_bus.emit(
                        TaskPlanningStarted(
                            session_id=session_id, intent=intent, strategy=strategy.value
                        )
                    )
    ...
    async def _generate_tasks_for_intent(self, intent: str, context: Dict[str, Any]) -> List[Task]:
        """Generate initial tasks based on intent."""
        tasks = []
            ...
            if pattern:
                ...
                for i, task_def in enumerate(pattern.get("tasks", [])):
                    task = Task(
                        ...
                        required_resources=self._parse_resource_requirements(
                            task_def.get("resources", {})
                        ),
                    )
                    tasks.append(task)
    ...
    def _parse_resource_requirements(
        self, resources_dict: Dict[str, Any]
    ...
    async def _optimize_hierarchical(self, plan: ExecutionPlan) -> None:
        ...
        for level in sorted(task_levels.keys()):
            ...
            level_tasks.sort(
                key=lambda t: (t.priority.value, -self.heuristic.estimate_cost(t, plan.context)),
                reverse=True,
            )
    ...
    async def _create_execution_order(
        self, tasks: List[Task], score_func: Callable[[Task], float]
    ) -> List[List[str]]:
    ...
    async def _calculate_plan_metrics(self, plan: ExecutionPlan) -> None:
        ...
        for batch in plan.execution_order:
            batch_duration = (
                max(plan.tasks[task_id].estimated_duration for task_id in batch) if batch else 0.0
            )
        ...
        plan.estimated_total_cost = sum(
            self.heuristic.estimate_cost(task, plan.context) for task in tasks
        )
        ...
        total_benefit = sum(self.heuristic.estimate_benefit(task, plan.context) for task in tasks)
    ...
    async def plan_workflow_execution(
        self, workflow_def: Dict[str, Any], context: Dict[str, Any]
    ) -> List[List[str]]:
        """
        ...
        try:
            ...
            execution_order = await self._create_execution_order(tasks, lambda t: t.priority.value)
    ...
    async def adapt_plan(
        self, plan_id: str, execution_feedback: Dict[str, Any], context_changes: Dict[str, Any]
    ) -> ExecutionPlan:
        """
