...
from src.core.events.event_types import (
    UncertaintyQuantified, UncertaintyThresholdExceeded, UncertaintyModelUpdated,
    UncertaintyCalibrated, ConfidenceScoreCalculated, ProbabilisticDecisionMade,
    UncertaintyLearningOccurred, UncertaintyAnomalyDetected, ErrorOccurred,
    SystemStateChanged, ComponentHealthChanged, LearningEventOccurred
)
...
class UncertaintyQuantifier(ABC):
    ...
    async def quantify(self, 
                      ...
                      context: UncertaintyContext) -> UncertaintyEstimate:
    ...
    async def calibrate(self, 
                       validation_data: List[Tuple[Any, Any, float]]) -> Dict[str, float]:
    ...
    async def quantify(self, 
        ...
        try:
            ...
            if not isinstance(model_predictions, list):
                ...
                ensemble_preds = [
                    base_pred + np.random.normal(0, 0.1) 
...
class InformationTheoreticQuantifier(UncertaintyQuantifier):
    ...
    async def quantify(self, 
        ...
        try:
            ...
            # Calculate entropy
            entropy = -np.sum(probs * np.log(probs + 1e-12))  # Add small epsilon for numerical stability
            ...
            raise UncertaintyError(f"Information-theoretic uncertainty quantification failed: {str(e)}")
...
class UncertaintyAggregator:
    ...
    async def aggregate(self, 
                       ...
                       weights: Optional[List[float]] = None) -> UncertaintyEstimate:
    ...
    async def _maximum_uncertainty(self, 
                                  estimates: List[UncertaintyEstimate],
                                  weights: Optional[List[float]] = None) -> UncertaintyEstimate:
    ...
    async def _minimum_uncertainty(self, 
                                  estimates: List[UncertaintyEstimate],
                                  weights: Optional[List[float]] = None) -> UncertaintyEstimate:
...
class EnhancedUncertaintyHandler:
    ...
    def __init__(self, container: Container):
        ...
        # Configuration
        self.default_confidence_level = self.config.get("uncertainty.default_confidence_level", 0.95)
    ...
    def _setup_quantifiers(self) -> None:
        ...
        try:
            ...
            self.quantifiers[UncertaintyMethod.INFORMATION_THEORY] = InformationTheoreticQuantifier()
    ...
    async def quantify_uncertainty(self,
                                  ...
                                  uncertainty_types: Optional[List[UncertaintyType]] = None) -> List[UncertaintyEstimate]:
        """
        ...
        try:
            with self.tracer.trace("uncertainty_quantification") as span:
                ...
                for method in methods:
                    ...
                    supported_types = [
                        utype for utype in uncertainty_types 
                    ...
                    try:
                        ...
                        await self.event_bus.emit(UncertaintyQuantified(
                        ...
                        self.logger.error(f"Uncertainty quantification failed for method {method}: {str(e)}")
                        continue
    ...
    async def _apply_calibration(self, estimate: UncertaintyEstimate, method: UncertaintyMethod) -> None:
    ...
    def _generate_cache_key(self, input_data: Any, model_predictions: Any, methods: List[UncertaintyMethod]) -> str:
    ...
    async def _check_uncertainty_thresholds(self, 
                                           estimates: List[UncertaintyEstimate],
                                           context: UncertaintyContext) -> None:
        ...
        for estimate in estimates:
            ...
            if estimate.std_deviation > threshold:
                await self.event_bus.emit(UncertaintyThresholdExceeded(
                    estimate_id=estimate.estimate_id,
                    threshold=threshold,
    ...
    @handle_exceptions
    async def aggregate_uncertainties(self,
                                     estimates: List[UncertaintyEstimate],
                                     strategy: str = "weighted_average",
                                     weights: Optional[List[float]] = None) -> UncertaintyEstimate:
        """
    ...
    async def calculate_confidence_score(self,
                                        estimate: UncertaintyEstimate,
                                        context: Optional[UncertaintyContext] = None) -> float:
        """
        ...
        try:
            ...
            if context:
                ...
                importance_factor = 1.0 - (context.decision_importance * 0.1)  # Slight penalty for important decisions
                confidence *= importance_factor
                
                # Adjust for time pressure
                time_factor = 1.0 - (context.time_pressure * 0.05)  # Slight penalty for time pressure
            ...
            await self.event_bus.emit(ConfidenceScoreCalculated(
                estimate_id=estimate.estimate_id,
    ...
    @handle_exceptions
    async def make_probabilistic_decision(self,
                                         alternatives: List[Dict[str, Any]],
                                         uncertainty_estimates: List[UncertaintyEstimate],
                                         context: Optional[UncertaintyContext] = None,
                                         decision_criteria: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            if len(alternatives) != len(uncertainty_estimates):
                raise UncertaintyError("Number of alternatives must match number of uncertainty estimates")
            ...
            for i, (alternative, estimate) in enumerate(zip(alternatives, uncertainty_estimates)):
                ...
                # Uncertainty penalty
                uncertainty_penalty = estimate.std_deviation * decision_criteria["uncertainty_penalty"]
            ...
            exp_scores = [math.exp(score - max_score) for score in scores]  # Subtract max for numerical stability
            ...
            await self.event_bus.emit(ProbabilisticDecisionMade(
    ...
    @handle_exceptions
    async def update_uncertainty_model(self,
        ...
        try:
            ...
            await self.event_bus.emit(UncertaintyModelUpdated(
                estimate_id=estimate_id,
    ...
    async def _learn_from_feedback(self,
        ...
        try:
            ...
            # Emit learning event
            await self.event_bus.emit(UncertaintyLearningOccurred(
                estimate_id=estimate.estimate_id,
    ...
    async def detect_uncertainty_anomalies(self,
                                          estimates: List[UncertaintyEstimate],
                                          context: Optional[UncertaintyContext] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for estimate in estimates:
                ...
                if anomaly_reasons:
                    ...
                    await self.event_bus.emit(UncertaintyAnomalyDetected(
                        estimate_id=estimate.estimate_id,
    ...
    def get_uncertainty_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_estimation_time = (
                ...
                if self.estimation_times else 0.0
            )
            ...
            try:
                if self.enable_calibration and len(self.calibration_history) > 0:
                    ...
                    for method in self.quantifiers.keys():
                        ...
                        await self.event_bus.emit(UncertaintyCalibrated(
                            method=method.value,
            ...
            try:
                if self.enable_learning and self.accuracy_history:
                    ...
                    learning_data = {
                        ...
                        "calibration_history": list(self.calibration_history)[-100:],  # Last 100 entries
                        "timestamp": datetime.now(timezone.utc)
                    }
