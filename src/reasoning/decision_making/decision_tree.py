...
from src.core.events.event_types import (
    DecisionStarted, DecisionCompleted, DecisionFailed, DecisionLearned,
    DecisionTreeAdapted, DecisionCriteriaUpdated, DecisionOutcomeRecorded,
    WorkflowDecisionMade, UserPreferenceInfluencedDecision, SystemDecisionMade,
...
class DecisionError(Exception):
    ...
    def __init__(self, message: str, decision_id: Optional[str] = None, 
                 tree_id: Optional[str] = None, node_id: Optional[str] = None):
        ...
        for option in options:
            ...
            for criterion in criteria:
                ...
                value = context.get(f"{criterion.criterion_id}_value", 
                                  option.metadata.get(criterion.criterion_id))
...
class ProbabilisticEvaluator(DecisionEvaluator):
    ...
    async def _calculate_likelihood(
        ...
        try:
            ...
            self.logger.warning(f"Error calculating likelihood for {criterion.criterion_id}: {str(e)}")
            return 0.1
...
class MLBasedEvaluator(DecisionEvaluator):
    ...
    def _extract_features(self, criteria: List[DecisionCriterion], context: Dict[str, Any]) -> Dict[str, float]:
    ...
    def _extract_option_features(self, option: DecisionOption, criteria: List[DecisionCriterion]) -> Dict[str, float]:
...
class EnhancedDecisionTree:
        ...
        async with self.decision_semaphore:
            ...
            try:
                with self.tracer.trace("decision_making") as span:
                    ...
                    await self.event_bus.emit(DecisionStarted(
                        decision_id=decision_id,
                        tree_id=tree_id,
                    ...
                    await self.event_bus.emit(DecisionCompleted(
                        decision_id=decision_id,
                        tree_id=tree_id,
                        selected_option=result.selected_option.option_id if result.selected_option else None,
                ...
                await self.event_bus.emit(DecisionFailed(
                    decision_id=decision_id,
                    tree_id=tree_id,
                    error_message=str(e),
    ...
    async def _navigate_tree(
        ...
        while current_node_id and depth < max_depth:
            ...
            if node.node_type == "decision":
                ...
                result = await self._evaluate_decision_node(node, context, decision_id, tree.tree_id)
    ...
    async def _evaluate_decision_node(
        ...
        try:
            ...
            if best_option:
                ...
                result.reasoning_trace.append(
                    f"Utility score: {best_option.utility_score:.2f}"
                )
                
                for criterion_id, score in criteria_evaluations.items():
                    result.reasoning_trace.append(
                        f"Criterion '{criterion_id}': {score:.2f}"
                    )
    ...
    async def provide_feedback(
        ...
        try:
            ...
            # Emit feedback event
            await self.event_bus.emit(DecisionLearned(
                decision_id=decision_id,
            ...
            if correctness and quality_score > 0.7:
                ...
                for criterion in node.criteria:
                    if criterion.criterion_id in decision_result.criteria_evaluations:
                        ...
                        criterion.weight = min(1.0, criterion.weight * (1 + tree.adaptation_threshold))
                ...
                for criterion in node.criteria:
                    if criterion.criterion_id in decision_result.criteria_evaluations:
                        ...
                        criterion.weight = max(0.1, criterion.weight * (1 - tree.adaptation_threshold))
            
            # Update tree metadata
            tree.updated_at = datetime.now(timezone.utc)
            
            # Emit adaptation event
            await self.event_bus.emit(DecisionTreeAdapted(
                tree_id=tree.tree_id,
                node_id=node.node_id,
    ...
    def get_decision_history(
        ...
        for decision in reversed(self.decision_history):
            ...
            filtered_decisions.append({
                ...
                'selected_option': decision.selected_option.option_id if decision.selected_option else None,
