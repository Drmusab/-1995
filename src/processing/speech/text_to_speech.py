...
class VoiceManager:
    ...
    def _create_voice_profile_from_catalog(
        self, voice_key: str, voice_info: Dict[str, Any]
    ) -> VoiceProfile:
        ...
        try:
            ...
            for model_file in model_files:
                ...
                if config_file.exists():
                    try:
                        voice_profile = self._create_voice_profile_from_files(
    ...
    def _create_voice_profile_from_files(
        self, model_file: Path, config_file: Path
    ...
    async def _download_voice(self, voice_id: str) -> None:
        ...
        try:
            ...
            model_path, config_path = ensure_voice_exists(
                voice_id, self.voice_catalog, str(self.model_dir)
            )
...
class ProsodyProcessor:
    ...
    def apply_prosody_to_synthesis(
        self, voice: PiperVoice, prosody: ProsodySettings
    ) -> Dict[str, Any]:
...
class EnhancedTextToSpeech:
    ...
    def __init__(self, container: Container):
        ...
        self.health_check.register_component("text_to_speech", self._health_check_callback)
    ...
    def _setup_device(self) -> None:
        ...
        self.device = torch.device(
            "cuda"
            if torch.cuda.is_available() and self.config.get("text_to_speech.device.use_gpu", True)
            else "cpu"
        )
    ...
    def _setup_models_and_voices(self) -> None:
        ...
        # Model directory
        self.model_dir = Path(
            self.config.get("text_to_speech.models.model_dir", "data/models/text_to_speech")
        )
        self.model_dir.mkdir(parents=True, exist_ok=True)
        ...
        self.default_voice_id = self.config.get(
            "text_to_speech.default_voice", self.DEFAULT_VOICE_ID
        )
        self.voice_selection_strategy = self.config.get(
            "text_to_speech.voice_selection", "quality_first"
        )
    ...
    def _setup_audio_processing(self) -> None:
        ...
        self.audio_processor = EnhancedAudioProcessor(
            sample_rate=self.default_sample_rate, container=self.container
        )

        # Post-processing settings
        self.enable_audio_enhancement = self.config.get(
            "text_to_speech.post_processing.enhancement", True
        )
        self.enable_normalization = self.config.get(
            "text_to_speech.post_processing.normalization", True
        )
    ...
    def _setup_learning(self) -> None:
        ...
        self.personalization_enabled = self.config.get(
            "text_to_speech.learning.personalization", True
        )
    ...
    def _setup_streaming(self) -> None:
        ...
        self.default_streaming_config = StreamingConfig(
            ...
            enable_sentence_streaming=self.config.get(
                "text_to_speech.streaming.sentence_streaming", True
            ),
        )
        ...
        self.stream_thread_pool = ThreadPoolExecutor(
            max_workers=4, thread_name_prefix="tts_streaming"
        )
    ...
    @handle_exceptions
    async def synthesize(self, request: Union[str, SynthesisRequest]) -> SynthesisResult:
        """
        ...
        try:
            with self.tracer.trace("tts_synthesis") as span:
                ...
                if request.cache_result and self.cache_enabled:
                    ...
                    if cached_result:
                        self.logger.info(
                            f"Returning cached synthesis result for session {request.session_id}"
                        )
    ...
    async def _select_and_load_voice(
        self, request: SynthesisRequest
    ) -> tuple[VoiceProfile, PiperVoice]:
    ...
    def _voice_quality_score(self, voice_profile: VoiceProfile) -> float:
        ...
        quality_scores = {"low": 0.3, "medium": 0.7, "high": 1.0, "x_low": 0.1, "x_high": 1.2}
    ...
    async def _analyze_and_setup_prosody(
        self, text: str, request: SynthesisRequest
    ) -> ProsodySettings:
    ...
    def _apply_speaking_style(
        self, prosody: ProsodySettings, style: SpeakingStyle
    ) -> ProsodySettings:
        ...
        style_adjustments = {
            ...
            SpeakingStyle.SAD: {"pitch_base": 0.8, "speech_rate": 0.9, "emphasis_strength": 0.7},
            SpeakingStyle.ANGRY: {"pitch_base": 1.1, "speech_rate": 1.2, "emphasis_strength": 1.4},
            SpeakingStyle.EXCITED: {
                "pitch_base": 1.3,
                "speech_rate": 1.3,
                "emphasis_strength": 1.5,
            },
            ...
            SpeakingStyle.CALM: {"pitch_base": 1.0, "speech_rate": 0.95, "emphasis_strength": 0.8},
    ...
    async def _synthesize_batch(
        self, text: str, voice_model: PiperVoice, synthesis_params: Dict[str, Any]
    ) -> np.ndarray:
    ...
    async def _postprocess_audio(
        self, audio_data: np.ndarray, request: SynthesisRequest
    ) -> np.ndarray:
    ...
    async def _apply_audio_effects(
        self, audio: np.ndarray, request: SynthesisRequest
    ) -> np.ndarray:
    ...
    async def _assess_synthesis_quality(
        self, audio: np.ndarray, text: str, voice_profile: VoiceProfile
    ) -> Dict[str, float]:
        ...
        try:
            ...
            spectral_centroid = np.mean(
                librosa.feature.spectral_centroid(y=audio, sr=voice_profile.sample_rate)
            )
            ...
            metrics["overall_quality"] = np.mean(
                [metrics["audio_quality"], metrics["naturalness"], metrics["intelligibility"]]
            )
    ...
    async def _save_audio_to_file(
        self, audio: np.ndarray, metadata: AudioMetadata, request: SynthesisRequest
    ) -> Optional[Path]:
    ...
    async def synthesize_streaming(
        ...
        if isinstance(request, str):
            request = SynthesisRequest(text=request, synthesis_mode=SynthesisMode.STREAMING)
        else:
