...
class BaseFusionStrategy(ABC):
    ...
    async def fuse_modalities(
        self, modalities: Dict[ModalityType, ModalityData], context: FusionContext
    ) -> FusionResult:
    ...
    async def fuse_modalities(
        ...
        try:
            ...
            return FusionResult(
                ...
                modality_contributions={mod: data.confidence for mod, data in modalities.items()},
...
class LateFusionStrategy(BaseFusionStrategy):
    ...
    def _process_modality_decision(
        self, modality_type: ModalityType, data: ModalityData
    ) -> Dict[str, Any]:
    ...
    def _weighted_decision_fusion(
        ...
        normalized_weights = (
            {mod: conf / total_confidence for mod, conf in confidences.items()}
        ...
        try:
            ...
            return FusionResult(
                ...
                modality_contributions={
                    mod: np.mean(attention_weights_np.get(mod, [0])) for mod in modalities.keys()
                },
                request_id=context.request_id,
                session_id=context.session_id,
            )
    ...
    async def fuse_modalities(
        ...
        try:
            ...
            if len(self.performance_history[selected_strategy_type]) > max_history:
                self.performance_history[selected_strategy_type] = self.performance_history[
    ...
    def __init__(self, input_dims: Dict[ModalityType, int], output_dim: int):
        ...
        for modality, dim in input_dims.items():
            self.projections[modality.value] = nn.Sequential(
                nn.Linear(dim, output_dim), nn.ReLU(), nn.Dropout(0.1)
            )
    ...
    def __init__(self, modality_dims: Dict[ModalityType, int], attention_dim: int):
        ...
        for modality, dim in modality_dims.items():
            self.attention_layers[modality.value] = nn.Sequential(
                nn.Linear(dim, attention_dim), nn.Tanh(), nn.Linear(attention_dim, 1)
            )
    ...
    def __init__(self, modality_dims: Dict[ModalityType, int]):
        ...
        for modality, dim in modality_dims.items():
            self.alignment_projections[modality.value] = nn.Sequential(
                nn.Linear(dim, 256), nn.ReLU(), nn.Linear(256, 128)
            )
...
class StrategySelector:
    ...
    def _estimate_complexity(self, modalities: Dict[ModalityType, ModalityData]) -> str:
        ...
        total_complexity = sum(complexity_scores.get(mod, 1) for mod in modalities.keys())
    ...
    def _select_best_performing_strategy(
        self, performance_history: Dict[FusionStrategy, List[Dict[str, Any]]]
...
class MultimodalFusionStrategy:
    ...
    async def _start_background_tasks(self) -> None:
        ...
        self._background_tasks.append(asyncio.create_task(self._strategy_optimization_loop()))
            ...
            with self.tracer.trace("multimodal_fusion") if self.tracer else None:
                ...
                if self.metrics:
                    ...
                    self.metrics.increment(
                        "fusion_strategy_usage", tags={"strategy": selected_strategy.value}
                    )
    ...
    def _create_fusion_context(
        self, context: Optional[Dict[str, Any]], weights: Optional[Dict[str, float]]
    ) -> FusionContext:
    ...
    def _convert_modalities(self, modalities: Dict[str, Any]) -> Dict[ModalityType, ModalityData]:
        ...
        for modality_str, data in modalities.items():
            try:
                ...
                features, embeddings, confidence, quality, metadata = self._extract_modality_info(
                    data
                )
