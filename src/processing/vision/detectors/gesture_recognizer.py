...
class GestureRecognitionError(Exception):
    ...
    def __init__(
        self, message: str, gesture_id: Optional[str] = None, error_code: Optional[str] = None
    ):
...
class GestureClassifier:
    ...
    def classify_static_gestures(
        self, landmarks: List[HandLandmarks]
    ) -> List[Tuple[GestureType, float]]:
    ...
    def _classify_hand_shape(self, hand: HandLandmarks) -> Tuple[GestureType, float]:
        ...
        try:
            ...
            # Check for thumbs up
            if (
                thumb_tip[1] < thumb_ip[1]  # thumb pointing up
                and index_tip[1] > index_pip[1]  # index folded
                and middle_tip[1] > middle_pip[1]  # middle folded
                and ring_tip[1] > ring_pip[1]  # ring folded
            ...
            if (
                thumb_tip[1] > thumb_ip[1]  # thumb pointing down
                and index_tip[1] > index_pip[1]  # index folded
                and middle_tip[1] > middle_pip[1]  # middle folded
                and ring_tip[1] > ring_pip[1]  # ring folded
                and pinky_tip[1] > pinky_pip[1]
            ...
            if (
                index_tip[1] < index_pip[1]  # index extended
                and middle_tip[1] < middle_pip[1]  # middle extended
                and ring_tip[1] > ring_pip[1]  # ring folded
                and pinky_tip[1] > pinky_pip[1]
            ):  # pinky folded
            ...
            if (
                thumb_index_distance < 0.05  # thumb and index close
                and middle_tip[1] < middle_pip[1]  # middle extended
                and ring_tip[1] < ring_pip[1]  # ring extended
                and pinky_tip[1] < pinky_pip[1]
            ):  # pinky extended
...
class TemporalGestureAnalyzer:
    ...
    def add_frame_data(self, landmarks: Dict[str, Any], timestamp: datetime):
        ...
        self.gesture_buffer.append({"landmarks": landmarks, "timestamp": timestamp})
        ...
        if "hands" in landmarks and landmarks["hands"]:
            ...
            self.trajectory_buffer.append({"center": center, "timestamp": timestamp})
...
class EnhancedGestureRecognizer:
    ...
    def __init__(self, container: Container):
        ...
        self.thread_pool = ThreadPoolExecutor(
            max_workers=4, thread_name_prefix="gesture_recognizer"
        )
    ...
    async def _load_gesture_models(self) -> None:
        ...
        try:
            ...
            model_path = Path(
                self.config.get("gesture_recognition.model_path", "data/models/gestures/")
            )
    ...
    async def _start_background_tasks(self) -> None:
        ...
        if self.gesture_config.enable_learning:
            self._background_tasks.append(asyncio.create_task(self._model_adaptation_loop()))
        ...
        async with self._processing_lock:
            try:
                with self.tracer.trace("gesture_recognition") if self.tracer else None:
                    self.logger.info(
                        f"Starting gesture recognition for request {request.request_id}"
                    )
    ...
    async def _recognize_from_image(self, request: GestureRequest) -> GestureResult:
        ...
        try:
            ...
            landmarks_data = await asyncio.get_event_loop().run_in_executor(
                self.thread_pool, self.mediapipe_processor.process_frame, request.image
            )
    ...
    async def _recognize_from_video_frames(self, request: GestureRequest) -> GestureResult:
        ...
        try:
            ...
            for i, frame in enumerate(request.video_frames):
                ...
                # Process frame
                landmarks_data = await asyncio.get_event_loop().run_in_executor(
                    self.thread_pool, self.mediapipe_processor.process_frame, frame
                )
            ...
            if all_landmarks:
                ...
                if (
                    swipe_direction
                    and swipe_confidence >= self.gesture_config.min_confidence_threshold
                    and swipe_confidence > result.confidence
                ):
                ...
                if result.trajectory:
                    ...
                    result.acceleration = self._calculate_acceleration(
                        result.trajectory, timestamps
                    )
                    result.direction = self._calculate_direction(result.trajectory)

                # Use best frame for landmarks
                best_frame_landmarks = max(all_landmarks, key=lambda x: len(x.get("hands", [])))

                if "hands" in best_frame_landmarks:
                    result.hand_landmarks = best_frame_landmarks["hands"]
                    result.bounding_box = self._calculate_bounding_box(
                        best_frame_landmarks["hands"]
                    )
                    result.center_point = self._calculate_center_point(
                        best_frame_landmarks["hands"]
                    )
    ...
    def _calculate_velocity(
        self, trajectory: List[Tuple[float, float]], timestamps: List[datetime]
    ) -> Optional[float]:
    ...
    async def _cache_cleanup_loop(self) -> None:
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                expired_keys = [
                    key
                    for key, value in self._gesture_cache.items()
    ...
    async def _handle_session_ended(self, event) -> None:
        ...
        session_recognitions = [
            req_id
            for req_id, recognition in self._active_recognitions.items()
