...
class OCREngine(ABC):
    ...
    async def extract_text(
        self, image: np.ndarray, config: Optional[OCRConfiguration] = None
    ) -> OCRResult:
...
class EasyOCREngine(OCREngine):
    ...
    async def extract_text(
        ...
        try:
            ...
            overall_confidence = (
                np.mean([r.confidence for r in text_regions]) if text_regions else 0.0
            )
...
class ImagePreprocessor:
    ...
    def preprocess_image(
        self, image: np.ndarray, config: OCRConfiguration
...
class EnhancedOCREngine:
    ...
    def _load_configuration(self) -> None:
        ...
        try:
            self.ocr_config = OCRConfiguration(
                ...
                fallback_methods=[
                    OCRMethod(m) for m in self.config.get("ocr.fallback_methods", ["easyocr"])
                ],
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            await self.event_bus.emit(
                ComponentInitialized(
                    component_id="ocr_engine", initialization_time=0.0  # Would track actual time
                )
            )
            ...
            await self.event_bus.emit(
                ComponentFailed(
                    component_id="ocr_engine", error_message=str(e), error_type=type(e).__name__
                )
            )
            raise
    ...
    def _select_best_method(self, config: OCRConfiguration, image: np.ndarray) -> OCRMethod:
    ...
    async def _extract_with_method(
        self, method: OCRMethod, image: np.ndarray, config: OCRConfiguration, request_id: str
    ) -> OCRResult:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                engine.extract_text(image, config), timeout=config.timeout_seconds
            )
            ...
            raise OCRError(
                f"OCR extraction timed out after {config.timeout_seconds}s",
                "EXTRACTION_TIMEOUT",
                method.value,
            )
        except Exception as e:
            raise OCRError(
                f"OCR extraction failed with {method.value}: {str(e)}",
                "EXTRACTION_FAILED",
                method.value,
            )
    ...
    def _deserialize_document_layout(self, layout_data: Dict[str, Any]) -> "DocumentLayout":
        ...
        from src.processing.vision.layout_analyzer import BoundingBox, DocumentLayout, TextBlock
    ...
    async def _store_extraction_for_learning(
        self, result: OCRResult, config: OCRConfiguration, session_id: Optional[str]
    ) -> None:
        """Store extraction result for learning and improvement."""
        try:
            if self.feedback_processor:
                await self.feedback_processor.process_ocr_extraction(result, config, session_id)
        except Exception as e:
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"service": "ocr_engine", "status": "unhealthy", "error": str(e)}
