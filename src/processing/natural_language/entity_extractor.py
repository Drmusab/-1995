...
from typing import Dict, List, Set, Any, Optional, Union, Tuple, AsyncGenerator
...
class SpacyEntityExtractor(EntityExtractor):
    ...
    def get_supported_entity_types(self) -> Set[EntityType]:
        ...
        return set(self.spacy_to_entity_type.values()) | {
            ...
            EntityType.HASHTAG, EntityType.MENTION
        }
...
class RelationshipExtractor:
    ...
    async def extract_relationships(self, entities: List[Entity], text: str) -> List[Dict[str, Any]]:
    ...
    async def _analyze_entity_pair(self, entity1: Entity, entity2: Entity, text: str) -> Optional[Dict[str, Any]]:
    ...
    def _detect_relationship_type(self, entity1: Entity, entity2: Entity, context: str) -> Optional[str]:
        ...
        if entity1.entity_type == EntityType.PERSON and entity2.entity_type == EntityType.ORGANIZATION:
            if any(word in context_lower for word in ["works at", "employed by", "ceo of", "founder of"]):
                return "works_at"
        
        elif entity1.entity_type == EntityType.PERSON and entity2.entity_type == EntityType.LOCATION:
            ...
            if any(word in context_lower for word in ["based in", "headquartered in", "located in"]):
                return "headquarters"
...
class EnhancedEntityExtractor:
    ...
    def _load_configuration(self) -> None:
        ...
        self.max_entities_per_request = self.config.get("entity_extraction.max_entities_per_request", 100)
    ...
    async def _initialize_extractors(self) -> None:
        ...
        if self.config.get("entity_extraction.enable_spacy", True):
            ...
            self.extractors[ExtractionStrategy.SPACY] = SpacyEntityExtractor(spacy_model, self.logger)
    ...
    async def extract_entities(self, request: ExtractionRequest) -> ExtractionResult:
        ...
        try:
            with self.tracer.trace("entity_extraction") if self.tracer else None:
                ...
                strategy = request.strategy if request.strategy != ExtractionStrategy.HYBRID else self.default_strategy
                if strategy not in self.extractors:
                    strategy = ExtractionStrategy.HYBRID if ExtractionStrategy.HYBRID in self.extractors else list(self.extractors.keys())[0]
                ...
                # Create result
                result = await self._create_extraction_result(entities, request, start_time, strategy)
    ...
    async def _post_process_entities(self, entities: List[Entity], request: ExtractionRequest) -> List[Entity]:
    ...
    async def _create_extraction_result(
        ...
        if request.enable_relationship_extraction and self.relationship_extractor:
            relationships = await self.relationship_extractor.extract_relationships(entities, request.text)
        ...
        text_stats = {
            ...
            "entity_density": len(entities) / len(request.text.split()) if request.text.split() else 0.0
        }
