...
from src.core.events.event_types import (
    ContextUpdated,
    IntentCategoryUpdated,
    IntentClassified,
    IntentConfidenceChanged,
    IntentDetected,
    IntentLearned,
    IntentRoutingCompleted,
...
class IntentError(Exception):
    ...
    def __init__(
        self, message: str, intent_name: Optional[str] = None, error_code: Optional[str] = None
    ):
...
class PatternBasedDetector(IntentDetector):
    ...
    async def _calculate_pattern_confidence(
        self, text: str, intent_def: IntentDefinition, context: Dict[str, Any]
    ) -> float:
    ...
    async def _calculate_context_score(
        self, intent_def: IntentDefinition, context: Dict[str, Any]
    ) -> float:
...
class MLBasedDetector(IntentDetector):
    ...
    async def detect_intent(
        ...
        try:
            ...
            model_input = {"text": text, "context": context, "task": "intent_classification"}
    ...
    async def _parse_ml_results(
        self, prediction: Dict[str, Any], text: str, context: Dict[str, Any]
    ) -> List[IntentDetectionResult]:
    ...
    async def train(self, training_data: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Successfully trained ML intent detector with {len(training_data)} samples"
            )
        except Exception as e:
...
class HybridDetector(IntentDetector):
    ...
    async def detect_intent(
        ...
        filtered_results = [
            r for r in combined_results if r.confidence >= self.confidence_threshold
        ]
    ...
    async def _combine_results(
        self, pattern_results: List[IntentDetectionResult], ml_results: List[IntentDetectionResult]
    ) -> List[IntentDetectionResult]:
    ...
    async def train(self, training_data: List[Dict[str, Any]]) -> None:
        ...
        await asyncio.gather(
            self.pattern_detector.train(training_data), self.ml_detector.train(training_data)
        )
...
class IntentRouter:
    ...
    async def route_intent(
        self, intent_result: IntentDetectionResult, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "intent": intent_result.intent_name}
...
class IntentLearner:
    ...
    def __init__(
        self, feedback_processor: FeedbackProcessor, preference_learner: PreferenceLearner
    ):
    ...
    async def learn_from_interaction(
        self,
        text: str,
        detected_intent: IntentDetectionResult,
    ...
    async def _process_correction(self, interaction_data: Dict[str, Any]) -> None:
        ...
        self.logger.info(
            f"Processed intent correction: {interaction_data['detected_intent']} -> {interaction_data['actual_intent']}"
        )
    ...
    async def get_learning_insights(self) -> Dict[str, Any]:
        ...
        accuracy = (
            (total_interactions - corrections) / total_interactions if total_interactions > 0 else 0
        )
...
class EnhancedIntentManager:
            ...
            try:
                ...
                if self._enable_learning:
                    self.intent_learner = IntentLearner(
                        self.feedback_processor, self.preference_learner
                    )
                ...
                self.logger.info(
                    f"Intent manager initialized with {len(self.intent_definitions)} intent definitions"
                )
    ...
    def _parse_intent_definition(
        self, intent_name: str, config: Dict[str, Any]
    ) -> IntentDefinition:
    ...
    async def _start_background_tasks(self) -> None:
        ...
        self._background_tasks.append(asyncio.create_task(self._performance_monitoring_loop()))

        # Learning optimization
        if self._enable_learning:
            self._background_tasks.append(asyncio.create_task(self._learning_optimization_loop()))

        # Intent definition updates
        self._background_tasks.append(asyncio.create_task(self._intent_update_loop()))
    ...
    async def detect_intent(
        ...
        try:
            with self.tracer.trace("intent_detection") if self.tracer else None:
                ...
                # Filter by confidence threshold
                filtered_results = [
                    r for r in results if r.confidence >= self._confidence_threshold
                ]
                ...
                if self.metrics:
                    ...
                    self.metrics.record(
                        "intent_detection_duration_seconds", time.time() - start_time
                    )
                    if filtered_results:
                        self.metrics.record(
                            "intent_confidence_score", filtered_results[0].confidence
                        )
    ...
    @handle_exceptions
    async def route_intent(
        self, intent_result: IntentDetectionResult, context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def register_intent(self, intent_definition: IntentDefinition) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to register intent {intent_definition.intent_name}: {str(e)}"
            )
    ...
    async def _update_detectors(self) -> None:
        ...
        if "hybrid" in self.intent_detectors:
            hybrid_detector = HybridDetector(
                self.intent_detectors["pattern"], self.intent_detectors["ml"]
            )
            self.intent_detectors["hybrid"] = hybrid_detector
    ...
    async def get_intent_analytics(self) -> Dict[str, Any]:
        ...
        successful_detections = sum(
            1 for record in self._detection_history if record["detected_intents"]
        )

        avg_processing_time = np.mean(
            [record["processing_time"] for record in self._detection_history]
        )
        ...
        return {
            ...
            "most_frequent_intents": dict(
                sorted(intent_frequency.items(), key=lambda x: x[1], reverse=True)[:10]
            ),
    ...
    def list_intents(
        self, category: Optional[IntentCategory] = None, intent_type: Optional[IntentType] = None
    ) -> List[Dict[str, Any]]:
            ...
            try:
                ...
                if self._detection_history:
                    ...
                    if recent_detections:
                        ...
                        success_rate = sum(
                            1 for r in recent_detections if r["detected_intents"]
        ...
        while True:
            try:
                ...
                if self.intent_learner:
                    ...
                    self.logger.debug(
                        f"Updated confidence threshold to {self._confidence_threshold}"
                    )
    ...
    async def _health_check_callback(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": (
                    "healthy"
                    if all(h == "healthy" for h in detector_health.values())
                    else "degraded"
                ),
