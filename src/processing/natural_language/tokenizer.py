...
from typing import (
    ...
    AsyncGenerator, Callable, NamedTuple
)
import threading
import weakref
import hashlib
...
from transformers import (
    ...
    BertTokenizer, GPT2Tokenizer, T5Tokenizer, XLMRobertaTokenizer
)
...
from src.core.events.event_types import (
    ComponentRegistered, ComponentInitialized, ComponentFailed,
    TokenizationCompleted, TokenizationFailed, CacheHit, CacheMiss
)
...
class TokenizationError(Exception):
    ...
    def __init__(self, message: str, strategy: Optional[TokenizationStrategy] = None, 
...
class TransformerTokenizer(BaseTokenizer):
    ...
    async def tokenize(self, text: str, **kwargs) -> TokenizationResult:
        ...
        try:
            ...
            special_token_count = sum(1 for token in token_strings 
...
class TikTokenTokenizer(BaseTokenizer):
    ...
    async def tokenize(self, text: str, **kwargs) -> TokenizationResult:
        ...
        try:
            ...
            raise TokenizationError(f"TikToken tokenization failed: {str(e)}", 
    ...
    async def detokenize(self, tokens: Union[List[str], List[int]], **kwargs) -> str:
        ...
        try:
            ...
            raise TokenizationError(f"TikToken detokenization failed: {str(e)}", 
                                  TokenizationStrategy.TIKTOKEN)
...
class CustomTokenizer(BaseTokenizer):
    ...
    async def tokenize(self, text: str, **kwargs) -> TokenizationResult:
        ...
        try:
            ...
            raise TokenizationError(f"Custom tokenization failed: {str(e)}", 
                                  TokenizationStrategy.CUSTOM)
    ...
    async def detokenize(self, tokens: Union[List[str], List[int]], **kwargs) -> str:
        ...
        try:
            ...
            raise TokenizationError(f"Custom detokenization failed: {str(e)}", 
                                  TokenizationStrategy.CUSTOM)
    ...
    def preprocess(self, text: str) -> str:
        ...
        if self.config.normalization in [TextNormalization.UNICODE, TextNormalization.AGGRESSIVE]:
            ...
            if self.config.strip_accents:
                processed_text = ''.join(
                    char for char in unicodedata.normalize('NFD', processed_text)
...
class TokenizerCache:
    ...
    def __init__(self, cache_strategy: Optional[CacheStrategy] = None, 
    ...
    async def set(self, text: str, config: TokenizerConfig, 
...
class EnhancedTokenizer:
        ...
        for strategy in default_strategies:
            try:
                config = TokenizerConfig(
                    strategy=strategy,
                    model_name=self._get_default_model_for_strategy(strategy)
                )
        ...
        **kwargs
        ...
        with self.tracer.trace("tokenization") if self.tracer else None:
            try:
                ...
                await self.event_bus.emit(TokenizationCompleted(
                ...
                # Emit failure event
                await self.event_bus.emit(TokenizationFailed(
                    text_length=len(text),
                    error_message=str(e),
                ...
                raise TokenizationError(f"Tokenization failed: {str(e)}", 
                                      config.strategy if config else None)
    ...
    def _get_supported_models(self, strategy: TokenizationStrategy) -> List[str]:
        ...
        models = {
            ...
            TokenizationStrategy.SUBWORD_SENTENCEPIECE: [
                "t5-small", "t5-base", "t5-large", "xlm-roberta-base"
            ],
            TokenizationStrategy.TIKTOKEN: [
                "gpt-3.5-turbo", "gpt-4", "text-davinci-003", "text-ada-001"
            ]
        }
