...
from src.core.security.sanitization import (
    ...
    ContentType, SecurityLevel, SanitizationResult
)
...
class SanitizedNLPProcessor:
    ...
    def __init__(self, container: Container):
        ...
        self.malicious_patterns = {
            ...
            'sql_injection': re.compile(r'\b(union|select|insert|update|delete|drop|create|alter|exec|execute)\b', re.IGNORECASE),
            'command_injection': re.compile(r'[;&|`$]|\$\(|\$\{|<\(|>\('),
            'path_traversal': re.compile(r'\.\.[/\\]'),
            'xss_attempt': re.compile(r'(javascript:|onerror=|onload=|onclick=|onmouseover=)', re.IGNORECASE),
    ...
    async def process_text(
        ...
        try:
            ...
            sanitized_text = await self._nlp_specific_sanitization(
                sanitized_text,
                context
            )
            
            # Process with intent detection
            intent_result = await self._safe_intent_detection(
                sanitized_text,
                session_id
            )
            
            # Extract entities with sanitization
            entity_result = await self._safe_entity_extraction(
                sanitized_text,
                session_id
            )
            
            # Analyze sentiment safely
            sentiment_result = await self._safe_sentiment_analysis(
                sanitized_text,
                session_id
            )
    ...
    async def _nlp_specific_sanitization(
        self,
        text: str,
    ...
    async def _safe_intent_detection(
        self,
        text: str,
        session_id: str
    ) -> Dict[str, Any]:
    ...
    async def _safe_entity_extraction(
        self,
        text: str,
        session_id: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _safe_sentiment_analysis(
        self,
        text: str,
        session_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "sentiment": re.sub(r'[^a-zA-Z0-9_-]', '', str(sentiment_result.get("sentiment", "neutral"))),
