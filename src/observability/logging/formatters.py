...
class SensitiveDataRedactor:
    ...
    def redact_message(
        self, message: str, sensitivity: SensitivityLevel = SensitivityLevel.INTERNAL
    ) -> str:
    ...
    def redact_data(
        self, data: Dict[str, Any], sensitivity: SensitivityLevel = SensitivityLevel.INTERNAL
    ) -> Dict[str, Any]:
...
class StructuredFormatter(BaseFormatter):
    ...
    def format(self, record: LogRecord) -> str:
        ...
        # Timestamp
        timestamp = (
            context.timestamp.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
            if context.timestamp
...
class ELKFormatter(JSONFormatter):
    ...
    def format(self, record: LogRecord) -> str:
        ...
        elk_entry = {
            ...
            "process": {
                "pid": context.process_id or record.process,
                "thread": {"id": context.thread_id or record.thread, "name": record.threadName},
            },
            ...
            # Environment
            "environment": {"hostname": context.hostname, "env": context.environment},
        }
        ...
        if record.exc_info:
            elk_entry["exception"] = {
                ...
                "message": str(record.exc_info[1]) if record.exc_info[1] else None,
                "stack_trace": (
                    self.format_exception(record.exc_info) if self.include_stack_trace else None
                ),
            }
        ...
        tags.append(
            f"component_{context.component_type.value if context.component_type else 'unknown'}"
        )
        if record.exc_info:
...
class component_context:
    ...
    def __enter__(self):
        context_vars = {"component_type": self.component_type, **self.additional_context}
