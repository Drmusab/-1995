...
class SensitiveDataFilter(LogFilter):
    ...
    def apply_filter(self, record: logging.LogRecord, context: Dict[str, Any]) -> FilterResult:
        ...
        return FilterResult(
            ...
            metadata={"detected_types": detected_types, "patterns_matched": len(detected_types)},
...
class RateLimitFilter(LogFilter):
    ...
    def apply_filter(self, record: logging.LogRecord, context: Dict[str, Any]) -> FilterResult:
        ...
        return FilterResult(
            ...
            rule_id=self.rule.rule_id,
            original_message=original_message,
            metadata={"rate_limit_key": key, "current_rate": len(self.message_counts[key])},
            processing_time=processing_time,
        )
...
class PerformanceFilter(LogFilter):
    ...
    def apply_filter(self, record: logging.LogRecord, context: Dict[str, Any]) -> FilterResult:
        ...
        self.performance_history.append(
            {"timestamp": time.time(), "system_load": system_load, "action": action.value}
        )
        self.last_activity = time.time()
        ...
        return FilterResult(
            ...
            rule_id=self.rule.rule_id,
            original_message=original_message,
            metadata={"system_load": system_load, "load_threshold": self.load_threshold},
            processing_time=processing_time,
        )
...
class AdaptiveFilter(LogFilter):
    ...
    def _determine_adaptive_action(
        self, record: logging.LogRecord, context: Dict[str, Any], features: Dict[str, Any]
    ) -> FilterAction:
    ...
    def provide_feedback(
        self, pattern_id: str, feedback: str, user_id: Optional[str] = None
    ) -> None:
...
class EnhancedLogFilterManager:
    ...
    def _rebuild_filter_chain(self) -> None:
        ...
        sorted_filters = sorted(
            self.filters.values(), key=lambda f: (f.rule.priority.value, f.rule.rule_id)
        )
                    ...
                    asyncio.create_task(
                        self.event_bus.emit(
                            LogDataMasked(
                                ...
                                sensitivity_level=(
                                    result.sensitivity_detected.value
                                    if result.sensitivity_detected
                                    else "unknown"
                                ),
...
def create_rate_limit_filter(
    rate: int, window: float = 60.0, user_specific: bool = False
) -> FilterRule:
...
def create_context_filter(
    context_keys: List[str], action: FilterAction = FilterAction.ANNOTATE
) -> FilterRule:
