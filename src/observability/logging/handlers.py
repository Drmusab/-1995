...
from src.core.events.event_types import AuditLogEntry, BaseEvent, LoggingEvent, SecurityEvent
from src.core.health_check import HealthCheck
from src.core.security.encryption import EncryptionManager
from src.core.security.sanitization import SecuritySanitizer
from src.observability.logging.config import LogContext, LogFormat, LoggingConfig, LogLevel
...
class LogHandlerError(Exception):
    ...
    def __init__(
        self, message: str, handler_name: Optional[str] = None, error_code: Optional[str] = None
    ):
...
class BaseEnhancedHandler(logging.Handler, ABC):
    ...
    def _setup_processing(self) -> None:
        ...
        if self.handler_config.delivery_mode == DeliveryMode.ASYNCHRONOUS:
            ...
            for i in range(self.handler_config.async_workers):
                worker = threading.Thread(
                    target=self._worker_loop, name=f"{self.handler_name}_worker_{i}", daemon=True
                )
                worker.start()
        ...
        if self.handler_config.delivery_mode == DeliveryMode.BATCH:
            self.batch_queue = []
            self.batch_timer = threading.Timer(self.handler_config.batch_timeout, self._flush_batch)
    ...
    def _create_log_entry(self, record: logging.LogRecord) -> LogEntry:
        ...
        level_priorities = {"CRITICAL": 4, "ERROR": 3, "WARNING": 2, "INFO": 1, "DEBUG": 0}
...
class ElasticsearchHandler(BaseEnhancedHandler):
    ...
    def __init__(
        self, elasticsearch_client, config: HandlerConfig, index_prefix: str = "ai-assistant"
    ):
    ...
    def _setup_index_template(self) -> None:
        ...
        try:
            ...
            self.elasticsearch.indices.put_index_template(
                name=f"{self.index_prefix}-template", body=template
            )
    ...
    def _deliver_log(self, entry: LogEntry) -> None:
        ...
        doc = {
            ...
            "process": {
                ...
                "thread": {"id": entry.record.thread, "name": entry.record.threadName},
            },
        }
        ...
        self.elasticsearch.index(index=index_name, body=doc, id=entry.entry_id)
    ...
    def _deliver_batch(self, entries: List[LogEntry]) -> None:
        ...
        for entry in entries:
            ...
            # Index operation
            bulk_body.append({"index": {"_index": index_name, "_id": entry.entry_id}})
...
class KafkaHandler(BaseEnhancedHandler):
    ...
    def _deliver_log(self, entry: LogEntry) -> None:
        ...
        self.producer.send(
            ...
            key=(
                entry.context.correlation_id.encode("utf-8")
...
class WebhookHandler(BaseEnhancedHandler):
    ...
    def __init__(
        self, webhook_url: str, config: HandlerConfig, headers: Optional[Dict[str, str]] = None
    ):
    ...
    def _deliver_log(self, entry: LogEntry) -> None:
        ...
        response = self.session.post(self.webhook_url, json=payload, timeout=10.0)
    ...
    def _deliver_batch(self, entries: List[LogEntry]) -> None:
        ...
        # Send batch request
        response = self.session.post(self.webhook_url, json=batch_payload, timeout=30.0)
        response.raise_for_status()
...
class SecurityHandler(BaseEnhancedHandler):
    ...
    def _send_security_alert(self, entry: LogEntry, event_type: str, severity: str) -> None:
        ...
        try:
            self.alert_session.post(self.alert_webhook, json=alert_data, timeout=10.0)
        except Exception as e:
    ...
    def _deliver_log(self, entry: LogEntry) -> None:
        ...
        if self.metrics_collector:
            self.metrics_collector.record(
                metric_name, metric_value, tags={"component": component, "unit": metric_unit}
            )
...
class HandlerManager:
    ...
    def create_elasticsearch_handler(self) -> ElasticsearchHandler:
        ...
        return ElasticsearchHandler(
            es_client, config, es_config.get("index_prefix", "ai-assistant")
        )
    ...
    def create_redis_handler(self) -> RedisHandler:
        ...
        return RedisHandler(
            redis_client, config, redis_config.get("key_prefix", "ai-assistant:logs")
        )
    ...
    def create_security_handler(self) -> SecurityHandler:
        ...
        return SecurityHandler(config, security_config.get("alert_webhook"))
...
def get_logger_with_handlers(
    name: str, container: Container, handler_names: Optional[List[str]] = None
) -> logging.Logger:
