...
class MemoryProfiler:
    ...
    def __init__(self, container: Container):
        ...
        # Configuration
        self.profiling_level = ProfilingLevel(self.config.get("memory_profiler.level", "component"))
        self.snapshot_interval_seconds = self.config.get("memory_profiler.snapshot_interval", 300)
        self.retention_days = self.config.get("memory_profiler.retention_days", 7)
        self.enable_leak_detection = self.config.get("memory_profiler.enable_leak_detection", True)
        self.enable_gpu_monitoring = self.config.get("memory_profiler.enable_gpu_monitoring", True)
        self.warning_threshold_percent = self.config.get("memory_profiler.warning_threshold", 70)
        self.critical_threshold_percent = self.config.get("memory_profiler.critical_threshold", 85)
        self.snapshot_dir = Path(
            self.config.get("memory_profiler.snapshot_dir", "data/profiling/memory")
        )
        ...
        try:
            with self.tracer.trace("memory_snapshot") as span:
                ...
                snapshot = MemorySnapshot(
                    snapshot_id=snapshot_id, level=self.profiling_level, snapshot_trigger=trigger
                )
                ...
                if (
                    self.profiling_level in [ProfilingLevel.DETAILED, ProfilingLevel.COMPREHENSIVE]
    ...
    @handle_exceptions
    async def get_memory_usage(self, component: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            memory_info = {
                ...
                "python": {"gc_objects": sum(gc.get_count())},
    ...
    @handle_exceptions
    async def track_component_memory(self, component_name: str, memory_mb: float) -> None:
        """
        ...
        self.metrics.set("memory_component_used_mb", memory_mb, tags={"component": component_name})
    ...
    def _detect_memory_leaks(self) -> List[MemoryAllocation]:
        ...
        for allocation in self.allocations.values():
            ...
            # 3. Not deallocated
            if (
                allocation.size_bytes > 1024 * 1024
                and lifetime_seconds > 600
                and not allocation.is_leaked
            ):
    ...
    async def _cleanup_old_allocations(self) -> None:
        ...
        for allocation_id, allocation in self.allocations.items():
            if allocation.is_deallocated and (now - allocation.timestamp).total_seconds() > 3600:
        ...
        try:
            while self.is_running:
                try:
                    ...
                    self.metrics.set(
                        "memory_system_available_mb", system_memory.available / (1024 * 1024)
                    )
    ...
    async def profile_scope(
        self, name: str, component: Optional[str] = None, session_id: Optional[str] = None
    ):
        """
        ...
        with self.tracer.trace(f"memory_profile_{name}") as span:
            span.set_attributes(
                {"component": component or "unknown", "session_id": session_id or "unknown"}
            )
            ...
            try:
                ...
                # Update span
                span.set_attributes(
                    {"memory_change_bytes": memory_change, "execution_time_seconds": elapsed_time}
                )
    ...
    def get_memory_optimization_suggestions(self) -> List[Dict[str, Any]]:
        ...
        if (
            not suggestions
            and latest_snapshot.process_rss_mb > 0.7 * latest_snapshot.total_physical_mb
        ):
            suggestions.append(
                {
                    "type": "general",
