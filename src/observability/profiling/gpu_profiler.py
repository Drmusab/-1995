...
class SimpleGPUProfiler:
    ...
    async def start_profiling_session(self, session_id: str, 
                                    framework: Optional[GPUFramework] = None) -> GPUProfileSession:
    ...
    async def end_profiling_session(self, session_id: str) -> Optional[GPUProfileSession]:
        ...
        if session.metrics_history:
            ...
            session.peak_memory_mb = max(
                m.memory_used_mb for m in session.metrics_history
            )
    ...
    def profile_context(self, session_id: str):
        ...
        try:
            session = asyncio.create_task(
                self.start_profiling_session(session_id)
            ).result() if asyncio.get_event_loop().is_running() else None
            yield session
        finally:
            if session:
                try:
