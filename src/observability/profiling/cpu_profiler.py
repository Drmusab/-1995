...
class BottleneckDetector:
    ...
    def analyze_profiles(
        self, function_profiles: Dict[str, FunctionProfile], system_metrics: Dict[str, List[float]]
    ...
    def _detect_memory_correlation(
        ...
        for func_key, profile in function_profiles.items():
            if profile.memory_usage_during_calls:
                avg_memory = sum(profile.memory_usage_during_calls) / len(
                    profile.memory_usage_during_calls
                )
...
class FlameGraphGenerator:
    ...
    def generate_flamegraph(
        self, function_profiles: Dict[str, FunctionProfile], output_path: Path, format: str = "svg"
    ) -> bool:
    ...
    def _generate_html_flamegraph(
        self, flame_data: List[Dict[str, Any]], output_path: Path
    ) -> bool:
...
class EnhancedCPUProfiler:
    ...
    def _setup_event_handlers(self) -> None:
        ...
        try:
            ...
            self.event_bus.subscribe(
                "component_health_changed", self._handle_component_health_change
            )
        ...
        with self.profiler_lock:
            ...
            if mode or level:
                config = ProfilingConfig(
                    ...
                    **{k: v for k, v in asdict(self.config).items() if k not in ["mode", "level"]},
                )

            # Create session
            self.current_session = ProfilingSession(
                session_id=session_id, name=session_name, description=description, config=config
            )
    ...
    def _start_deterministic_profiling(self) -> None:
        ...
        self.cProfiler = cProfile.Profile(builtins=False, subcalls=True)
        self.cProfiler.enable()

    def _start_sampling_profiling(self) -> None:
        """Start statistical sampling profiling."""
        # Start sampling thread
        self.profiling_thread = threading.Thread(target=self._sampling_profiling_loop, daemon=True)
    ...
    def _start_adaptive_profiling(self) -> None:
        ...
        self.profiling_thread = threading.Thread(target=self._adaptive_profiling_loop, daemon=True)
        self.profiling_thread.start()
    ...
    def _should_profile_function(self, filename: str, function_name: str) -> bool:
        ...
        if not any(
            path in filename for path in ["src/", "assistant/", "processing/", "reasoning/"]
        ):
            return False
        ...
        with self.profiler_lock:
            ...
            try:
                ...
                self.metrics.increment(
                    "profiler_functions_profiled", len(session.function_profiles)
                )
    ...
    def _calculate_session_statistics(self, session: ProfilingSession) -> None:
        ...
        # Calculate totals
        session.total_function_calls = sum(
            fp.call_count for fp in session.function_profiles.values()
        )
    ...
    async def _generate_flamegraph_async(self, session: ProfilingSession, output_path: str) -> bool:
