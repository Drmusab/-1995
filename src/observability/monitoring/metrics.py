...
from src.core.events.event_types import (
    MetricRecorded, MetricExported, MetricThresholdExceeded,
    ComponentHealthChanged, SystemStateChanged, PerformanceWarning,
    UserInteractionStarted, UserInteractionCompleted, WorkflowStarted,
    WorkflowCompleted, SessionStarted, SessionEnded, ProcessingStarted,
    ProcessingCompleted, PluginLoaded, PluginUnloaded
)
...
class MetricsCollector:
    ...
    def __init__(self, container: Container):
        ...
        self.metric_store = MetricStore(
            max_samples=self.config.get("metrics.max_samples", 10000)
        )
        ...
        try:
            ...
            for backend_type in backends_config:
                if backend_type == "prometheus":
                    endpoint = self.config.get("metrics.prometheus.endpoint", "http://localhost:9090")
                    ...
                    file_path = self.config.get("metrics.json.file_path", "data/metrics/metrics.json")
        ...
        try:
            ...
            if self.enable_system_metrics:
                system_metrics = [
                    ...
                    ("system_memory_usage_percent", MetricType.GAUGE, "System memory usage percentage"),
                ...
                for name, metric_type, description in system_metrics:
                    self.register_metric(name, metric_type, description, unit="percent" if "percent" in name else "bytes")
            
            # Core engine metrics
            core_metrics = [
                ("engine_requests_total", MetricType.COUNTER, "Total engine requests"),
                ("engine_processing_duration_seconds", MetricType.HISTOGRAM, "Engine processing duration"),
            ...
            workflow_metrics = [
                ...
                ("workflow_executions_successful", MetricType.COUNTER, "Successful workflow executions"),
                ("workflow_executions_failed", MetricType.COUNTER, "Failed workflow executions"),
                ("workflow_execution_duration_seconds", MetricType.HISTOGRAM, "Workflow execution duration"),
            ...
            interaction_metrics = [
                ...
                ("response_generation_time_seconds", MetricType.HISTOGRAM, "Response generation time"),
            ...
            plugin_metrics = [
                ...
                ("plugin_execution_duration_seconds", MetricType.HISTOGRAM, "Plugin execution duration"),
            ]
            ...
            # Component metrics
            component_metrics = [
                ("component_registrations_total", MetricType.COUNTER, "Total component registrations"),
                ("component_initializations_total", MetricType.COUNTER, "Total component initializations"),
                ("component_failures_total", MetricType.COUNTER, "Total component failures"),
                ("component_initialization_duration_seconds", MetricType.HISTOGRAM, "Component initialization duration"),
    ...
    def increment(self, name: str, value: float = 1.0, tags: Optional[Dict[str, str]] = None) -> None:
        ...
        try:
            ...
            if self.enable_realtime_export:
                asyncio.create_task(self.event_bus.emit(MetricRecorded(
                    metric_name=name,
                    metric_type="counter",
                    value=value,
                    tags=final_tags
    ...
    async def export_metrics(self) -> None:
        ...
        try:
            ...
            if export_errors == 0:
                await self.event_bus.emit(MetricExported(
                    backend_count=len(self.export_backends),
                    metric_count=len(metrics),
                    timestamp=datetime.now(timezone.utc)
                ))
            
            self.logger.debug(f"Exported {len(metrics)} metrics to {len(self.export_backends)} backends")
    ...
    async def _handle_workflow_step_completed(self, event) -> None:
        ...
        self.increment("workflow_steps_executed", tags={
    ...
    async def _handle_interaction_started(self, event) -> None:
        ...
        self.increment("interactions_total", tags={
    ...
    async def _handle_component_registered(self, event) -> None:
        ...
        self.increment("component_registrations_total", tags={"component_type": event.component_type})
    ...
    async def _health_check_callback(self) -> Dict[str, Any]:
        ...
        try:
            ...
            status = "healthy" if health_score >= 0.9 else "degraded" if health_score >= 0.5 else "unhealthy"
            ...
            return {
                ...
                "last_collection": self.collection_stats["last_collection_time"].isoformat() if self.collection_stats["last_collection_time"] else None
            }
