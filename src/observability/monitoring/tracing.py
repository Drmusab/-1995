...
from typing import Optional, Dict, Any, List, Set, Callable, Type, Union, AsyncGenerator, ContextManager
import asyncio
import threading
import time
import traceback
...
from src.core.events.event_types import (
    TraceStarted, TraceCompleted, TraceFailed, SpanStarted, SpanCompleted,
    SpanError, PerformanceThresholdExceeded, ComponentPerformanceAlert,
    TracingConfigurationChanged, TraceCorrelationEstablished
)
...
class TracingError(Exception):
    ...
    def __init__(self, message: str, trace_id: Optional[TraceId] = None, 
...
class ConsoleSpanExporter(SpanExporter):
    ...
    async def export(self, spans: List[SpanData]) -> None:
        ...
        for span in spans:
            self.logger.info(f"TRACE: {span.trace_id} | SPAN: {span.span_id} | "
...
class SamplingDecider:
    ...
    def should_sample(self, operation_name: str, parent_context: Optional[TraceContext] = None) -> bool:
    ...
    def _probabilistic_sampling(self, operation_name: str) -> bool:
        ...
        sampling_rate = self.config.component_sampling.get(operation_name, self.config.sampling_rate)
    ...
    def update_metrics(self, operation_name: str, duration: float, is_error: bool) -> None:
        ...
        self._error_rates[operation_name] = 0.9 * current_error_rate + 0.1 * (1.0 if is_error else 0.0)
...
class EnhancedSpan:
    ...
    def __init__(self, span_data: SpanData, processor: SpanProcessor, 
    ...
    def add_event(self, name: str, attributes: Optional[Dict[str, Any]] = None, 
                  timestamp: Optional[datetime] = None) -> 'EnhancedSpan':
    ...
    def add_link(self, trace_id: TraceId, span_id: SpanId, 
                 attributes: Optional[Dict[str, Any]] = None) -> 'EnhancedSpan':
    ...
    def record_exception(self, exception: Exception, attributes: Optional[Dict[str, Any]] = None) -> 'EnhancedSpan':
    ...
    def add_checkpoint(self, name: str, metadata: Optional[Dict[str, Any]] = None) -> 'EnhancedSpan':
...
class TraceManager:
    ...
    def __init__(self, container: Container, config: Optional[TracingConfiguration] = None):
        ...
        # Performance tracking
        self._trace_statistics: Dict[str, Dict[str, float]] = defaultdict(lambda: defaultdict(float))
    ...
    @handle_exceptions
    def create_trace(self, operation_name: str, correlation_id: Optional[CorrelationId] = None,
        ...
        asyncio.create_task(self.event_bus.emit(TraceStarted(
            trace_id=trace_id,
            operation_name=operation_name,
            user_id=user_id,
            session_id=session_id,
        ...
        if self.metrics:
            ...
            self.metrics.increment("sampling_decisions_total", 
                                 tags={"sampled": str(context.is_sampled)})
        
        self.logger.debug(f"Created trace: {trace_id} for operation: {operation_name}")
        return context

    @handle_exceptions
    def start_span(self, operation_name: str, span_type: SpanType = SpanType.COMPONENT_INITIALIZATION,
                  parent_context: Optional[TraceContext] = None,
                  attributes: Optional[Dict[str, Any]] = None,
                  links: Optional[List[Dict[str, Any]]] = None) -> EnhancedSpan:
        """
        Start a new span.
        ...
        span = EnhancedSpan(span_data, self._processors[0] if self._processors else None, parent_context)
        ...
        asyncio.create_task(self.event_bus.emit(SpanStarted(
            trace_id=trace_id,
            span_id=span_id,
            operation_name=operation_name,
    ...
    @contextmanager
    def trace(self, operation_name: str, span_type: SpanType = SpanType.COMPONENT_INITIALIZATION,
             attributes: Optional[Dict[str, Any]] = None,
             correlation_id: Optional[CorrelationId] = None) -> EnhancedSpan:
        """
    ...
    @asynccontextmanager
    async def async_trace(self, operation_name: str, span_type: SpanType = SpanType.COMPONENT_INITIALIZATION,
                         attributes: Optional[Dict[str, Any]] = None,
                         correlation_id: Optional[CorrelationId] = None) -> EnhancedSpan:
        """
            ...
            return [
                {
                    ...
                    "duration": (datetime.now(timezone.utc) - span.span_data.start_time).total_seconds(),
    ...
    def update_configuration(self, config: TracingConfiguration) -> None:
        ...
        asyncio.create_task(self.event_bus.emit(TracingConfigurationChanged(
    ...
    async def _update_statistics(self, operation_name: str, duration: float, is_error: bool) -> None:
    ...
    async def _cleanup_loop(self) -> None:
        ...
        while True:
            try:
                ...
                with self._context_lock:
                    old_traces = [
                        trace_id for trace_id, context in self._trace_contexts.items()
                ...
                if old_traces or old_spans:
                    self.logger.info(f"Cleaned up {len(old_traces)} old traces and {len(old_spans)} old spans")
            ...
            try:
                ...
                if self.metrics:
                    ...
                    for operation_name, stats in self._trace_statistics.items():
                        self.metrics.set("operation_avg_duration_seconds", 
                                       stats.get("avg_duration", 0.0),
                                       tags={"operation": operation_name})
                        self.metrics.set("operation_error_rate", 
