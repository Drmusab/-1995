    ...
    async def send_notification(self, alert: Alert, target: NotificationTarget) -> bool:
        ...
        try:
            ...
            if "webhook_url" in config:
                ...
                payload = {"channel": config["channel"], **message}
...
class WebhookNotificationSender(NotificationSender):
    ...
    async def send_notification(self, alert: Alert, target: NotificationTarget) -> bool:
        ...
        try:
            ...
            response = requests.post(
                config["url"], json=payload, headers=headers, timeout=config.get("timeout", 10)
            )
            response.raise_for_status()
...
class AlertingEngine:
    ...
    def __init__(self, container: Container):
        ...
        self.max_concurrent_notifications = self.config.get(
            "alerting.max_concurrent_notifications", 5
        )
        ...
        try:
            ...
            for target_id, target_config in targets_config.items():
                ...
                target = NotificationTarget(
                    ...
                    severity_filter=set(
                        AlertSeverity(s) for s in target_config.get("severity_filter", [])
                    ),
                    category_filter=set(
                        AlertCategory(c) for c in target_config.get("category_filter", [])
                    ),
            ...
            for rule_id, rule_config in rules_config.items():
                rule = AlertRule(
                    ...
                    notification_channels=[
                        ...
                        for ch in rule_config.get("notification_channels", ["log"])
                    ],
            ...
            self.logger.info(
                f"Loaded {len(self.notification_targets)} notification targets and {len(rules_config)} custom rules"
            )
    ...
    async def _register_event_handlers(self) -> None:
        ...
        self.event_bus.subscribe(
            "performance_threshold_exceeded", self._handle_performance_threshold
        )
    ...
    def register_notification_target(self, target: NotificationTarget) -> None:
        ...
        self.logger.info(
            f"Registered notification target: {target.target_id} ({target.channel.value})"
        )
    ...
    async def _gather_evaluation_context(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.metrics, "get_metric"):
                context["system_cpu_usage_percent"] = await self.metrics.get_metric(
                    "system_cpu_usage_percent", 0.0
                )
                context["system_memory_usage_percent"] = await self.metrics.get_metric(
                ...
                context["average_response_time_seconds"] = await self.metrics.get_metric(
            ...
            if self.session_manager:
                ...
                if session_stats.get("total_active_sessions", 0) > 0:
                    ...
                    context["session_failure_rate_percent"] = (
                        failed_sessions / total_sessions
                    ) * 100
                else:
            ...
            if self.workflow_orchestrator:
                try:
                    ...
                    failed_workflows = len(
                        [e for e in active_executions if e.get("state") == "failed"]
                    )
                    total_workflows = len(active_executions)
                    if total_workflows > 0:
                        context["workflow_failure_rate_percent"] = (
    ...
    async def _trigger_alert_from_rule(
        self, rule: AlertRule, context: Dict[str, Any], metric_value: Optional[float] = None
    ) -> None:
        """Trigger an alert from a rule evaluation."""
        # Check if similar alert already exists
        existing_alerts = await self.alert_manager.get_active_alerts()
        for alert in existing_alerts:
            if alert.rule_id == rule.rule_id and alert.affected_component == context.get(
                "affected_component"
            ):
        ...
        for channel in rule.notification_channels:
            for target in self.notification_targets.values():
                if (
                    ...
                    and target.enabled
                    and self._target_matches_alert(target, alert)
                ):
    ...
    def _target_matches_alert(self, target: NotificationTarget, alert: Alert) -> bool:
        ...
        if (
            target.component_filter
            and alert.affected_component
            and alert.affected_component not in target.component_filter
        ):
            return False
    ...
    def _should_send_notification(self, target: NotificationTarget, alert: Alert) -> bool:
        ...
        if target.quiet_hours_start is not None and target.quiet_hours_end is not None:
            ...
            if target.quiet_hours_start <= target.quiet_hours_end:
                ...
                if (
                    current_hour >= target.quiet_hours_start
                    or current_hour < target.quiet_hours_end
                ):
                    return False
        ...
        if target.rate_limit_enabled and target.last_notification:
            ...
            if (
                target.last_notification > hour_ago
                and target.notification_count >= target.max_notifications_per_hour
            ):
                return False
    ...
    async def _send_notification(self, alert: Alert, target: NotificationTarget) -> bool:
        ...
        try:
            ...
            success = await asyncio.wait_for(
                sender.send_notification(alert, target), timeout=self.notification_timeout
            )
