...
from src.core.events.event_types import (
    LearningEventOccurred, FeedbackReceived, MemoryItemStored
)
...
class MemoryLearningBridge:
    ...
    def __init__(self, container: Container):
        ...
        try:
            ...
            self.metrics.register_counter("learning_events_processed", 
                                         "Learning events processed")
            self.metrics.register_counter("feedback_memories_created", 
    ...
    async def _handle_learning_event(self, event: LearningEventOccurred) -> None:
        ...
        try:
            ...
            # Record metric
            if self.metrics:
                self.metrics.increment("learning_events_processed", 
    ...
    async def _handle_feedback(self, event: FeedbackReceived) -> None:
        ...
        try:
            ...
            # Record metric
            if self.metrics:
                self.metrics.increment("feedback_memories_created", 
                                      tags={"rating": str(event.rating)})
    ...
    async def _analyze_feedback_memories(
        self, 
        memories: List[MemoryItem]
    ) -> List[Dict[str, Any]]:
        """
