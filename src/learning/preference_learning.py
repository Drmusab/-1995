...
class UserPreferenceProfile:
    ...
    contextual_preferences: Dict[PreferenceDimension, ContextualPreference] = field(
        default_factory=dict
    )

    # Preference hierarchies and conflicts
    preference_priorities: Dict[PreferenceDimension, float] = field(default_factory=dict)
    conflict_resolutions: Dict[Tuple[PreferenceDimension, PreferenceDimension], str] = field(
        default_factory=dict
    )
    ...
    def get_preference(
        self, dimension: PreferenceDimension, context: Optional[Dict[str, Any]] = None
    ) -> float:
        ...
        if context and dimension in self.contextual_preferences:
            ...
            for context_key, context_value in context.items():
                if isinstance(context_value, (int, float, bool)):
                    current_modifier = self.contextual_preferences[dimension].context_modifiers.get(
                        context_key, 0.0
                    )
...
class PreferenceLearningError(Exception):
    ...
    def __init__(
        self, message: str, user_id: Optional[str] = None, preference_type: Optional[str] = None
    ):
...
class PreferenceLearner(ABC):
    ...
    async def learn_from_interaction(
        self, interaction_data: Dict[str, Any], user_profile: UserPreferenceProfile
    ) -> None:
...
class ReinforcementPreferenceLearner(PreferenceLearner):
    ...
    def _update_preferences_from_q_values(
        self, user_profile: UserPreferenceProfile, action: str, q_value: float
    ) -> None:
...
class CollaborativeFilteringLearner(PreferenceLearner):
            ...
            if total_weight > 0:
                ...
                if base_confidence > 0.1:
                    final_value = (
                        ...
                        + collaborative_value * collaborative_confidence
                    ) / (base_confidence + collaborative_confidence)
                    final_confidence = min(1.0, base_confidence + collaborative_confidence * 0.3)
                else:
                    final_value = collaborative_value
                    final_confidence = (
                        collaborative_confidence * 0.7
                    )  # Reduce confidence for cold start
    ...
    async def _update_from_similar_users(
        self, user_profile: UserPreferenceProfile, similar_users: List[str]
    ) -> None:
...
class PreferenceLearningEngine:
    ...
    def __init__(self, container: Container):
        ...
        self.min_interactions_for_learning = self.config.get(
            "preference_learning.min_interactions", 5
        )
        self.enable_collaborative_filtering = self.config.get(
            "preference_learning.collaborative_filtering", True
        )
        self.enable_clustering = self.config.get("preference_learning.clustering", True)
        self.auto_clustering_interval = self.config.get(
            "preference_learning.clustering_interval", 3600
        )
    ...
    def _setup_learning_algorithms(self) -> None:
        ...
        try:
            ...
            self.learning_algorithms[LearningAlgorithm.BAYESIAN_INFERENCE] = (
                BayesianPreferenceLearner()
            )
            self.learning_algorithms[LearningAlgorithm.REINFORCEMENT_LEARNING] = (
                ReinforcementPreferenceLearner()
            )

            if self.enable_collaborative_filtering:
                self.learning_algorithms[LearningAlgorithm.COLLABORATIVE_FILTERING] = (
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if self.enable_clustering:
                self.background_tasks.append(asyncio.create_task(self._clustering_loop()))
    ...
    async def predict_user_preference(
        ...
        self.logger.debug(
            f"Predicted preference for user {user_id}, dimension {preference_dimension}: {prediction:.3f} (confidence: {confidence:.3f})"
        )

        return prediction, confidence

    @handle_exceptions
    async def update_from_interaction(self, user_id: str, interaction_data: Dict[str, Any]) -> None:
        """
        ...
        async with self.preference_locks[user_id]:
            try:
                ...
                self.learning_stats[user_id]["total_updates"] = (
                    self.learning_stats[user_id].get("total_updates", 0) + 1
                )
    ...
    async def update_from_feedback(
        ...
        async with self.preference_locks[user_id]:
            try:
                ...
                self.logger.error(
                    f"Error updating preferences from feedback for user {user_id}: {str(e)}"
                )
    ...
    def _select_best_algorithm(
        self, profile: UserPreferenceProfile, dimension: PreferenceDimension
    ) -> PreferenceLearner:
    ...
    def _extract_preference_signals_from_feedback(
        self, feedback_data: Dict[str, Any], interaction_context: Optional[Dict[str, Any]] = None
    ) -> Dict[PreferenceDimension, Tuple[float, float]]:
                ...
                try:
                    ...
                    if isinstance(pref_value, (int, float)):
                        ...
                        signals[dimension] = (
                            normalized_value,
                            0.9,
                        )  # High confidence for direct feedback
                except ValueError:
                    continue
    ...
    async def _store_user_profile(self, profile: UserPreferenceProfile) -> None:
        ...
        try:
            ...
            if self.redis_cache:
                cache_data = {
                    "preferences": {
                        dim.value: pref.value for dim, pref in profile.preferences.items()
                    },
                    "last_updated": profile.last_updated.isoformat(),
                    "total_interactions": profile.total_interactions,
                }
            ...
            for key, value in data.items():
                if isinstance(value, datetime):
                    ...
                    if any(isinstance(k, Enum) for k in value.keys()):
                        data[key] = {
                            (k.value if isinstance(k, Enum) else k): v for k, v in value.items()
                        }
                ...
                try:
                    ...
                    if isinstance(pref_data, dict):
                        ...
                        if "last_updated" in pref_data:
                            pref_value.last_updated = datetime.fromisoformat(
