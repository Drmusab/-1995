...
class ComponentConfiguration:
    ...
    def _register_core_services(self, container: Container) -> None:
        ...
        container.register(
            ConfigLoader, factory=lambda: ConfigLoader(), scope=LifecycleScope.SINGLETON
        )

        # Event bus (singleton)
        container.register(EventBus, factory=lambda: EventBus(), scope=LifecycleScope.SINGLETON)
        ...
        container.register(
            HealthCheck, factory=lambda: HealthCheck(), scope=LifecycleScope.SINGLETON
        )
    ...
    def _register_processing_components(self, container: Container) -> None:
        ...
        # NLP components
        container.register(
            IntentManager, factory=lambda: IntentManager(container), scope=LifecycleScope.SINGLETON
        )

        container.register(
            LanguageChain, factory=lambda: LanguageChain(container), scope=LifecycleScope.SINGLETON
        )
    ...
    def _register_memory_components(self, container: Container) -> None:
        ...
        container.register(
            VectorStore, factory=lambda: VectorStore(container), scope=LifecycleScope.SINGLETON
        )

        # Memory types
        container.register(
            WorkingMemory, factory=lambda: WorkingMemory(container), scope=LifecycleScope.SINGLETON
        )
        ...
        container.register(
            MemoryManager, factory=lambda: MemoryManager(container), scope=LifecycleScope.SINGLETON
        )
    ...
    def _register_reasoning_components(self, container: Container) -> None:
        ...
        container.register(
            SkillRegistry, factory=lambda: SkillRegistry(container), scope=LifecycleScope.SINGLETON
        )

        container.register(
            SkillFactory, factory=lambda: SkillFactory(container), scope=LifecycleScope.SINGLETON
        )

        # Reasoning components
        container.register(
            LogicEngine, factory=lambda: LogicEngine(container), scope=LifecycleScope.SINGLETON
        )
    ...
    def _register_integration_components(self, container: Container) -> None:
        ...
        # Model router
        container.register(
            ModelRouter, factory=lambda: ModelRouter(container), scope=LifecycleScope.SINGLETON
        )

        # Cache strategy
        container.register(
            CacheStrategy, factory=lambda: CacheStrategy(container), scope=LifecycleScope.SINGLETON
        )
    ...
    def _register_observability_components(self, container: Container) -> None:
        ...
        container.register(
            TraceManager, factory=lambda: TraceManager(container), scope=LifecycleScope.SINGLETON
        )
