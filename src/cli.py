...
class AssistantCLI:
    ...
    def _create_argument_parser(self):
        ...
        mode_group.add_argument(
            "-r", "--repl", action="store_true", help="Run in REPL (Read-Eval-Print Loop) mode"
        )
        mode_group.add_argument(
            "-s", "--script", type=str, help="Execute commands from a script file"
        )
        mode_group.add_argument(
            "-m", "--monitor", action="store_true", help="Run in system monitoring mode"
        )

        # Other options
        parser.add_argument("-u", "--user", type=str, help="User ID for authentication")
    ...
    async def _run_interactive_mode(self):
        ...
        self.print_system_message(
            "Type a message to chat with the assistant, or start with '/' for commands."
        )
                ...
                if RICH_AVAILABLE:
                    ...
                    for component_name, component_status in components.items():
                        ...
                        if component_name == "core_engine" and component_status:
                            ...
                            details = (
                                f"Active workflows: {component_status.get('active_executions', 0)}"
                            )
    ...
    def _cmd_mode(self, mode: str = None):
        ...
        try:
            ...
            if new_mode == CLIMode.INTERACTIVE:
                ...
                self.print_error(
                    f"Can't directly switch to {new_mode.value} mode. Please restart the CLI."
                )
        ...
        if action == "new":
            ...
            if RICH_AVAILABLE:
                ...
                for session in sessions:
                    current_marker = (
                        "* " if session["session_id"] == self.current_session_id else ""
                    )
                    table.add_row(
                ...
                for session in sessions:
                    ...
                    print(
                        f"{current_marker}{session['session_id']} - User: {session.get('user_id', 'anonymous')}, "
            ...
            if session_id == self.current_session_id:
                ...
                self.current_session_id = new_session_id
                self.print_system_message(
                    f"Ended previous session and created new session: {new_session_id}"
                )
            else:
    ...
    async def _cmd_workflow(
        self, action: str = "list", workflow_id: str = None, params_json: str = None
    ):
        """
        ...
        if action == "list":
            ...
            if RICH_AVAILABLE:
                ...
                for workflow in workflows:
                    print(
                        f"{workflow['workflow_id']} - {workflow.get('name', 'unknown')} "
            ...
            if RICH_AVAILABLE:
                with Progress(
                    ...
                    while status["state"] in ["running", "planning", "ready"]:
                        ...
                        status = await self.assistant.workflow_orchestrator.get_execution_status(
                            execution_id
                        )
            else:
            ...
            if status["state"] == "completed":
                self.print_system_message(
                    f"Workflow completed successfully in {status.get('execution_time', 0):.2f}s"
                )
            else:
            ...
            if not workflow_id:
                ...
                if RICH_AVAILABLE:
                    ...
                    for execution in executions:
                        print(
                            f"{execution['execution_id']} - Workflow: {execution.get('workflow_id', 'unknown')}, "
    ...
    async def _cmd_plugin(self, action: str = "list", plugin_id: str = None):
        ...
        if action == "list":
            ...
            if RICH_AVAILABLE:
                ...
                for plugin in plugins:
                    print(
                        f"{plugin['plugin_id']} - {plugin.get('name', 'unknown')} "
