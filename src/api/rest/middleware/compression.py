...
from ..core.events.event_types import APIEvent, CompressionEvent
...
class AdaptiveCompressionStrategy:
    ...
    def __init__(
        self, strategies: Optional[List[CompressionStrategy]] = None, sample_size: int = 1024
    ):
...
class CompressionMiddleware:
    ...
    def _init_strategies(self):
        ...
        if self.adaptive_compression:
            self.adaptive_strategy = AdaptiveCompressionStrategy(list(self.strategies.values()))
    ...
    def should_compress_response(
        self, content: bytes, content_type: str, headers: Dict[str, str]
    ) -> bool:
    ...
    async def compress_response(
        self, content: bytes, content_type: str, accept_encoding: str
    ) -> tuple[bytes, str, CompressionStats]:
    ...
    def get_compression_stats(self) -> Dict[str, Any]:
        ...
        avg_compression_time = sum(s.compression_time for s in self.stats_buffer) / len(
            self.stats_buffer
        )
        ...
        return {
            ...
            "bytes_processed": {"original": total_original, "compressed": total_compressed},
        }
...
def create_compression_middleware(
    event_bus: Optional[EventBus] = None, metrics_collector: Optional[MetricsCollector] = None
