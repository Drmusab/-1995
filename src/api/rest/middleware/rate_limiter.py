...
class RateLimitStrategy(ABC):
    ...
    async def check_limit(
        self, rule: RateLimitRule, state: RateLimitState, context: Dict[str, Any]
    ) -> RateLimitResult:
    ...
    async def update_state(
        self, state: RateLimitState, allowed: bool, context: Dict[str, Any]
    ) -> None:
...
class AdaptiveStrategy(RateLimitStrategy):
    ...
    async def check_limit(
        ...
        state.adaptation_history.append(
            {"timestamp": current_time, "system_load": system_load, "adapted_limit": adapted_limit}
        )
    ...
    async def _get_system_load(self, context: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if self.metrics:
                ...
                load = (
                    cpu_usage * 0.4 + memory_usage * 0.4 + min(active_connections / 1000, 1.0) * 0.2
                )
                return min(1.0, load)
...
class CircuitBreakerStrategy(RateLimitStrategy):
    ...
    async def check_limit(
        ...
        if (
            ...
            and (current_time - state.last_failure).total_seconds() > rule.recovery_timeout_seconds
        ):
    ...
    async def cleanup_expired_states(self) -> int:
        ...
        with self.lock:
            for key, state in self.states.items():
                ...
                if (
                    state.last_request
                    and (current_time - state.last_request).total_seconds() > 86400
                ):
...
class RedisStorage(RateLimitStorage):
    ...
    def _deserialize_state(self, data: Dict[str, Any]) -> RateLimitState:
        ...
        state.request_times = deque(
            [datetime.fromisoformat(t) for t in data["request_times"]], maxlen=1000
        )
...
class EnhancedRateLimiter:
    ...
    def __init__(self, container: Container):
        ...
        self.default_user_tier = UserTier(
            self.config.get("rate_limiting.default_user_tier", "free")
        )
                        ...
                        for identifier in identifiers:
                            if identifier["scope"] == rule.scope:
                                ...
                                if not result.allowed:
                                    ...
                                    or result.remaining_requests
                                    < most_restrictive_result.remaining_requests
                                ):
                                    most_restrictive_result = result

                    final_result = most_restrictive_result or RateLimitResult(
                        allowed=True, rule_id="default", identifier="none", action=ActionType.WARN
                    )
    ...
    async def _extract_identifiers(self, request_context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if user_id:
            identifiers.append({"scope": LimitScope.USER, "value": user_id})

        # Session identifier
        session_id = request_context.get("session_id")
        if session_id:
            identifiers.append({"scope": LimitScope.SESSION, "value": session_id})

        # IP address identifier
        ip_address = request_context.get("client_ip")
        if ip_address:
            identifiers.append({"scope": LimitScope.IP_ADDRESS, "value": ip_address})

        # API key identifier
        api_key = request_context.get("api_key")
        if api_key:
            identifiers.append({"scope": LimitScope.API_KEY, "value": api_key})

        # Global identifier
        identifiers.append({"scope": LimitScope.GLOBAL, "value": "global"})
    ...
    async def _find_applicable_rules(
        ...
        for rule in self.rules.values():
            ...
            # Check scope match
            scope_matches = any(identifier["scope"] == rule.scope for identifier in identifiers)
    ...
    async def _check_single_rule(
        self, rule: RateLimitRule, identifier: str, request_context: Dict[str, Any]
    ) -> RateLimitResult:
    ...
    async def reset_user_limits(self, user_id: str) -> None:
        ...
        # Emit reset event
        await self.event_bus.emit(
            RateLimitReset(identifier=user_id, scope=LimitScope.USER.value, reason="Manual reset")
        )
    ...
    async def bypass_limits(
        self, identifier: str, scope: LimitScope, duration_seconds: int = 3600
    ) -> None:
        """
        ...
        try:
            ...
            try:
                ...
                test_state = RateLimitState(
                    identifier="health_check", rule_id="test", scope=LimitScope.GLOBAL
                )
