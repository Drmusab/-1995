    ...
    def matches(self, origin: str) -> bool:
        ...
        try:
            if self.validation_type == OriginValidationType.EXACT_MATCH:
                ...
                return origin_lower == domain or origin_lower.endswith(f".{domain}")
...
class PreflightCache:
    ...
    async def set(
        self, origin: str, method: str, headers: Set[str], response: Dict[str, Any], max_age: int
    ) -> None:
...
class CorsMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self, app: ASGIApp, container: Container, config: Optional[CorsConfiguration] = None
    ):
        """
    ...
    def _load_default_configuration(self) -> CorsConfiguration:
        ...
        config = CorsConfiguration(
            ...
            enable_detailed_logging=cors_settings.get(
                "detailed_logging", environment != "production"
            ),
        ...
        for origin_config in origins_config:
            if isinstance(origin_config, str):
                ...
                rule = OriginRule(
                    ...
                    allowed_methods=set(origin_config.get("methods", []))
                    or self.cors_config.allowed_methods,
    ...
    async def _handle_preflight_request(self, request: Request) -> Response:
        ...
        if not origin_rule:
            raise CorsViolation(
                f"Origin '{origin}' not allowed", "origin_not_allowed", origin=origin
            )

        # Validate method
        if (
            requested_method
            and self.cors_config.enable_method_validation
            and requested_method not in origin_rule.allowed_methods
        ):
            raise CorsViolation(
        ...
        if self.cors_config.enable_header_validation:
            forbidden_headers = (
                requested_headers - origin_rule.allowed_headers - self.cors_config.allowed_headers
            )
    ...
    def _create_preflight_response(self, response_data: Dict[str, Any]) -> Response:
        ...
        return Response(status_code=self.cors_config.options_success_status, headers=headers)
    ...
    async def _handle_cors_violation(self, request: Request, violation: CorsViolation) -> Response:
        ...
        if self.cors_config.block_suspicious_requests and self.violation_counts[origin] > 10:
        ...
        if self.metrics:
            self.metrics.increment(
                "cors_violations_total", tags={"violation_type": violation.violation_type}
            )
            if origin in self.blocked_origins:
    ...
    async def update_origin_rules(self, rules: List[OriginRule]) -> None:
        ...
        if self.event_bus:
            await self.event_bus.emit(
                SecurityPolicyUpdated(
                    policy_type="cors_origins", rules_count=len(rules), updated_by="system"
                )
            )
    ...
    async def remove_origin_rule(self, pattern: str) -> bool:
        ...
        self.cors_config.origin_rules = [
            rule for rule in self.cors_config.origin_rules if rule.pattern != pattern
        ]
...
def create_production_cors_middleware(
    app: ASGIApp, container: Container, allowed_origins: List[str]
) -> CorsMiddleware:
    """Create CORS middleware with production-ready settings."""
    origin_rules = [
        OriginRule(origin, OriginValidationType.EXACT_MATCH) for origin in allowed_origins
    ]
