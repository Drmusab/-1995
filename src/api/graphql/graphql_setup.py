...
try:
    import graphene
    from graphene import ObjectType, String, Field, List as GrapheneList, Int, Float, Boolean
...
if GRAPHENE_AVAILABLE:
    ...
    class Mutation(ObjectType):
        ...
        async def resolve_process_text(self, info, text, session_id=None):
            ...
            if assistant:
                try:
                    result = await assistant.process_text_input(
                        text=text,
...
class GraphQLServer:
    ...
    async def execute_query(self, query: str, variables: Optional[Dict] = None) -> Dict:
        ...
        if not GRAPHENE_AVAILABLE:
            return {
                "errors": [{"message": "GraphQL not available - graphene not installed"}]
            }
