...
class AuthenticationInterceptor(aio.ServerInterceptor):
    ...
    async def intercept_service(self, continuation, handler_call_details):
        ...
        try:
            ...
            if not user_info:
                await handler_call_details.abort(grpc.StatusCode.UNAUTHENTICATED, "Invalid token")
                return
    ...
    async def EndInteraction(self, request, context):
        ...
        try:
            await self.interaction_handler.end_interaction(
                request.interaction_id, request.reason or "user_ended"
            )
    ...
    async def CreateSession(self, request, context):
        ...
        try:
            ...
            session_id = await self.session_manager.create_session(
                user_id=user_id, session_config=session_config, context_data=context_data
            )

            response = assistant_pb2.CreateSessionResponse(session_id=session_id, success=True)
    ...
    async def EndSession(self, request, context):
        ...
        try:
            await self.session_manager.end_session(
                request.session_id, request.reason or "user_ended"
            )
...
class ComponentService(assistant_pb2_grpc.ComponentServiceServicer):
    ...
    async def GetComponentStatus(self, request, context):
        ...
        try:
            ...
            response = assistant_pb2.ComponentStatusResponse(status=json.dumps(status))
...
class MetricsService(metrics_pb2_grpc.MetricsServiceServicer):
    ...
    async def GetMetrics(self, request, context):
        ...
        try:
            ...
            response = metrics_pb2.GetMetricsResponse(metrics=json.dumps(metrics_data))
...
class GrpcServer:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            self.server = aio.server(interceptors=interceptors, options=self.server_options)
            ...
            # Add health service
            health_pb2_grpc.add_HealthServicer_to_server(HealthService(self.container), self.server)
...
async def create_grpc_server(
    container: Container, host: str = "0.0.0.0", port: int = 50051
) -> GrpcServer:
