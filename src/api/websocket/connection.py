...
from src.core.events.event_types import ConnectionClosed as ConnectionClosedEvent
from src.core.events.event_types import (
...
class ConnectionError(Exception):
    ...
    def __init__(
        self, message: str, connection_id: Optional[str] = None, error_code: Optional[str] = None
    ):
...
class MessageRouter:
    ...
    async def route_message(
        self, message: WebSocketMessage, source_connection: ConnectionInfo
    ) -> List[str]:
...
class BroadcastManager:
    ...
    async def broadcast(
        ...
        target_connections = await self._filter_connections(scope, connections, filter_criteria)
        ...
        if self.redis_cache and broadcast_count > 0:
            try:
                ...
                await self.redis_cache.publish(self.redis_channel, json.dumps(distributed_message))
            except Exception as e:
                self.logger.warning(f"Failed to distribute broadcast via Redis: {str(e)}")

        return broadcast_count

    async def _filter_connections(
        self,
        scope: BroadcastScope,
        connections: Dict[str, ConnectionInfo],
        filter_criteria: Optional[Dict[str, Any]] = None,
    ...
    async def _apply_custom_filters(
        self, connection: ConnectionInfo, filter_criteria: Dict[str, Any]
    ) -> bool:
...
class ConnectionManager:
    ...
    def _setup_message_handlers(self) -> None:
        ...
        self.message_router.register_handler(MessageType.AUTH_REQUEST, self._handle_auth_request)
        ...
        self.message_router.register_handler(MessageType.SESSION_END, self._handle_session_end)

        # Interaction handlers
        self.message_router.register_handler(MessageType.USER_MESSAGE, self._handle_user_message)
        self.message_router.register_handler(
    ...
    async def start_server(self) -> None:
        ...
        try:
            ...
            self.server = await websockets.serve(
                self._handle_connection, self.host, self.port, ssl=ssl_context, **server_options
            )
    ...
    async def _handle_connection(self, websocket: WebSocketServerProtocol, path: str) -> None:
        ...
        try:
            ...
            await self.event_bus.emit(
                ConnectionEstablished(
                    connection_id=connection_id, client_ip=client_ip, user_agent=user_agent
                )
            )
    ...
    async def _connection_handler(self, connection: ConnectionInfo) -> None:
        ...
        try:
            ...
            # Wait for either task to complete
                ...
                [message_task, queue_task], return_when=asyncio.FIRST_COMPLETED
            )
            ...
            try:
                ...
                try:
                    ...
                    await self._send_error(
                        connection, f"Message parsing error: {str(e)}", "PARSE_ERROR"
                    )
                    continue
                ...
                try:
                    ...
                    target_connections = await self.message_router.route_message(
                        message, connection
                    )
                    ...
                    await self._send_error(
                        connection, f"Message handling error: {str(e)}", "HANDLER_ERROR"
                    )
                ...
                self.metrics.record(
                    "websocket_message_processing_duration_seconds", processing_time
                )
                ...
                self.logger.error(
                    f"Message processor error for {connection.connection_id}: {str(e)}"
                )
                break
    ...
    async def _send_message(self, connection: ConnectionInfo, message: WebSocketMessage) -> None:
        ...
        try:
            if connection.websocket and connection.state not in [
                ...
                if connection.is_authenticated and self.config.get(
                    "websocket.enable_encryption", False
                ):
    ...
    async def _send_error(
        self, connection: ConnectionInfo, error_message: str, error_code: str
    ) -> None:
    ...
    async def _send_ack(self, connection: ConnectionInfo, message_id: str) -> None:
        ...
        ack_message = WebSocketMessage(
            ...
            data={"ack_for": message_id, "timestamp": datetime.now(timezone.utc).isoformat()},
        )
    ...
    async def _handle_auth_request(
        self, message: WebSocketMessage, connection: ConnectionInfo
    ) -> None:
        ...
        try:
            ...
            if auth_result.get("valid"):
                ...
                if len(self.user_connections[user_id]) >= self.max_connections_per_user:
                    await self._send_error(
                        connection, "Too many connections for user", "CONNECTION_LIMIT"
                    )
                    return
                ...
                await self.event_bus.emit(
                    UserAuthenticated(
                        user_id=user_id, connection_id=connection.connection_id, roles=list(roles)
                    )
                )

                self.metrics.increment(
                    "websocket_authentication_attempts_total", tags={"status": "success"}
                )
                self.logger.info(
                    f"User {user_id} authenticated on connection {connection.connection_id}"
                )
    ...
    async def _handle_session_start(
        ...
        try:
            ...
            session_id = await self.session_manager.create_session(
                user_id=connection.user_id, session_config=message.data.get("config")
            )
            ...
            response = WebSocketMessage(
                ...
                data={"success": True, "session_id": session_id},
            )
            await self._send_message(connection, response)
        ...
        try:
            ...
            if not connection.current_interaction_id:
                interaction_id = await self.interaction_handler.start_interaction(
                    ...
                    interaction_mode=InteractionMode(
                        message.data.get("interaction_mode", "conversational")
                    ),
                )
            ...
            response_message = WebSocketMessage(
                ...
                data={
                    ...
                    "text": response.text,
                    "audio_data": (
                        response.audio_data.tolist() if response.audio_data is not None else None
                    ),
            ...
            await self._send_error(
                connection, f"Message processing error: {str(e)}", "PROCESSING_ERROR"
            )
    ...
    async def _handle_interaction_start(
        ...
        try:
            ...
            response = WebSocketMessage(
                ...
                data={"success": True, "interaction_id": interaction_id},
            )
            await self._send_message(connection, response)

        except Exception as e:
            await self._send_error(
                connection, f"Interaction start error: {str(e)}", "INTERACTION_ERROR"
            )

    async def _handle_interaction_end(
        self, message: WebSocketMessage, connection: ConnectionInfo
    ) -> None:
        ...
        try:
            ...
            await self._send_error(
                connection, f"Interaction end error: {str(e)}", "INTERACTION_ERROR"
            )
    ...
    async def _handle_workflow_execute(
        ...
        try:
            ...
            response = WebSocketMessage(
                ...
                data={"success": True, "execution_id": execution_id},
            )
            await self._send_message(connection, response)

        except Exception as e:
            await self._send_error(
                connection, f"Workflow execution error: {str(e)}", "WORKFLOW_ERROR"
            )
    ...
    async def _handle_heartbeat(
        ...
        response = WebSocketMessage(
            ...
            data={"timestamp": connection.last_heartbeat.isoformat(), "status": "alive"},
        )
        await self._send_message(connection, response)
    ...
    async def _handle_plugin_message(
        ...
        try:
            ...
            if not plugin_id or not plugin_message:
                await self._send_error(
                    connection, "Plugin ID and message required", "PLUGIN_INVALID"
                )
                return
        ...
        while True:
            try:
                ...
                for connection_id, connection in list(self.active_connections.items()):
                    try:
                        ...
                        if time_since_last_pong > timeout_threshold:
                            ...
                            self.logger.warning(
                                f"Connection {connection_id} timed out (no pong for {time_since_last_pong:.1f}s)"
                            )
                ...
                if self.active_connections:
                    self.logger.debug(
                        f"Heartbeat check completed: {len(self.active_connections)} active connections, {len(connections_to_remove)} removed"
                    )
