...
class RequestHandler(BaseHandler):
    ...
    def __init__(
        ...
        metadata = HandlerMetadata(
            handler_id=handler_id, handler_type=HandlerType.REQUEST, priority=priority
        )
        super().__init__(metadata)
...
class MiddlewareHandler(BaseHandler):
    ...
    def __init__(
        ...
        metadata = HandlerMetadata(
            handler_id=handler_id, handler_type=HandlerType.MIDDLEWARE, priority=priority
        )
        super().__init__(metadata)
...
class SecurityHandler(BaseHandler):
    ...
    async def handle(self, security_event: Any, context: HandlerContext) -> HandlerResult:
        ...
        try:
            ...
            if (
                hasattr(security_event, "user_id")
                and hasattr(security_event, "resource")
                and hasattr(security_event, "action")
                and self.authz_manager
            ):

                is_authorized = await self.authz_manager.check_permission(
                    security_event.user_id, security_event.resource, security_event.action
                )
...
class PerformanceHandler(BaseHandler):
        ...
        try:
            ...
            if hasattr(performance_event, "metric_name") and hasattr(performance_event, "value"):
                self.performance_data[performance_event.metric_name].append(
                    {"value": performance_event.value, "timestamp": datetime.now(timezone.utc)}
                )
    ...
    async def _handle_threshold_exceeded(self, event: Any) -> None:
        ...
        self.logger.warning(
            f"Performance threshold exceeded for {metric_name}: " f"{value} > {threshold}"
        )
...
class MultimodalHandler(BaseHandler):
    ...
    async def handle(
        self, multimodal_request: MultimodalInput, context: HandlerContext
    ) -> HandlerResult:
        ...
        try:
            ...
            processing_result = await self.core_engine.process_multimodal_input(
                multimodal_request, multimodal_request.context
            )
        ...
        try:
            ...
            if isinstance(learning_event, LearningEventOccurred):
                ...
                if (
                    ...
                    and hasattr(learning_event, "user_id")
                    and learning_event.user_id
                ):
...
class HandlerRegistry:
    ...
    async def unregister_handler(self, handler_id: str) -> None:
        ...
        async with self._registry_lock:
            ...
            self._handlers_by_type[handler.metadata.handler_type] = [
                h
                for h in self._handlers_by_type[handler.metadata.handler_type]
            ...
            for event_type in handler.metadata.event_types:
                self._handlers_by_event[event_type] = [
                    h
                    for h in self._handlers_by_event[event_type]
                    if h.metadata.handler_id != handler_id
                ]
    ...
    def get_handlers_for_event(self, event: Any) -> List[BaseHandler]:
        ...
        additional_handlers = [
            h for h in self._handlers.values() if h not in handlers and h.can_handle(event)
        ]
...
class EnhancedHandlerManager:
    ...
    def __init__(self, container: Container):
        ...
        self.thread_pool = ThreadPoolExecutor(max_workers=10, thread_name_prefix="handler_manager")
    ...
    async def handle_request(
        ...
        context = HandlerContext(
            ...
            + timedelta(seconds=timeout or self._default_timeout),
        )
    ...
    async def _execute_handlers(
        self, handlers: List[BaseHandler], data: Any, context: HandlerContext
    ) -> List[HandlerResult]:
        ...
        async with self.execution_semaphore:
            ...
            try:
                ...
                if self._enable_parallel_execution and len(handlers) > 1:
                    ...
                    tasks = [
                        self._execute_single_handler(handler, data, context) for handler in handlers
                    ]
                    ...
                    for handler in handlers:
                        ...
                        if (
                            not result.success
                            and handler.metadata.priority == HandlerPriority.CRITICAL
                        ):
                            break
    ...
    async def _execute_single_handler(
        self, handler: BaseHandler, data: Any, context: HandlerContext
    ) -> HandlerResult:
        ...
        while retry_count <= handler.metadata.max_retries:
            try:
                ...
                # Execute handler
                with (
                    self.tracer.trace(f"handler_{handler.metadata.handler_id}")
                    if self.tracer
                    else None
                ):
                ...
                if self.metrics:
                    self.metrics.increment(
                        "handler_executions_total", tags={"handler_id": handler.metadata.handler_id}
                    )
                    self.metrics.record(
                        "handler_execution_duration_seconds",
                        execution_time,
                ...
                if self.metrics:
                    self.metrics.increment(
                        "handler_errors_total", tags={"handler_id": handler.metadata.handler_id}
                    )
    ...
    def get_handler_status(self, handler_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if handler_id:
            ...
            return {
                ...
                "handlers_by_type": {
                    handler_type.value: len(
                        self.handler_registry.get_handlers_by_type(handler_type)
                    )
