    ...
    async def handle_message(self, connection_id: str, message: str):
        ...
        try:
            ...
            if message_type == "ping":
                ...
                status = (
                    self.assistant.get_status() if self.assistant else {"status": "unavailable"}
                )
                await self.send_to_connection(
                    connection_id, {"type": "status_response", "data": status}
                )
                ...
                if text and self.assistant:
                    ...
                    await self.send_to_connection(
                        connection_id, {"type": "text_response", "data": response}
                    )
            else:
                await self.send_to_connection(
                    connection_id,
    ...
    async def broadcast(self, data: Dict):
        ...
        if self.connections:
            ...
            await asyncio.gather(
                *[conn.send(message) for conn in self.connections.values()], return_exceptions=True
            )
