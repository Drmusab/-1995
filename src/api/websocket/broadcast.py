...
class BroadcastManager:
    ...
    def __init__(
        self,
        redis_cache: Optional[Any] = None,  # Changed from RedisCache to Any
    ...
    async def initialize(self) -> None:
        ...
        if self.redis_cache:
            try:
                ...
                self.redis_handler_task = asyncio.create_task(self._redis_message_handler())
    ...
    def register_filter(
        self, name: str, filter_func: Callable, description: str = "", enabled: bool = True
    ) -> None:
        """
        ...
        self.broadcast_filters[name] = BroadcastFilter(
            name=name, filter_func=filter_func, description=description, enabled=enabled
        )
    ...
    async def _should_include_connection(
        self,
        scope: BroadcastScope,
        connection: Any,
        filter_criteria: Optional[Dict[str, Any]] = None,
    ...
    async def _apply_custom_filters(self, connection: Any, filter_criteria: Dict[str, Any]) -> bool:
    ...
    async def _distribute_via_redis(
        self,
        message: Any,
        scope: BroadcastScope,
        filter_criteria: Optional[Dict[str, Any]] = None,
        ...
        try:
            distributed_message = {
                "message": (
                    message.to_json() if hasattr(message, "to_json") else json.dumps(message)
                ),
                "scope": scope.value,
                ...
                if message["type"] == "message":
                    try:
                        ...
                        if message_type == "user_broadcast":
                            ...
                            self.logger.warning(
                                f"Unknown distributed broadcast type: {message_type}"
                            )
...
async def broadcast_to_all(
    manager: BroadcastManager, message: Any, connections: Dict[str, Any]
) -> int:
...
async def broadcast_to_session(
    manager: BroadcastManager, message: Any, connections: Dict[str, Any], session_id: str
) -> int:
    """Broadcast message to all connections in a specific session."""
    return await manager.broadcast(
        message, BroadcastScope.SESSION, connections, filter_criteria={"session_id": session_id}
    )


async def broadcast_to_user(
    manager: BroadcastManager, message: Any, connections: Dict[str, Any], user_id: str
) -> int:
    """Broadcast message to all connections for a specific user."""
    return await manager.broadcast(
        message, BroadcastScope.USER, connections, filter_criteria={"user_id": user_id}
    )


async def broadcast_to_role(
    manager: BroadcastManager, message: Any, connections: Dict[str, Any], role: str
) -> int:
    """Broadcast message to all users with a specific role."""
    return await manager.broadcast(
        message, BroadcastScope.ROLE, connections, filter_criteria={"role": role}
    )
