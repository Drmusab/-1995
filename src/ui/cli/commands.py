...
class CommandRegistry:
    ...
    def register(self, name: str, handler: Callable, help_text: str, group: str = "general",
...
class SystemCommands(CommandHandler):
    ...
    async def status(self, args: argparse.Namespace) -> int:
        ...
        if not component_manager or not health_check:
            error_console.print("[bold red]Error:[/] Component Manager or Health Check not available")
            return 1
        ...
        status_text = "[bold green]System is healthy[/]" if overall_health else "[bold red]System has issues[/]"
        console.print(f"\nOverall: {status_text}")
    ...
    async def info(self, args: argparse.Namespace) -> int:
        ...
        for name, component_info in registered_components.items():
            status = "[green]Active[/]" if component_info.get("initialized", False) else "[yellow]Registered[/]"
            component_type = component_info.get("type", "Unknown")
            components_table.add_row(name, status, component_type.__name__ if hasattr(component_type, "__name__") else str(component_type))
    ...
    async def config(self, args: argparse.Namespace) -> int:
        ...
        if args.action == "show":
            ...
            if not args.path or not args.value:
                error_console.print("[bold red]Error:[/] Both path and value are required for 'set' action")
                return 1
    ...
    async def create_session(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            session_id = await session_manager.create_session(
                ...
                metadata={"source": "cli", "created_by": "command"}
            )
    ...
    async def show_session(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            session_info = {
                ...
                "Last Active": datetime.fromisoformat(session.last_activity).strftime("%Y-%m-%d %H:%M:%S"),
            ...
            if args.history:
                ...
                if not history:
                    ...
                    for interaction in history:
                        timestamp = datetime.fromisoformat(interaction.get("timestamp", "")).strftime("%H:%M:%S")
    ...
    async def chat(self, args: argparse.Namespace) -> int:
        ...
        if not session_id:
            ...
            try:
                session_id = await session_manager.create_session(
                    user_id=args.user_id,
                    metadata={"source": "cli", "interface": "chat"}
                )
    ...
    async def send(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            response = await interaction_handler.handle_text_input(
                text=message,
                session_id=session_id,
                user_id=args.user_id or session.user_id
            )
        ...
        try:
            ...
            if memory.metadata:
                ...
                metadata = {
                    ...
                    "Last Accessed": memory.metadata.last_accessed.strftime("%Y-%m-%d %H:%M:%S") if memory.metadata.last_accessed else "Never",
    ...
    async def export_memories(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            for memory in memories:
                ...
                if memory.metadata:
                    memory_dict["metadata"] = {
                        ...
                        "last_accessed": memory.metadata.last_accessed.isoformat() if memory.metadata.last_accessed else None,
            ...
            if export_format == "json":
                ...
                error_console.print(f"[bold red]Error:[/] Unsupported export format: {export_format}")
                return 1
            
            console.print(f"[bold green]Successfully exported {len(export_data)} memories to {output_path}[/]")
            return 0
...
class SkillCommands(CommandHandler):
        ...
        try:
            ...
            for category, skills_list in sorted(categories.items()):
                ...
                for name, info in sorted(skills_list, key=lambda x: x[0]):
                    status = "[green]Installed[/]" if info.get("installed", False) else "[yellow]Available[/]"
    ...
    async def install_skill(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            with Progress(
                SpinnerColumn(),
                TextColumn("[bold blue]Installing skill...[/]"),
                console=console
            ) as progress:
            ...
            if result:
                ...
                error_console.print(f"[bold red]Error:[/] Failed to install skill {args.skill_name}")
                return 1
    ...
    async def uninstall_skill(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            if not args.force:
                confirm = input(f"Are you sure you want to uninstall skill {args.skill_name}? (y/N): ")
                if confirm.lower() != 'y':
            ...
            with Progress(
                SpinnerColumn(),
                TextColumn("[bold blue]Uninstalling skill...[/]"),
                console=console
            ) as progress:
            ...
            if result:
                ...
                error_console.print(f"[bold red]Error:[/] Failed to uninstall skill {args.skill_name}")
                return 1
...
class ModelCommands(CommandHandler):
        ...
        try:
            ...
            for provider, provider_models in sorted(providers.items()):
                ...
                for model in sorted(provider_models, key=lambda x: x.get("id", "")):
                    ...
                    status = "[green]Available[/]" if model.get("available", False) else "[yellow]Unavailable[/]"
    ...
    async def set_default_model(self, args: argparse.Namespace) -> int:
        ...
        try:
            # Set default model
            success = await model_router.set_default_model(
                task=args.task,
                model_id=args.model_id
            )
            
            if success:
                console.print(f"[bold green]Successfully set default model for {args.task} to {args.model_id}[/]")
                return 0
            else:
...
class UtilityCommands(CommandHandler):
    ...
    async def benchmark(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            with Progress() as progress:
                task = progress.add_task(f"[blue]Running {args.type} benchmark...", total=args.iterations)
                ...
                for i in range(args.iterations):
                    ...
                    if args.type == "memory":
                        ...
                        await model_router.get_embeddings("This is a test sentence for embedding generation.")
                    else:
                        error_console.print(f"[bold red]Error:[/] Unsupported benchmark type: {args.type}")
                        return 1
                    ...
                    # Update progress
                    progress.update(task, advance=1, description=f"[blue]Running {args.type} benchmark... {i+1}/{args.iterations}")
    ...
    async def diagnostics(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            console.print(f"Summary: {healthy_count} healthy, {unhealthy_count} unhealthy components")
