...
# Assistant imports
from src.assistant.core import CoreAssistantEngine as CoreEngine
from src.assistant.core import InteractionHandler
from src.assistant.core import EnhancedPluginManager as PluginManager
...
# Integration imports
# Integration imports - make optional to avoid blocking
try:
...
# Memory imports - make optional to avoid blocking
try:
...
# Observability imports - make optional to avoid blocking
try:
...
# Skills imports - make optional to avoid blocking
try:
    from src.skills.skill_registry import SkillRegistry
except ImportError:
    SkillRegistry = None

# Local imports - make optional to avoid blocking
try:
...
class CommandRegistry:
    ...
    def register(
        ...
        Args:
            command_or_name: Either a Command object or command name string
            handler: Command handler function (if name provided)
            help_text: Help text (if name provided)
            group: Command group
    ...
    def get_handler(self, name: str) -> Optional[Callable]:
        ...
        Returns:
            Command handler function if found, None otherwise
        """
        # Check direct command name first
        if name in self.commands:
            return self.commands[name]["handler"]
