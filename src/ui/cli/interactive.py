...
from src.core.events.event_types import (
    UserMessageReceived, AssistantMessageSent, InteractionCompleted,
    SessionCreated, SessionClosed, ErrorOccurred
)
...
class InteractiveSession:
    ...
    def _completer(self, text: str, state: int) -> Optional[str]:
        ...
        if text.startswith(self.command_prefix):
            ...
            matches = [
                f"{self.command_prefix}{cmd}" for cmd in self.commands.keys()
        ...
        if not self.session_id:
            try:
                self.session_id = await self.session_manager.create_session(
                    ...
                    metadata={"source": "cli", "interface": "interactive"}
                )
    ...
    async def _show_welcome(self) -> None:
        ...
        console.print(f"[dim]Started at: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}[/]")
        console.print("\n" + "─" * console.width + "\n")
    ...
    async def _process_user_message(self, message: str) -> None:
        ...
        # Emit event
        await self.event_bus.emit(UserMessageReceived(
        ...
        try:
            ...
            # Emit event
            await self.event_bus.emit(AssistantMessageSent(
                session_id=self.session_id,
            ...
            await self.event_bus.emit(InteractionCompleted(
                session_id=self.session_id,
                user_id=self.user_id,
    ...
    async def _cmd_help(self, args: List[str]) -> None:
        ...
        if args:
            ...
            if cmd_name in self.commands:
                ...
                # Show aliases if any
                if command.aliases:
                    aliases = ", ".join(f"{self.command_prefix}{alias}" for alias in command.aliases)
            ...
            for cmd_type, cmds in by_type.items():
                ...
                for cmd in sorted(cmds, key=lambda c: c.name):
                    ...
                    table.add_row(
                        f"{self.command_prefix}{cmd.name}",
                        cmd.help_text,
                        aliases
                    )
    ...
    async def _cmd_context(self, args: List[str]) -> None:
        ...
        if not args:
                ...
                if isinstance(content, dict):
                    ...
                    for item in content:
                        if isinstance(item, dict):
                            ...
                            summary = ", ".join(f"{k}: {v}" for k, v in item.items() if not isinstance(v, (dict, list)))
            ...
            if subcommand == "clear":
                ...
                # Add context element
                from src.memory.operations.context_manager import ContextType, ContextPriority
                try:
    ...
    async def _cmd_upload(self, args: List[str]) -> None:
        ...
        try:
            ...
            if file_ext in (".txt", ".md", ".py", ".js", ".html", ".css", ".json", ".yaml", ".yml"):
                ...
                # Image file
                file_type = "image"
                error_console.print("[bold yellow]Note:[/] Image processing not implemented in this version")
                return
            elif file_ext in (".pdf", ".doc", ".docx"):
                # Document file
                file_type = "document"
                error_console.print("[bold yellow]Note:[/] Document processing not implemented in this version")
                return
            else:
    ...
    async def _cmd_settings(self, args: List[str]) -> None:
        ...
        if not args:
            ...
            table.add_row(
                "stream_output",
                str(self.stream_output),
                "Stream assistant responses"
            )
            table.add_row(
                "multiline_input",
                str(self.multiline_input),
            ...
            if subcommand == "set" and len(args) >= 3:
                ...
                error_console.print(f"[bold red]Error:[/] Unknown settings subcommand: {subcommand}")
    ...
    async def _cmd_multiline(self, args: List[str]) -> None:
        ...
        console.print(f"[bold green]Multiline input mode: {'enabled' if self.multiline_input else 'disabled'}[/]")
    ...
    async def _cmd_info(self, args: List[str]) -> None:
        ...
        system_table.add_row("Current Time (UTC)", datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"))
