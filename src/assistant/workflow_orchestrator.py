...
from src.core.events.event_bus import (  # FIXED: Use EnhancedEventBus instead of EventBus
    EnhancedEventBus,
)
...
class ComponentStepExecutor(StepExecutor):
    ...
    def can_execute(self, step: WorkflowStep) -> bool:
        ...
        return step.step_type == StepType.DATA_PROCESSING and step.component_name is not None
...
class DecisionStepExecutor(StepExecutor):
    ...
    async def execute(self, step: WorkflowStep, context: Dict[str, Any]) -> Any:
        ...
        # Evaluate conditions
        decision_result = {"conditions_met": [], "next_steps": []}
...
class WorkflowBuilder:
    ...
    def __init__(self):
        self.workflow = WorkflowDefinition(workflow_id=str(uuid.uuid4()), name="New Workflow")

    def set_basic_info(
        self, workflow_id: str, name: str, description: str = None, version: str = "1.0.0"
    ) -> "WorkflowBuilder":
    ...
    def add_decision_step(
        self, step_id: str, name: str, conditions: List[WorkflowCondition], **kwargs
    ) -> "WorkflowBuilder":
    ...
    def set_flow(
        self, from_step: str, to_step: str, condition: WorkflowCondition = None
    ) -> "WorkflowBuilder":
        ...
        for step in self.workflow.steps.values():
            for next_step in step.next_steps:
                if next_step not in all_step_ids:
                    raise WorkflowError(
                        f"Step {step.step_id} references non-existent step {next_step}"
                    )
...
class WorkflowOrchestrator:
    ...
    def _create_builtin_templates(self) -> None:
        ...
        qa_workflow = (
            WorkflowBuilder()
            .set_basic_info(
                "builtin_qa", "Question Answering", "Simple question answering workflow"
            )
            .add_component_step(
                "intent_detection", "intent_manager", "detect_intent", "Detect User Intent"
            )
            .add_skill_step("knowledge_retrieval", "knowledge_search", "Retrieve Knowledge")
            .add_skill_step("response_generation", "text_generation", "Generate Response")
        ...
        multimodal_workflow = (
            WorkflowBuilder()
            .set_basic_info(
                "builtin_multimodal", "Multimodal Processing", "Process multimodal input"
            )
            .add_component_step(
                "speech_processing", "speech_processor", "process", "Process Speech Input"
            )
            .add_component_step(
                "vision_processing", "vision_processor", "process", "Process Vision Input"
            )
            .add_component_step(
                "text_processing", "text_processor", "process", "Process Text Input"
            )
            .add_skill_step("multimodal_fusion", "fusion_skill", "Fuse Modalities")
            .add_skill_step("response_generation", "response_generator", "Generate Response")
            ...
            for execution_id, execution in list(self.active_executions.items()):
                if execution.state == WorkflowState.RUNNING:
                    execution.warnings.append(
                        f"System error in {event.component_name}: {event.error_message}"
                    )
        ...
        async with self.execution_semaphore:
            ...
            try:
                with self.tracer.trace("workflow_execution") as span:
                    ...
                    self.metrics.record(
                        "workflow_execution_duration_seconds", execution.execution_time
                    )
    ...
    async def _execute_step(self, step: WorkflowStep, execution: WorkflowExecution) -> Any:
        ...
        async with self.step_semaphore:
            ...
            try:
                ...
                if not executor:
                    raise WorkflowError(
                        f"No executor found for step {step.step_id} of type {step.step_type}"
                    )

                # Execute step with timeout
                result = await asyncio.wait_for(
                    executor.execute(step, execution.context), timeout=step.timeout_seconds
                )
                ...
                self.step_performance[f"{step.step_type.value}_{step.step_id}"].append(
                    step.execution_time
                )
                ...
                self.logger.error(
                    f"Step {step.step_id} failed after {step.retry_count} retries: {str(e)}"
                )
                raise
    ...
    async def _store_execution_for_learning(
        ...
        try:
            ...
            await self.memory_manager.store_episodic_memory(
                event_type="workflow_execution", data=learning_data, session_id=execution.session_id
            )
    ...
    async def create_workflow_from_intent(
        self, intent: str, context: Dict[str, Any], session_id: str
    ) -> str:
        """
        ...
        try:
            ...
            execution_id = await self.execute_workflow(workflow.workflow_id, session_id, context)
