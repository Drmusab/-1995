...
class ProcessorProxy:
    ...
    async def process(self, input_data: Any, context: Optional[Dict] = None) -> Any:
        ...
        if self.state != ProcessorState.READY:
            raise RuntimeError(
                f"Processor {self.processor.processor_id} not ready (state: {self.state})"
            )
    ...
    async def health_check(self) -> ProcessorHealth:
        ...
        try:
            ...
            base_health.avg_processing_time = (
...
class ProcessorManager:
    ...
    async def register_processor(
        self, processor: ProcessorInterface, config: Optional[Dict[str, Any]] = None
    ) -> None:
    ...
    async def get_processors_by_type(self, processor_type: ProcessorType) -> List[ProcessorProxy]:
        ...
        return [
            proxy
            for proxy in self.processors.values()
            if proxy.processor.processor_type == processor_type
        ]

    async def process_with_processor(
        self, processor_id: str, input_data: Any, context: Optional[Dict] = None
    ) -> Any:
    ...
    async def process_with_type(
        ...
        if load_balance and len(processors) > 1:
            best_processor = min(processors, key=lambda p: p.error_count / max(p.success_count, 1))
        else:
                ...
                for processor_id, health in health_status.items():
                    if health.state == ProcessorState.ERROR:
                        ...
                        if health.error_count > self.max_retries:
                            self.logger.warning(
                                f"Attempting to restart unhealthy processor: {processor_id}"
                            )
                            try:
                                await self.restart_processor(processor_id)
                            except Exception as e:
                                self.logger.error(
                                    f"Failed to restart processor {processor_id}: {e}"
                                )
        ...
        try:
            ...
            if processor.processor_id in self.processors:
                await self._shutdown_processor(
                    processor.processor_id, self.processors[processor.processor_id]
                )
