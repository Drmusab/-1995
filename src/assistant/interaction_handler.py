...
from src.core.events.event_types import (
    ...
    MessageSent,
    ModalityDetected,
    RealTimeProcessingCompleted,
    RealTimeProcessingStarted,
...
from src.processing.speech.emotion_detection import AnalysisMode, EmotionDetectionRequest
from src.processing.speech.speech_to_text import TranscriptionQuality, TranscriptionRequest
...
class UserProfile:
    ...
    preferred_input_modalities: Set[InputModality] = field(
        default_factory=lambda: {InputModality.TEXT}
    )
    preferred_output_modalities: Set[OutputModality] = field(
        default_factory=lambda: {OutputModality.TEXT}
    )
...
class OutputGenerator(ABC):
    ...
    @abstractmethod
    async def generate(
        self, content: str, modalities: Set[OutputModality], context: InteractionContext
    ) -> AssistantResponse:
...
class TextInputProcessor(InputProcessor):
    ...
    async def process(self, message: UserMessage, context: InteractionContext) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence": max(
                intent_result.get("confidence", 0.0), sentiment_result.get("confidence", 0.0)
            ),
        }
...
class InteractionHandler:
    ...
    def _setup_processors(self) -> None:
        ...
        try:
            ...
            self.input_processors = [
                TextInputProcessor(
                    self.intent_manager, self.sentiment_analyzer, self.input_sanitizer
                ),
            ...
            self.logger.info(
                f"Initialized {len(self.input_processors)} input processors and "
    ...
    async def start_interaction(
        ...
        if interaction_mode == InteractionMode.CONVERSATIONAL:
            ...
            await self.event_bus.emit(
                ConversationStarted(
                    conversation_id=conversation_id, session_id=session_id, user_id=user_id
                )
            )
    ...
    async def _process_user_input(
        self, message: UserMessage, context: InteractionContext
    ) -> Dict[str, Any]:
    ...
    async def _update_interaction_context(
        self, context: InteractionContext, message: UserMessage, processed_input: Dict[str, Any]
    ) -> None:
    ...
    async def _process_standard(
        self, processed_input: Dict[str, Any], context: InteractionContext
    ) -> AssistantResponse:
        ...
        processing_result = await self.core_engine.process_multimodal_input(
            multimodal_input, multimodal_input.context
        )
    ...
    async def _process_real_time(
        ...
        await self.event_bus.emit(
            RealTimeProcessingStarted(
                interaction_id=context.interaction_id, session_id=context.session_id
            )
        )
        ...
        await self.event_bus.emit(
            RealTimeProcessingCompleted(
                interaction_id=context.interaction_id, processing_time=response.processing_time
            )
        )
    ...
    async def _process_streaming(
        ...
        await self.event_bus.emit(
            StreamingStarted(interaction_id=context.interaction_id, session_id=context.session_id)
        )
    ...
    async def _generate_response(
        ...
        response = await generator.generate(
            processing_result.response_text or "I understand.", context.output_modalities, context
        )
    ...
    async def _store_interaction_memory(
        self, context: InteractionContext, message: UserMessage, response: AssistantResponse
    ) -> None:
        ...
        try:
            ...
            await self.working_memory.update(
                context.session_id,
                {"last_interaction": interaction_data, "context": context.__dict__},
            )
    ...
    async def _update_conversation_history(
        ...
        if context.conversation_id:
            history_entry = {
                ...
                "user_message": {"text": message.text, "modality": message.modality.value},
                "assistant_response": {
                    "text": response.text,
                    "modalities": [m.value for m in response.modalities],
                    "confidence": response.confidence,
        ...
        try:
            ...
            if not is_authorized:
                raise InteractionError(
                    f"User {user_id} not authorized for interactions", interaction_id
                )
        ...
        while True:
            try:
                ...
                for interaction_id, context in self.active_interactions.items():
                    ...
                    ).total_seconds() > self.max_interaction_duration:
                        expired_interactions.append(interaction_id)
                ...
                for interaction_id in expired_interactions:
                    try:
                        ...
                        self.logger.error(
                            f"Failed to cleanup interaction {interaction_id}: {str(e)}"
                        )
        ...
        while True:
            try:
                ...
                for interaction_id, context in self.active_interactions.items():
                    if (
                        context.user_id
                        and context.interaction_mode == InteractionMode.CONVERSATIONAL
                    ):
    ...
    async def update_user_preferences(self, user_id: str, preferences: Dict[str, Any]) -> None:
        ...
        await self.event_bus.emit(UserPreferenceUpdated(user_id=user_id, preferences=preferences))
