...
from src.core.performance_utils import LRUCache, PerformanceTimer, async_cache
...
class EnhancedCoreEngine:
    ...
    def _setup_threading(self) -> None:
        ...
        self.thread_pool = ThreadPoolExecutor(
            max_workers=self.config.max_concurrent_requests, thread_name_prefix="core_engine"
        )
    ...
    def _register_core_components(self) -> None:
        ...
        self.component_manager.register_component(
            ...
            priority=ComponentPriority.ESSENTIAL,
            dependencies=[ComponentDependency("session_manager", DependencyType.REQUIRED)],
    ...
    def _register_processing_components(self) -> None:
        ...
        if self.config.enable_speech_processing:
            ...
            self.component_manager.register_component(
                ...
                priority=ComponentPriority.NORMAL,
                dependencies=[ComponentDependency("audio_pipeline", DependencyType.OPTIONAL)],
        ...
        if self.config.enable_vision_processing:
            ...
            self.component_manager.register_component(
                ...
                priority=ComponentPriority.NORMAL,
                dependencies=[ComponentDependency("vision_processor", DependencyType.REQUIRED)],
    ...
    def _register_reasoning_components(self) -> None:
        ...
        if self.config.enable_reasoning:
            ...
            self.component_manager.register_component(
                ...
                priority=ComponentPriority.HIGH,
                dependencies=[ComponentDependency("model_router", DependencyType.REQUIRED)],
            ...
            self.component_manager.register_component(
                ...
                priority=ComponentPriority.HIGH,
                dependencies=[ComponentDependency("logic_engine", DependencyType.REQUIRED)],
    ...
    def _register_memory_systems(self) -> None:
        ...
        self.component_manager.register_component(
            ...
            WorkingMemory,
            priority=ComponentPriority.HIGH,
            dependencies=[ComponentDependency("vector_store", DependencyType.REQUIRED)],
        ...
        self.component_manager.register_component(
            ...
            MemoryRetriever,
            priority=ComponentPriority.HIGH,
            dependencies=[ComponentDependency("memory_manager", DependencyType.REQUIRED)],
    ...
    async def _get_component_references(self) -> None:
        ...
        self.workflow_orchestrator = await self.component_manager.get_component(
            "workflow_orchestrator"
        )
        ...
        if self.config.enable_speech_processing:
            try:
                ...
                self.emotion_detector = await self.component_manager.get_component(
                    "emotion_detector"
                )
                self.speaker_recognition = await self.component_manager.get_component(
                    "speaker_recognition"
                )
            except Exception as e:
        ...
        if self.config.enable_vision_processing:
            try:
                self.vision_processor = await self.component_manager.get_component(
                    "vision_processor"
                )
        ...
        try:
            ...
            self.sentiment_analyzer = await self.component_manager.get_component(
                "sentiment_analyzer"
            )
        ...
        if self.config.enable_learning:
            try:
                self.continual_learner = await self.component_manager.get_component(
                    "continual_learner"
                )
                self.preference_learner = await self.component_manager.get_component(
                    "preference_learner"
                )
                self.feedback_processor = await self.component_manager.get_component(
                    "feedback_processor"
                )
    ...
    async def _initialize_memory_integration(self) -> None:
        ...
        if self.config.enable_memory_enhancement:
            try:
                ...
                self.memory_integrator = await self.component_manager.get_component(
                    "session_memory_integrator"
                )
                self.memory_retriever = await self.component_manager.get_component(
                    "memory_retriever"
                )
                self.memory_context_manager = await self.component_manager.get_component(
                    "memory_context_manager"
                )
    ...
    async def process_multimodal_input(
        self, input_data: MultimodalInput, context: Optional[ProcessingContext] = None
    ) -> ProcessingResult:
        """
        ...
        if context is None:
            context = ProcessingContext(session_id=str(uuid.uuid4()), request_id=str(uuid.uuid4()))
                ...
                async with PerformanceTimer("multimodal_processing") as timer:
                    ...
                    await self.event_bus.emit(
                        ProcessingStarted(
                            ...
                            input_modalities=[
                                k
                                for k, v in asdict(input_data).items()
                    ...
                    if context.memory_enhanced and self.config.enable_memory_enhancement:
                        memory_context = await self._get_memory_context(
                            context.session_id, input_data.text or "", context.user_id
                        )
                    ...
                    # Memory operations
                    memory_result = await self._perform_memory_operations(
                        input_data, context, result
                    )
                    ...
                    response_result = await self._generate_response(
                        input_data, context, result, memory_context
                    )
                    ...
                    if self.config.enable_memory_enhancement:
                        await self._extract_and_store_knowledge(
                            result.response_text or "", context.session_id, context.user_id
                        )
    ...
    async def process_message(
        ...
        input_data = MultimodalInput(text=message, context=context)
        ...
        response = {
            ...
            "context_used": (
                len(result.memory_context.context_elements) if result.memory_context else 0
            ),
    ...
    async def _get_memory_context(
        ...
        try:
            ...
            memory_context = MemoryContext(
                ...
                semantic_memories=[
                    {"content": memory.content, "relevance": score, "memory_id": memory.memory_id}
                ...
                episodic_memories=[
                    {
                        ...
                        "timestamp": (
                            memory.timestamp.isoformat() if hasattr(memory, "timestamp") else None
                        ),
                    }
            ...
            self.metrics.record(
                "memory_retrieval_time", memory_context.retrieval_metadata["retrieval_time"]
            )
    ...
    async def _process_modalities(
        self, input_data: MultimodalInput, context: ProcessingContext, result: ProcessingResult
    ) -> Dict[str, Any]:
        ...
        if input_data.audio is not None and self.config.enable_speech_processing:
            modality_tasks.append(self._process_speech_modality(input_data.audio, context, result))

        # Vision processing
        if input_data.image is not None and self.config.enable_vision_processing:
            modality_tasks.append(self._process_vision_modality(input_data.image, context, result))

        # Text processing
        if input_data.text is not None:
            modality_tasks.append(self._process_text_modality(input_data.text, context, result))
    ...
    async def _process_speech_modality(
        self, audio: np.ndarray, context: ProcessingContext, result: ProcessingResult
    ) -> Dict[str, Any]:
        ...
        try:
            await self.event_bus.emit(
                ModalityProcessingStarted(
                    session_id=context.session_id, modality="speech", request_id=context.request_id
                )
            )
            ...
            if hasattr(self, "emotion_detector"):
                ...
                emotion_result = await self.emotion_detector.detect_emotion(audio, emotion_request)
    ...
    async def _process_vision_modality(
        self, image: np.ndarray, context: ProcessingContext, result: ProcessingResult
    ) -> Dict[str, Any]:
        ...
        try:
            await self.event_bus.emit(
                ModalityProcessingStarted(
                    session_id=context.session_id, modality="vision", request_id=context.request_id
                )
            )
    ...
    async def _process_text_modality(
        self, text: str, context: ProcessingContext, result: ProcessingResult
    ) -> Dict[str, Any]:
        """Process text input."""
        text_results = {}
        ...
        try:
            await self.event_bus.emit(
                ModalityProcessingStarted(
                    session_id=context.session_id, modality="text", request_id=context.request_id
                )
            )
    ...
    async def _perform_multimodal_fusion(
        ...
        try:
            ...
            fusion_result = await self.fusion_strategy.fuse_modalities(
                modality_results, weights=input_data.modality_weights
            )
