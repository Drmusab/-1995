...
class SessionMemoryIntegrator:
    ...
    async def _handle_session_started(self, event: SessionStarted):
        ...
        try:
            ...
            context = SessionMemoryContext(session_id=session_id, user_id=user_id)
        ...
        try:
            ...
            if self.episodic_memory:
                recent_episodes = await self.episodic_memory.retrieve_by_user(
                    user_id=user_id, limit=5, days_back=self.memory_retention_days
                )
            ...
            if self.semantic_memory:
                user_facts = await self.semantic_memory.retrieve_by_user(user_id=user_id, limit=10)
    ...
    async def _consolidate_session_memories(
        self, context: SessionMemoryContext, duration: float, summary: Dict[str, Any]
    ):
        ...
        try:
            ...
            session_summary = {
                ...
                "interaction_count": len(
                    [
                        e
                        for e in context.context_elements
    ...
    def get_session_statistics(self) -> Dict[str, Any]:
        ...
        total_retrievals = sum(
            ctx.memory_retrieval_count for ctx in self._session_contexts.values()
        )
