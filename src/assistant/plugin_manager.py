...
from src.skills.skill_factory import SkillExecutionContext, SkillFactory

# Import skill registry and validator
from src.skills.skill_registry import SkillInterface, SkillMetadata, SkillRegistry, SkillType
...
class PluginError(Exception):
    ...
    def __init__(
        self, message: str, plugin_id: Optional[str] = None, error_code: Optional[str] = None
    ):
...
class PluginSandbox:
    ...
    def __init__(
        self, plugin_id: str, security_level: SecurityLevel, resource_limits: Dict[str, Any]
    ):
        self.plugin_id = plugin_id
...
class PluginValidator:
    ...
    async def validate_plugin(
        ...
        if security_issues:
            self.logger.warning(
                f"Security issues found in plugin {metadata.plugin_id}: {security_issues}"
            )
        ...
        if metadata.plugin_type == PluginType.SKILL:
            ...
            spec = importlib.util.spec_from_file_location(
                f"plugin_temp_{metadata.plugin_id}", plugin_path / "__init__.py"
            )
            ...
            if main_class:
                ...
                report = await self.skill_validator.validate_skill(
                    metadata.plugin_id, main_class, skill_metadata
                )
    ...
    def _create_basic_validation_report(
        self, metadata: PluginMetadata, security_issues: List[str]
    ) -> ValidationReport:
...
class PluginLoader:
    ...
    async def load_plugin_from_path(
        self, plugin_path: Path, validate: bool = True
    ) -> Optional[Any]:
    ...
    async def register_plugin_as_skill(self, plugin_info: PluginInfo) -> bool:
        ...
        try:
            ...
            success = await self.skill_registry.register_skill(
                plugin_info.metadata.plugin_id, type(plugin_info.instance), skill_metadata
            )
    ...
    async def unregister_plugin_skill(self, plugin_info: PluginInfo) -> bool:
        ...
        try:
            success = await self.skill_registry.unregister_skill(
                plugin_info.skill_id, reason="plugin_unloaded"
            )
...
class EnhancedPluginManager:
        ...
        try:
            ...
            if self.validation_required:
                try:
                    ...
                    self.logger.error(
                        f"Plugin validation failed for {metadata.plugin_id}: {str(e)}"
                    )
            ...
            self.logger.info(
                f"Discovered plugin: {metadata.plugin_id} (validated: {validation_passed})"
            )
    ...
    async def load_plugin(self, plugin_id: str) -> None:
        ...
        try:
            ...
            if (
                self.enable_sandboxing
                and plugin_info.metadata.security_level <= SecurityLevel.SANDBOX
            ):
            ...
            module = await self.plugin_loader.load_plugin_from_path(
                plugin_info.installation_path, validate=False  # Already validated during discovery
            )
    ...
    async def enable_plugin(self, plugin_id: str) -> None:
        ...
        if plugin_info.instance:
            ...
            if plugin_info.skill_registered:
                await self.skill_registry.update_skill_state(plugin_info.skill_id, SkillType.ACTIVE)
        ...
        if plugin_info.instance:
            ...
            if plugin_info.skill_registered:
                await self.skill_registry.update_skill_state(
                    plugin_info.skill_id, SkillType.INACTIVE
                )
    ...
    async def unload_plugin(self, plugin_id: str, force: bool = False) -> None:
        ...
        try:
            ...
            await self.event_bus.emit(
                ComponentUnregistered(
                    component_id=plugin_id, component_type="plugin", reason="unloaded"
                )
            )
    ...
    async def get_plugin_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "plugins_by_type": {pt.value: len(self.plugins_by_type[pt]) for pt in PluginType},
            "plugins_by_state": {},
        ...
        for plugin_info in self.plugins.values():
            ...
            status["plugins_by_state"][state_name] = (
                status["plugins_by_state"].get(state_name, 0) + 1
            )
                ...
                for plugin_id, plugin_info in self.plugins.items():
                    if plugin_info.state == PluginState.ENABLED and plugin_info.instance:
                        try:
                            ...
                            plugin_info.error_count += 1
                            self.logger.error(
                                f"Health check failed for plugin {plugin_id}: {str(e)}"
                            )
                ...
                for plugin_id, plugin_info in self.plugins.items():
                    if plugin_info.installation_path:
                        ...
                        if plugin_id in plugin_mtimes and current_mtime > plugin_mtimes[plugin_id]:
                            self.logger.info(
                                f"Detected changes in plugin {plugin_id}, hot reloading..."
                            )
            ...
            try:
                ...
                for plugin_id, plugin_info in list(self.plugins.items()):
                    if plugin_info.state == PluginState.ERROR and plugin_info.error_count > 5:
                        self.logger.warning(
                            f"Removing failed plugin {plugin_id} after {plugin_info.error_count} errors"
                        )
                        await self.unload_plugin(plugin_id, force=True)
        ...
        for plugin_id, plugin_info in self.plugins.items():
            if component_id in [dep.plugin_id for dep in plugin_info.metadata.dependencies]:
                if plugin_info.state == PluginState.DISCOVERED:
                    ...
                    self.logger.info(
                        f"Dependency {component_id} available, loading plugin {plugin_id}"
                    )
                    await self.load_plugin(plugin_id)
    ...
    async def _health_check_callback(self) -> Dict[str, Any]:
        ...
        try:
            total_plugins = len(self.plugins)
            enabled_plugins = sum(
                1 for p in self.plugins.values() if p.state == PluginState.ENABLED
            )
