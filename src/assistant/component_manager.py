...
class ComponentError(Exception):
    ...
    def __init__(
        self, message: str, component_id: Optional[str] = None, error_code: Optional[str] = None
    ):
...
class ComponentFactory:
    ...
    def create_component(
        self, component_type: Type, config: Optional[Dict[str, Any]] = None
    ) -> Any:
...
class ComponentMonitor:
    ...
    async def start_monitoring(
        self, component_id: str, interval: float, health_callback: Callable
    ) -> None:
        ...
        while True:
            try:
                ...
                if component_id in self._health_callbacks:
                    ...
                    if self.metrics:
                        self.metrics.set(
                            f"component_health_{component_id}",
                            1 if health_status.get("status") == "healthy" else 0,
                        )

                        if "response_time" in health_status:
                            self.metrics.record(
                                f"component_health_response_time_{component_id}",
...
class EnhancedComponentManager:
    ...
    def register_component(
        ...
        **kwargs,
        ...
        component_info = ComponentInfo(metadata=metadata, state=ComponentState.REGISTERED)
    ...
    async def _initialize_component(self, component_id: str) -> None:
        ...
        if component_info.state != ComponentState.REGISTERED:
            self.logger.debug(
                f"Component {component_id} already initialized (state: {component_info.state})"
            )
            return
        ...
        try:
            ...
            instance = self._component_factory.create_component(
                component_info.metadata.component_type, config
            )
            ...
            await self.event_bus.emit(
                ComponentInitialized(
                    component_id=component_id, initialization_time=initialization_time
                )
            )
            ...
            if self.metrics:
                ...
                self.metrics.record(
                    "component_initialization_duration_seconds", initialization_time
                )
            ...
            await self.event_bus.emit(
                ComponentFailed(
                    component_id=component_id, error_message=str(e), error_type=type(e).__name__
                )
            )
            ...
            raise ComponentError(
                f"Failed to initialize component {component_id}: {str(e)}", component_id
            )
    ...
    async def get_component(self, component_id: str) -> Any:
        ...
        if component_info.state != ComponentState.RUNNING:
            raise ComponentError(
                f"Component {component_id} is not running (state: {component_info.state})"
            )
    ...
    async def stop_component(self, component_id: str, timeout: Optional[float] = None) -> None:
        ...
        try:
            ...
            if hasattr(component_info.instance, "stop"):
                await asyncio.wait_for(component_info.instance.stop(), timeout=timeout)

            # Cleanup resources
            if hasattr(component_info.instance, "cleanup"):
                await asyncio.wait_for(component_info.instance.cleanup(), timeout=timeout)
            ...
            if running_components:
                ...
                for batch in shutdown_batches:
                    ...
                    for priority in sorted(
                        batch_by_priority.keys(), key=lambda p: p.value, reverse=True
                    ):
                        tasks = [
                            self.stop_component(comp_id) for comp_id in batch_by_priority[priority]
                        ]
    ...
    def get_component_status(self, component_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if component_id:
            ...
            return {
                ...
                "startup_time": (
                    component_info.startup_time.isoformat() if component_info.startup_time else None
                ),
    ...
    def list_components(self, state_filter: Optional[ComponentState] = None) -> List[str]:
        ...
        if state_filter:
            return [
                comp_id for comp_id, info in self._components.items() if info.state == state_filter
            ]
        else:
    ...
    async def _health_check_callback(self) -> Dict[str, Any]:
        ...
        try:
            total_components = len(self._components)
            running_components = len(
                [info for info in self._components.values() if info.state == ComponentState.RUNNING]
            )
            failed_components = len(
                [info for info in self._components.values() if info.state == ComponentState.FAILED]
            )
    ...
    def __del__(self):
        ...
        try:
            if hasattr(self, "_components") and any(
                info.state == ComponentState.RUNNING for info in self._components.values()
            ):
