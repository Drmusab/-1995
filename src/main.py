...
from src.core.events.event_types import (
    SystemStarted, SystemShutdownStarted, SystemShutdownCompleted,
    ErrorOccurred, ComponentHealthChanged
)
...
class AIAssistant:
    ...
    async def initialize(self):
        ...
        try:
            ...
            await self.event_bus.emit(SystemStarted(
    ...
    async def _health_monitoring_loop(self):
        ...
        while not self.shutting_down:
            try:
                ...
                # Log health status
                healthy_components = sum(1 for c in health_result["components"].values() 
                ...
                # Update metrics
                if self.metrics:
                    self.metrics.set("system_health_score", 
    ...
    async def process_multimodal_input(
        ...
        message = UserMessage(
            ...
            modality=InputModality.MULTIMODAL if len(input_modalities) > 1 else next(iter(input_modalities))
        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "components": {
                    "component_manager": self.component_manager.get_component_status() if self.component_manager else None,
                    ...
                    "workflow_orchestrator": {
                        "active_executions": len(self.workflow_orchestrator.get_active_executions()) 
                            if self.workflow_orchestrator else 0,
                        "available_workflows": len(self.workflow_orchestrator.list_workflows())
                            if self.workflow_orchestrator else 0
                    },
                    "interaction_handler": {
                        "active_interactions": len(self.interaction_handler.get_active_interactions())
                            if self.interaction_handler else 0
                    }
                }
            }
